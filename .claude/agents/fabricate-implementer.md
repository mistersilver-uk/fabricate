...
Sandbox: full access. Implement the scoped change under `src/`, `tests/`, `styles/`, `lang/`, the canonical specs under `openspec/specs/` the delta requires, and related runtime files; run `npm test` and `npm run build` before handoff. Stay within your assigned file ownership when other agents work in parallel.
