...
Sandbox: full access. Implement the scoped change under `src/`, `tests/`, `styles/`, `lang/`, the canonical specs under `openspec/specs/` the delta requires, and related runtime files; run all validation gates the skill lists before handoff — `npm test`, `npm run build`, and the `lint` CI gate, which is `npm run lint` (ESLint) **plus** `npm run lint:css` and `npm run format:check` (Prettier), so ESLint passing alone is not enough. Stay within your assigned file ownership when other agents work in parallel.
