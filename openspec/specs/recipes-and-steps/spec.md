...
This guarantee is **preserved** under essence-block resolution and is restated here because block resolution puts it squarely in play: moving every essence option out of its group's in-place branches into a single last-placed node moves it after the other item branches, never after currency.
A group authored `[essence, currency]` therefore still tries the block branch before any currency option, and a block that cannot fund must backtrack into that group's currency option rather than stranding the group.
- **Deterministic pick:** the greedy author-order assignment whenever one satisfies, otherwise the first satisfying assignment found by an item-level search under a fixed traversal: the set's non-essence groups in author order, followed by the set's essence options as a single last-placed backtrackable **essence block** (per non-essence group, non-currency options in author order then currency; per option, the greedy item subset first then alternatives).
...
It is REQUIRED because the node cap bounds nodes, not work — each node copies and restores the whole available-item ledger, so per-node cost is proportional to held inventory and a wall-clock reading of a resolve cannot separate the two terms.
An operation count is therefore the only deterministic measure of assignment cost, and without it a bound on ingredient-assignment work is unfalsifiable.
...
An essence option is resolved inside the ingredient set's single essence block rather than in its own author position, so every essence requirement in the set draws from one shared pool of consumed units (see `data-models` §Essence-Alternative Consumption).
A multi-option group stays in place on its component/tag branches and enters the block only on the branch where its essence option is taken.
The player steers the block through `essenceAllocation`, which selects **items** rather than options: `optionOverrides` answers "which option does this group use", while `essenceAllocation` answers "which held items fund the block, and how many units of each".
The legacy per-set `IngredientSet.essences` map is a back-compat read for one release (the 1.17.0 migration folds each positive entry into a single-option essence group); it is threshold-only and never consumed, so it never enters an allocation.
