...
4. Create result items, except where a resolved amount is zero: that EMPTY AWARD creates nothing and is stated on the chat card as its own row.
   It writes no award receipt, because no item was created, so it does not reach the run record.
