...
9. **THE ACTOR- AND ITEM-FLAG REMAP IS NOT A REGISTRY ENTRY**, on the same ground `1.30.0` requirement 13 states: `MigrationRunner` reads and writes settings payloads only and holds no Actor or Item handle.
   ...
   **OF THE THREE RUN CONTAINERS, ONLY `craftingRuns` CARRIES `resolvedEssences` OR `essenceEnabled` TODAY**, and the list says so rather than implying three writers exist: `CraftingRunManager.markStepPrepared` is the ONLY writer of either field anywhere in `src/`, and the container it writes is `craftingRuns` alone.
