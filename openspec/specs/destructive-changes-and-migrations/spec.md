...
- **Retiring an alias from the WRITE path requires no migration and no registry entry when its READ fallback is retained.**
  ...
  The `1.25.0` retirement of the Recipe flat `results` alias (issue 1087) is the worked example: no migration, no registry entry, no downgrade loss, and a permanent inbound shim (`data-models/spec.md § Write-Retired Aliases (Read Permanently)`).
