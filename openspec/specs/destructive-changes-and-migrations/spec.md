...
A **structural** residue check runs before any dice engine is consulted, so a residue that would open with `*`/`/`/`%` or end in a binary operator is refused on this path too; the migration passes no engine, so that check is the only guard on what it writes.
