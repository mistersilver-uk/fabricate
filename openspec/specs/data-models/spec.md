...
2. **`category` departs from the plain pattern on its INHERITING branch**: the world category wins IF AUTHORED, and otherwise the local value falls through.
   ...
   **A FLAG WRITE THAT LANDS AHEAD OF A VALUE WRITE THAT THEN FAILS IS ROLLED BACK.**
   ...
   **THE DELETE'S OWN WRITE DOES NOT YET REACH AS FAR AS ITS CONFIRMATION NOW COUNTS, AND THAT ASYMMETRY IS RECORDED AS A KNOWN GAP** (issue 1371, revision 20).
   The system-scope essence delete strips the essence from each `system.components` row and never from the world `essences` section, so a component that carried the essence ONLY by inheriting it keeps resolving a quantity for an essence the system no longer defines — while the confirmation, correctly reading the resolved map, has just counted that component among those the delete affects.
   Correcting the read before the write is the right order, because a GM deciding on a destructive action needs the true reach first; but a confirmation that states a reach the write does not have is requirement 1's rule on a destructive path, and closing it means either stripping the world section too (a world-scope write from a system-scope verb) or narrowing the impact to what the write touches and saying what it leaves.
