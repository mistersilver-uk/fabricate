...
2. **`category` departs from the plain pattern on its INHERITING branch**: the world category wins IF AUTHORED, and otherwise the local value falls through.
   ...
   **A SYSTEM-SCOPE ESSENCE WRITE IS AN OVERRIDE** (issue 1371 r19-store2), and this replaces the earlier reading under which such a write was simply shadowed until a GM found the switch.
   ...
   The flag write is per PAIR rather than per batch, so a refused one stops that pair and leaves the rest of the cohort to land; a refused world-setting write REJECTS rather than answering `false`, so the stop catches as well as testing the answer.
   ...
   **A SAVE THAT MERELY RESTATES THE RESOLVED MAP WRITES NO MAP AT ALL**, and that exemption is what protects authored data: the component editor seeds its steppers from what the system resolves and sends the section on every save whether or not the GM touched the card, so turning an untouched save into an override would flip a switch nobody touched and overwrite the system's dormant own map — which is exactly what an inheriting system falls back to if the world section is later cleared.
   ...
   **THE SYSTEM RULES LIST AND ITS INSPECTOR DRAW THE RESOLVED MAP, NOT THE PERSISTED ROW** (issue 1371 r19-store2).
   The authoring accessor answers the persisted in-system record by design — a merged row would be offered for editing that no writer can save back — so the row projection overlays the read union's answer onto each item card's essences, exactly as the world essence COLOUR is overlaid onto the essence definitions.
