...
19. `CurrencyConfig.spendStrategy` selects how currency is read and spent, once per world.
    ...
    The `wroteNothing` marker is what carries that last distinction and is tested first; three of the four unrunnable macro spellings additionally carry `thrown: true`, solely so that the shipped affordability answer for an unresolvable macro does not move.
    **This MOVES a shipped answer, and that is declared rather than discovered:** a player cancelling a craft in an `actorProperty` world whose `actorPath` is mis-typed now receives a PARTIAL refund rather than a reported full one, with a console error, because a discarded write was never a refund.
    ...
    The published credit answers such a world with _the world cannot express this credit_, never with _the spender declined_, because `increment` is the one macro key of the three that is optional and a missing one is therefore a normal world state rather than a broken one.
