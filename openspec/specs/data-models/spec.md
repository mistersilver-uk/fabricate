...
5. **Creation requires a resolvable source.** A `fabricate.interactable` behaviour is only ever persisted fully-formed (`interactableType`, `sourceUuid`, `systemId` are required with no `initial`). Creating one with no resolvable source (empty `system.sourceUuid`) — e.g. via Foundry's native Region → Behaviors "+ Add Behavior" — is **cancelled** at `preCreateRegionBehavior` with a GM notification pointing at the Fabricate scene control / Interactable browser (and the Manage Interactables panel). A freshly-created interactable behaviour **never inherits another interactable's linked visual**: an inherited `linkedVisual.uuid` (Foundry region-duplication) is neutralised at creation so two interactables never share one marker. The pure decisions live in `src/canvas/regions/interactableCreationGuard.js` and the `preCreateRegionBehavior` Foundry edge in `src/main.js` is a thin, no-throw adapter.
