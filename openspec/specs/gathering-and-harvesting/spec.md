...
A Gathering Task may be placed on the Foundry canvas as a **Scene Region** carrying a `fabricate.interactable` Region Behaviour; a linked Tile (or Drawing / existing Token) is a presentation-only marker (region-only is also supported). There is no synthetic actor or proxy token. Activation is **token presence**: a controlled token entering the region offers the controlling player a non-blocking interact prompt; on Interact the gathering app opens scoped to that task and resolved environment. The behaviour `system` schema, env-resolution precedence, depleted behavior, linked-visual model, and session-scoped tool injection are defined in `openspec/specs/data-models/spec.md` (Canvas Interactables); the gathering-runtime rules a placed task interactable imposes are:

### Requirements

1. **Per-behaviour node state.** A gathering-task interactable carries its own depletion/respawn state in `behavior.system.node` — a drop-time snapshot of the task's node CONFIG (config + runtime), independent of `environment.nodeRuntime[taskId]`. When present it is threaded into start-attempt and listing as a `nodeStateOverride` (via the behaviour-backed node adapter `src/canvas/regions/interactableRegionNodeAdapter.js`) and takes precedence over the environment node runtime for that task. The override is scoped to its own environment+task and must never leak into another listed task. Tool requirements are NOT snapshotted — they resolve from `task.toolIds` against `system.tools` at attempt time. A task with no node config is an UNLIMITED gathering point (`node: null`).
2. **GM-routed writes.** All node-state writes (depletion, respawn, depleted-behavior apply/revert, terminal delete of the linked visual) route through the active GM over the `module.fabricate` socket; only `game.users.activeGM` applies the behaviour/visual update (the GM applies its own writes locally). When a player interacts and no active GM is connected, surface a graceful "a GM must be online to gather here" message rather than silently failing.
3. **Player-facing depleted state.** The gathering app surfaces a behaviour-scoped node's depleted + calendar-aware respawn-ETA state, so a player sees "depleted, respawns in …" rather than an empty list.
...
5. **Timed `onSuccess` decrement at maturity.** For a timed task started from an interactable, the decrement must land on the **behaviour** node at maturity, not the environment node. The waiting run persists only the behaviour **ref** (`economyEvidence.tileNodeRef = { sceneId, regionId, behaviorId }` — the `tileNodeRef`/`tileRef` names are kept as opaque engine seams from the earlier tile model; the payload now carries the region+behaviour ids). At maturity (on the active GM) the engine rebuilds the behaviour-backed node adapter from that ref and threads it into the commit. When the interactable is gone or the ref is unresolvable, the commit falls back to the environment node rather than throwing.
