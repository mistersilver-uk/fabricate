...
A Gathering Task may be placed on the Foundry canvas as an **Interactable Tile** (a `TileDocument`, not an actor-backed token). A player double-clicks the tile to open the gathering app scoped to that task and resolved environment. The structural flag schema, env-resolution precedence, depleted behavior, and session-scoped tool injection are defined in `openspec/specs/data-models/spec.md` (Canvas Interactables); the gathering-runtime rules a placed task tile imposes are:

### Requirements

1. **Per-tile node state.** A gathering-task tile carries its own depletion/respawn state in `tile.flags.fabricate.node` — a drop-time snapshot of the task's node CONFIG (config + runtime), independent of `environment.nodeRuntime[taskId]`. When present it is threaded into start-attempt and listing as a `nodeStateOverride` and takes precedence over the environment node runtime for that task. The override is scoped to its own environment+task and must never leak into another listed task. Tool requirements are NOT snapshotted — they resolve from `task.toolIds` against `system.tools` at attempt time.
2. **GM-routed writes.** All node-state writes (depletion, respawn, depleted-behavior apply/revert, terminal delete) route through the active GM over the `module.fabricate` socket (`INTERACTABLE_NODE_UPDATE` / `INTERACTABLE_NODE_DELETE`); only `game.users.activeGM` applies the `tile.update` / `tile.delete` (the GM applies its own writes locally). When a player double-clicks a task tile and no active GM is connected, surface a graceful "a GM must be online to gather here" message rather than silently failing.
3. **Player-facing depleted state.** The gathering app surfaces a tile-scoped node's depleted + calendar-aware respawn-ETA state, so a player sees "depleted, respawns in …" rather than an empty list.
4. **Drop-time environment resolution.** The tile's `environmentId` is resolved at drop by the precedence chain (tagged Scene Region `flags.fabricate.environmentId` → task `defaultEnvironmentId` → GM dialog; Alt forces the dialog). This is a placement-time decision distinct from the runtime `environment.sceneUuid` gate.
5. **Timed `onSuccess` decrement at maturity.** For a timed task started from a tile, the decrement must land on the **tile** node at maturity, not the environment node. The waiting run persists only the tile **ref** (`economyEvidence.tileNodeRef = { sceneId, tileId }`); at maturity (on the active GM) the engine rebuilds the per-tile adapter from that ref and threads it into the commit. When the tile is gone or the ref is unresolvable, the commit falls back to the environment node rather than throwing.
