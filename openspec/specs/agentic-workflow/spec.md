...
- **AND** `npm run screenshots:ui:publish -- --pr <number>` uploads the collected screenshots with the `gh image` extension and embeds the returned `![pr-<number> ...](https://github.com/user-attachments/assets/...)` markdown into a managed block in the PR body
