...
Under `highest` or `playerPicks`, Modifiers readiness emits one non-blocking warning naming every eligible transformed-quantity entry whose average cannot be compared; the entry remains selectable and rollable, and the warning never becomes `modifierExpressionInvalid`.
