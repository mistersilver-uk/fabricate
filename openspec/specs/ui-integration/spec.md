...
8. **The rules editor expresses the `category` section as ONE FULL-WIDTH SELECT whose FIRST OPTION IS THE INHERIT CHOICE**, spelled `Inherit from world · {value}`, with the note directly beneath it — NOT as the essence screens' switch over a locked value card.
   ...
   **AND MAKING THAT SENTENCE REACHABLE HAS EXPOSED A SECOND DEFECT IN IT, RECORDED AS A KNOWN GAP** (issue 1371, revision 20): the state it now reaches is a roster whose every essence is DISABLED, and the sentence says the system defines none — a statement about a different cause, on requirement 1's own rule that a string may not assert a state that is not the case.
   The card needs the second sentence rather than the first stretched, because "you have defined none" and "you have disabled all of them" send a GM to two different screens.
...
10. While the selection is non-empty, the component browser's right inspector rail renders the bulk edit panel IN PLACE OF the single-component inspector, from the first selected row.
    ...
    **AND A STAGED ESSENCE AXIS WRITES AN OVERRIDE, WHICH IS WHAT MAKES THE PANEL'S OWN SENTENCE TRUE** (issue 1371, revision 19).
    ...
    The flag write is per PAIR and not per batch, so a refused pair is dropped from the value write and the rest of the cohort still lands, and a pair the world corpus holds no membership record for is written unchanged because nothing shadows it.
