...
2. **The catalogue's row states three things in three places: a NAME LINE carrying the source-type pill and, where a world link dangles, a `Broken link` flag; a DESCRIPTION as the second line; and TWO REACH STATS as a trailing cluster of two columns — the recipe count and the membership count as `{members}/{systems}`, each a mono numeral under its own micro-label.**
...
3. **The world Component entry authors the ONE world default `category`, the additive world `tags`, and identity ONLY WHERE THE RECORD HAS NO SOURCE ITEM — and it states how many member systems are affected BEFORE the change lands.**
...
8. **The rules editor expresses the `category` section as ONE FULL-WIDTH SELECT whose FIRST OPTION IS THE INHERIT CHOICE**, spelled `Inherit from world · {value}`, with the note directly beneath it — NOT as the essence screens' switch over a locked value card.
   ...
   **THE ORDER IS THE ANSWER TO A HALF-FAILED SAVE**, since there is no transaction across the two keys and either can land alone.
   ...
   A component draws exactly one such control and NO group chrome.
...
1. **The inherit SECTION SET is read from the SCOPE DESCRIPTOR, never listed per screen.**
   A component has exactly ONE section (`category`), an essence two and a tool two.
   **THE DESCRIPTOR DECIDES THE SET; IT DOES NOT DECIDE THE DRAWING.** The essence and tool editors draw theirs as the shared `InheritRow` switch inside the value card it governs; the component rules editor draws its one section as a single select whose first option is the inherit choice, on `### GM World Component Screens` requirement 8.
