...
10. **THE WORLD CATALOGUE'S BULK PANEL DRAWS EVERY AXIS ON ONE STAGING INSET, AT THE REFERENCE'S GEOMETRY, AND BOTH BULK PANELS DRAW THE SAME INSET** (maintainer rulings M24 and M25, revision 16).
   ...
   **THE PANEL GAINS AN `Essence values` GROUP OVER THE WORLD ESSENCE CATALOGUE'S ROSTER** (M25): each row is an essence with the `n/N` of selected components carrying it and a stepper, staged PER ESSENCE in three states — a value, a strip, or unstaged — and applying writes the staged values into each selected component's rules in EVERY crafting system where that component has rules, through a composed `bulkEditRules` verb published beside the family's join and part verbs, merging per component, skipping an unchanged map and issuing one batch per system per resulting map; a component with no rules in a system is untouched there, and the stage and apply sentences name that write.
   **AND A SYSTEM IS WRITTEN ONLY THE STAGED ESSENCES IT HOLDS** (revision 17): the staged map is narrowed per system to the essence ids on that system's own roster before it is merged, so a system holding none of the staged keys yields no batch, one holding some is written those alone, and a system with no roster holds nothing — the skip shows in the apply's record count, and the rows' `n/N` deliberately keeps counting carriers in the SELECTION rather than per system, because neither number the panel states is per-system and the note already states the rule.
