...
The accessible name of each face contains its visible label, so a speech-input user can activate the control by saying what they can read; the impact statement is programmatically associated with the armed control rather than merely adjacent to it; arming is announced through a live region that exists in the document before it has any text and is cleared when the control disarms; and the control shows a distinct in-progress face for the duration of the write, driven by the caller's own in-flight state and never derived from the armed state.
...
The character figure is a DISTINCT union of the actors the deleting client may write, resolved through the same scope the deletion cascade uses, so the number stated is the number the write reaches.
...
The set write persists through a single recipes write, a single crafting-system write and a single actor-flag pass regardless of set size, in that order, and emits both change signals.
On success it clears the selection and returns the rail to the single-recipe inspector; because that exit unmounts the panel, the completion message is the only surviving feedback and reports every non-zero outcome — recipes deleted, recipe items the prune rewrote, and characters who forgot them.
A write that deleted nothing reports no success, surfaces the failure to the GM, leaves the selection intact and returns the control to its idle face rather than leaving it in progress.
