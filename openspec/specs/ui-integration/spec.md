...
7. **The gateway files carry every seam a later scoped-entity or vocabulary change needs, and are closed to those changes.**
   ...
   **THE ENUMERATION IS AMENDED TO NAME THE EIGHT SEAMS THE COMPONENT FAMILY NEEDED AND IT DID NOT CARRY** (issue 1371), each supplied as a correction under this clause:
   the world entry editor's header ACTION PAIR and its participation in the route-exit guard; the entry's draft, dirty and identity REPORTING WIRES; the catalogue's LIFTED LIST VIEW-STATE, its SYSTEM-RULES DEEP LINK, its GAME-WORLD ITEM ROSTER and the CREATE-FROM-DROP UUID RESOLUTION that roster is dropped against; the world-scope `usage` LEG for the component family; the COMPOSED two-write join and part verbs on the published component write path; the two SYSTEM-SCOPE screens' `onOpenWorldEntry` EXIT into the world entry; the entry's WORLD VOCABULARY EXIT; and the system Component Rules list's ADD-FROM-CATALOGUE PICKER HOST.
   ...
   **IT CARRIES THE COMPOSED WRITE AND NEVER THE RAW MEMBERSHIP ONE**, on `### GM World Component Screens` requirement 6: a dialog writing the membership half alone would leave every adopted component invisible to the very list it was adopted into.
   ...
   The sixth is the pair the other five have no counterpart for, and it is a seam rather than a screen in the exact sense this clause draws: the rules list and the rules editor already existed and already rendered, and what they gained is one prop apiece carrying a route token and an entity id to the gateway that owns routing.
   ...
   Five of the seven post-date the enumeration outright — the list-state lift, the drop resolution, the drop-target roster, the system-scope exit and the vocabulary exit all shipped after it was written — and the other two are values no file outside the two gateways can supply: the action pair is structurally unreachable from a page because `.manager-header` is a sibling of `.manager-main`, and the usage counts are over corpora the projection is not handed.
