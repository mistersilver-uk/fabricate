...
1. **The world Component catalogue and entry are the first surfaces in this repository that WRITE world-scope component identity, and what they write is consumed unevenly.**
   ...
   **AND NO SURFACE MAY ASSERT THE FALSE HALF WHILE THE MERGE IS UNCONSUMED.** Three strings did, and all three are gone.
   ...
   The note now reports only what it can see, how many world tags this RECORD carries, and its key was RENAMED with its sentence: a key still named for the claim its string used to make is how the claim comes back.
   **THE RULE BINDS A COUNT AS TIGHTLY AS A SENTENCE**, and two more claims fell to it after review.
...
2. **The catalogue's row carries TWO REACH STATS on one meta line — the recipe count and the membership count as `{members}/{systems} systems` — and an `Unused` flag when the membership count is zero.**
   ...
   **The flag is `Unused` and never `Broken link`**, because the projection holds no resolution answer and `Unused` is answered exactly by the membership count.
   The stats occupy the shared list frame's ONE second-line slot, so the row states no description and shows the source pill on the UNLINKED row alone; the row's own exit is `Edit component ↗` and the inspector's pinned foot action is `Open catalogue entry`.
...
5. **PER-SYSTEM TAG MUTING IS PUBLISHED AND UNAUTHORED: no component screen offers a control that writes it, and both screens that show the state show it READ-ONLY.**
   ...
   The rules editor still renders the world tag run under the reference's own `From the world · click to mute here` label, and that label is a CAPTION of where muting happens rather than an instruction the card can honour: the chips are not controls, the muted paint still applies so a muted tag reads as muted, and the exit beside them is the route.
...
8. **The rules editor expresses the `category` section as ONE FULL-WIDTH SELECT whose FIRST OPTION IS THE INHERIT CHOICE**, spelled `Inherit from world · {value}`, with the note directly beneath it — NOT as the essence screens' switch over a locked value card.
   ...
   **CHOOSING A CONCRETE CATEGORY WHILE INHERITING IS TWO WRITES**, because the inherit flag is a membership write and the value is a draft edit: clearing the flag has to accompany the value, or the read union re-applies the world value after the in-system re-spread and discards the typed one on the very next read.
