...
3. **The world Component entry authors the ONE world default `category`, the additive world `tags`, and identity ONLY WHERE THE RECORD HAS NO SOURCE ITEM — and it states how many member systems are affected BEFORE the change lands.**
   ...
   **THE `Systems using this component` TABLE TAKES THE REFERENCE'S OWN SURFACES AND INK** (maintainer ruling M15, revision 13): every row on one uniform surface with a hairline separator, the name over its mode caption, the meta sentence in secondary ink, `View system rules ↗` as a bordered neutral button beside a separate square exit control, and a without-rules row on the SAME surface with its dashed `Add to system` — not alternating bands, a third ink and differing button faces.
   **THE `How players see it` TILE DRAWS THE ITEM'S ART AT FULL SIZE IN ITS BOX, AS THE PLAYER APP'S INVENTORY TILE DOES** (maintainer ruling M16, revision 13), and because the tile is the shared preview primitive's own, every catalogue preview follows it; a preview that deliberately draws a glyph rather than art keeps that branch.
...
12. **The shells own the list state machine**: which row is inspected, the selection set across pages, the filter, the sort, the page, and the armed destructive token.
    ...
    **A SHELL MAY OPT INTO INSPECTING THE FIRST SHOWN ROW WHEN NOTHING ELSE RESOLVES** (maintainer ruling M14, issue 1371 revision 13): the world Component catalogue and the system Component Rules list open with their first row inspected rather than a resting panel — a defaulted-off opt-in on the shared composition, byte-identical for the essence and tool catalogues, and a default on the rules list.
    It answers only the question "nothing resolves": a deep link and a remembered selection win over it, and a cohort, filter or sort change never re-selects, so the owner's control stated above is untouched.
...
20. **THE FOOT PAGER RENDERS ONLY WHERE THERE IS MORE THAN ONE PAGE.**
    ...
    **AND IT SPANS THE LIST'S FULL WIDTH** (maintainer ruling M19, issue 1371 revision 13): the world Component catalogue's footer sat narrower than the rows above it, and a footer that does not share the rows' edges reads as a control for something else; the system rules list's footer, which does, is the shape to copy.
