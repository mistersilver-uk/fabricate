...
5. **PER-SYSTEM TAG MUTING IS PUBLISHED AND UNAUTHORED: no component screen offers a control that writes it, and both screens that show the state show it READ-ONLY.**
   ...
   **The state is read BACK in two counted places on the entry** — the `World classification` note's `· muted in {n} systems` clause, which renders only where something IS muted, and each member row's summary, whose tag count is the world list MINUS that system's muted list — so a GM can see a mute that exists and cannot make or clear one.
