...
The recipe-item editor's right rail surfaces the same two gates as **read-only requirement summaries** (issue 544), only when Limited learning is on: the "How players see it" preview card renders a row of read-only "Needs: &lt;name&gt;" chips (Required Knowledge with a `fa-scroll` icon; Learning prerequisites with each prerequisite's own icon), and the "Effective rules" list adds one "Needs: &lt;name&gt;" row per requirement (the character-prerequisite row's sub is the `@path op value` preview).
The player **book detail** (`InventoryDetail`) renders a matching read-only "Needs: &lt;name&gt;" chip row near the access badge from the row's `requirements` array (surfaced by `InventoryListingBuilder`), but there it reflects the acting actor's met/unmet state per requirement (success ramp when met, danger ramp when unmet); it renders only when the book carries requirements (Limited learning on with at least one resolvable gate).
