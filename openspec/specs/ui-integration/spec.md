...
7. **The gateway files carry every seam a later scoped-entity or vocabulary change needs, and are closed to those changes.**
   ...
   The obligation is evaluated on a change REOPENING A GATEWAY FILE TO BUILD A SCREEN, as `git diff --name-only origin/main...HEAD` containing none of those seven paths.
