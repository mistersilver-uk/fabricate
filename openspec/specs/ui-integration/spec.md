...
- THE manager's labelled push-button exists at `src/ui/svelte/components/ManagerButton.svelte`, taking a `role` from a CLOSED set of six: `neutral`, `primary`, `ghost`, `danger`, `dashed` and `warning`.
  ...
  **The repaired site is unreachable, not merely unlinked**, so the vocabulary entry and the amber rule both carry that status rather than reading as a shipped treatment: the labelled Force add sits in that component's standalone Non-matching section, which the markup gates on `mode !== 'manual'`, while its own guard demands `mode === 'manual'`, and the two conditions are mutually exclusive.
  Four branches of that section carry the same contradiction — the row menu's Force add and its library-disabled note, and the row's labelled Force add and its own note — because manual mode grew a separate Available-to-add section carrying an icon Force add that does render, and these were left behind rather than removed.
  The class repair therefore fixes the vocabulary and leaves the reachability defect open: which way to resolve it is a product decision about where a manual force-add belongs, and until it is taken the amber treatment's only rendering consumer is that icon control.
