...
Both remain required, because Core's preview strip consumes the same two fields as an accessible name and a keyboard-visible tooltip.
- **Entering the route in provider mode scrolls the active Downtime sub-item into view**, so the route's first visible state offers a visible screen switcher; it scrolls by the smallest amount that reveals it, and repeats only when the active tab changes.
...
A companion that intends to own its layout owns its scroller explicitly rather than inferring one from its height.
