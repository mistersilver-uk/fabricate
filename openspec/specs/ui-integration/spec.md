...
1. **The world Component catalogue and entry are the first surfaces in this repository that WRITE world-scope component identity, and what they write is consumed unevenly.**
   ...
   **AND M31 HAS MADE THAT SAME STRING STALE AGAIN, WHICH IS RECORDED AS A KNOWN GAP RATHER THAN AS SATISFIED.**
   The shipped subtitle is `New rules start empty; the world category is inherited until this system overrides it.`, and since `1.32.0` a record adopted into a system inherits BOTH sections, because `inherit: {}` reads as inheriting every one the descriptor carries.
   So the new rules do not start empty — they resolve the world's essence values as well as its category — and the sentence denies a reach that exists, which is this requirement's own rule in its denying direction and the second time this one string has fallen to it.
   The string is the fix, not the seed; a lane that takes it names both sections and stops.
