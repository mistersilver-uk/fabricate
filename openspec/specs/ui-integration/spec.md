...
- The selected-system `Crafting` rail item is an expandable nav group modelled on the Gathering group, shown whenever a crafting system is selected.
  ...
  The stated exception is a cross-client republish of the selected system: another connected GM's `visibilityMode` edit replicates to this client and re-projects the selected system, so the reconciliation can redirect away from a dirty Crafting editor and discard its unsaved draft with no prompt.
  The redirect still wins over that draft, because withholding it for a dirty editor would leave the GM on exactly the entry-less route this reconciliation exists to prevent.
