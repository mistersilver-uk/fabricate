...
3. **The world Component entry authors the ONE world default `category`, the additive world `tags`, and identity ONLY WHERE THE RECORD HAS NO SOURCE ITEM — and it states how many member systems are affected BEFORE the change lands.**
   ...
   **ONE SHIPPED STRING STILL SENDS A GM TO THIS CARD TO MINT ONE, AND IT IS RECORDED AS WRONG RATHER THAN AS COPY.** The catalogue bulk panel's empty world-tag state reads `No world tags are authored yet. Add one on a component entry first.`, and no component entry can add one — it is the vocabulary screen that mints, which is this clause.
   It is the same class of defect as requirement 1's struck merge claims and falls to the same rule: a sentence directing a GM to a surface that cannot do what it says costs them the navigation and teaches them the wrong model of where the vocabulary lives.
   The correction is the route this clause already names.
...
6. **The system-scope Component screens carry the full membership model.**
   ...
   **WHAT NO SURFACE YET STATES IS THE RECIPE CASCADE, AND THAT IS RECORDED AS A KNOWN GAP RATHER THAN AS SATISFIED.** The armed control's consequence names the overrides alone (`Its overrides go with it; the world record and every other system are untouched`) and the catalogue's bulk `Remove from` note says only that each component `loses its rules in each chosen system`.
   Both are true and neither is complete, and the omitted half is the destructive one: a GM staging a removal across several systems at once is deciding about recipes no part of that panel shows them.
   A lane closing it states the rewrite and the disable in both places, on requirement 1's own rule that a consequence a control has belongs in that control's accessible name rather than only in a body sentence.
...
16. **DELETING A WORLD ENTITY ANY SYSTEM HAS A MEMBERSHIP RECORD FOR IS REFUSED, and the refusal NAMES the affected systems.**
    ...
    **AND THE WORLD COMPONENT CATALOGUE'S BULK DELETE AUTHORS NO SUCH REFUSAL, WHICH IS RECORDED AS AN OPEN DIVERGENCE FROM THIS REQUIREMENT RATHER THAN AS A SECOND READING OF IT** (issue 1371, revision 8).
    Its armed control calls the same unrefusing write path once per selected record and states the consequence — the world record, its world defaults and every system's rules go, and recipes referencing it stop resolving — but it stops no delete this requirement refuses one route away.
    So a GM told on the entry to remove the component from each system first can take the same action in one click from the catalogue, and the two screens disagree about whether it is allowed.
    **THIS REQUIREMENT IS NOT NARROWED TO THE ENTRY TO ABSORB IT.** Relaxing a destructive-action refusal is a product decision and needs a ruling, not a spec edit made to match what shipped; either the bulk control authors the refusal — naming the affected systems, as the entry's does — or the refusal is deliberately lifted for a bulk action and this requirement records that with its reason.
    A reader meeting the two behaviours before that is settled is meeting a known divergence, not a defect nobody has seen.
