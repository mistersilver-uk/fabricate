...
4. **A world-defaults `category` picker may never offer the reserved general bucket, and refuses it by the shipped case-insensitive predicate rather than by string equality**, because the two spellings are the same category downstream and an unauthored category that minted the bucket now RESETS every inheriting system on the first resolve.
   ...
   **AND AN APPLIED-BUT-UNAUTHORED WORLD TAG IS DRAWN ON THE ENTRY'S RUN, LIT AND STRUCK AND CLEARABLE** (maintainer ruling M33, revision 18), which closes the open question the offer rule above left behind.
   ...
   The unauthored tags are therefore the STAGED list minus the vocabulary, drawn AFTER the vocabulary's chips: LIT because the record applies them, STRUCK because the shared chip's struck face is the reference's own reading for a value switched off in the vocabulary and is the one face this run does not already use, and named `Remove the world tag {tag} (not in the world vocabulary)` so the chip says why it looks unlike its neighbours.
   **NOTHING THE WORLD HAS NOT AUTHORED CAN BE ADDED**, and the chip is drawn from the STAGED list, so clearing one removes it from the run under M34's staging and it is never re-offered — which is the category picker's own rule for the sibling control, where a persisted value outside the vocabulary paints the trigger and clearing it is the one edit offered.
   A vocabulary tag the record applies is lit and NOT struck, so the two states are distinguishable rather than collapsed into one lit face.
