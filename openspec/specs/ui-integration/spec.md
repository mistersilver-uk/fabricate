...
1. **The world Component catalogue and entry are the first surfaces in this repository that WRITE world-scope component identity, and what they write is consumed unevenly.**
   ...
   **The unconsumed half is therefore stated NOWHERE, and that is recorded as a known gap rather than as satisfied.** `componentWorldScopeDisclosure` survives exported with no caller, so the sentence exists and no screen renders it; a lane that restores a disclosure must get it M-numbered first.
   **Two shipped strings go further and assert the false half, and both are RECORDED AS WRONG.** The entry's world-tag note ends `in every system that has rules`, which the resolver-only merge does not deliver; and both `Disclosure*` strings say per-system muting is authored on the entry, which requirement 5 records that no screen does.
...
5. **PER-SYSTEM TAG MUTING IS PUBLISHED AND UNAUTHORED: no component screen offers a control that writes it, and both screens that show the state show it READ-ONLY.**
   ...
   **The state is read BACK in two counted places on the entry** — the `World classification` note's `· muted in {n} systems` clause, and each member row's summary, whose tag count is the world list MINUS that system's muted list — so a GM can see a mute that exists and cannot make or clear one.
   ...
   **A LANE THAT RESTORES A MUTING CONTROL NEEDS AN M-NUMBERED RULING FIRST**, and the two `Disclosure*` strings that already claim the entry authors muting are wrong until it does.
