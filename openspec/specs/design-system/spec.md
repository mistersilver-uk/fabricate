...
Four of those keys are THE CARET'S FIRST wherever the holder is a text field, and the holder takes each of them only from the edge at which the caret would not move: `Home` and `ArrowLeft` only with the caret at offset 0, `End` and `ArrowRight` only with it at the end of the value, never over a selection of any width, and never with `Shift`, `Ctrl`, `Alt` or `Meta` held.
Nothing becomes unreachable, because every cursor movement is still one further keypress away — the behaviour of every editable combobox — and a TRIGGER holder has no caret, so the search-suppressed shape keeps the whole map unconditionally.
