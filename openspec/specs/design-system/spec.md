...
Six components each carried a hand-written copy before issue 1500, and they agreed on the invariant and disagreed on everything around it — which clipping boundary, whether a scroll inside the panel counts, whether a missing `addEventListener` is guarded.
