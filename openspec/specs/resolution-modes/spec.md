...
- A check roll formula ALWAYS carries the system's **check-modifier scalar**, computed from its **`checkModifiers`** catalogue (`{id,label,icon?,expression}[]`) under a **COMBINATION RULE** — persisted as **`defaultModifierPolicy`** — over an eligible id set (`defaultModifierIds`).
The GM authors no placeholder and cannot forget one: a catalogue that reaches a rolled check always contributes.
The rule states BOTH how the eligible values reduce to one number AND **who selects them**, and it has four values: `addAll` (sum the system's own default set; nobody selects), `highest` (deterministic `max(...)` scalar of that same set, NOT a keep-highest dice pool; nobody selects), `byRecipe` ("Recipe picks": the RECIPE author selects, at recipe-edit time), and `playerPicks` (the PLAYER selects, at roll time).
The **SYSTEM owns both axes outright**: a recipe never overrides the rule, and never substitutes its own eligible set except where the rule itself says so.
`byRecipe` is therefore a first-class rule, not a delegation of authority — a recipe chooses WHICH modifiers apply, never HOW they combine.
- A recipe's **`Recipe.craftingModifier`** (`{ modifierIds? } | null`) is that recipe author's PICK, and is consulted **only** under `byRecipe`.
Under `addAll`, `highest` and `playerPicks` a stored `modifierIds` is ignored outright and the system's `defaultModifierIds` is the source.
...
An **authored empty** `modifierIds` (`{ modifierIds: [] }`) is a real pick under `byRecipe` — zero eligible modifiers, so the scalar is 0 and no term is appended for that recipe — distinct from an absent/`null` value, which inherits the system default set; the two are told apart by `Array.isArray` at the point the pick is authored, not by the filtered array's length, so a malformed entry (e.g. a stray non-string id) does not silently flip an authored empty set back to inherit.
- **`craftingCheck.maxModifierPicks`** bounds how many modifiers a **selecting** rule may pick.
It is a positive integer, or ABSENT — and absence means **unlimited**, reported as `Infinity` by `resolveMaxModifierPicks` (`craftingModifierResolver.js`), the single point that decides what every unbounded form (absent, `null`, `0`, non-integer) means.
Absence is meaningful rather than a defaulting accident: a system never asked the question must not silently acquire a bound that truncates recipe picks already on disk.
It applies to `byRecipe` and `playerPicks` only (`policyDefersSelection` is the membership test both authoring surfaces ask rather than re-deriving it) and is meaningless under `addAll`/`highest`, but is stored system-wide regardless of the current rule so flipping between the two selecting rules does not destroy it.
**Both selecting rules SUM what was picked**, so a cap of `1` reproduces the historical single-pick behaviour exactly.
Under `byRecipe` the cap TRUNCATES the resolved eligible list, keeping the first N in authored order — enforced at `resolveEligibleModifierIds`, not only at the picker, so a GM who lowers the cap below what a recipe already picked does not leave that recipe rolling more modifiers than the system now permits, and the recipe's stored picks are not destroyed either.
...
Resolution is **deterministic** for `addAll`, `highest` and `byRecipe` (by the time the engine rolls, a `byRecipe` selection is already made and stored, so the scalar is simply the sum of the already-narrowed, already-capped list), and for `playerPicks` whenever the deferred control below is not offered.
- **The resolved scalar is APPENDED, not substituted**, by the same operation that applies tool bonuses: ONE `+ N[Modifiers]` term, added **before** the string reaches Foundry's `Roll` and feeding **both** evaluation (`checkRoll.js` `evaluateCheckRoll`) and display (`resolveCheckFormulaDisplay`) so the shown formula equals what evaluates.
The term is **sign-aware** — `Constant` is unsigned in the dice grammar, which is why `appendToolBonusTerms`' `sign` + `Math.abs` split is required and correct (`+ -3[Modifiers]` would not parse; `- 3[Modifiers]` does) — **label-sanitized**, **SKIPPED when the value is `0`**, and **formatted through a decimal-safe formatter that refuses exponent notation and non-finite values**, because `Constant = _ [0-9]+ ("." [0-9]+)?` has no exponent production and `+ 1e-7[Modifiers]` would parse as a `StringTerm` and throw at evaluate.
...
Both paths build the modifier context through the one shared `buildCraftingModifierContext`, so a displayed formula cannot disagree with the rolled one on any axis the context carries.
...
(2) a token in a **NON-ADDITIVE** position — multiplicative, function argument, dice-count, lone parenthetical — is **neither stripped nor validated**, and the shim answers `''`, because the position is decided POSITIONALLY and never by asking `Roll.validate` about the residue: `Roll.validate('max(, 2)')` returns **true** (`FunctionTerm`'s head is `Expression?`, so it parses with zero argument terms, `Math.max()` yields `-Infinity` and `Roll#total`'s `Number(this._total) || 0` lets that through), so a validate-driven rule would roll `-Infinity` against the DC on every craft, forever, silently;
...
(4) that additive residue is validated with `Roll.validate` **called as a METHOD**, never detached — a detached `Roll.validate` returns `false` for every formula — and a rejected residue answers `''`;
(5) with `Roll.validate` unavailable (headless, tests) step 4 **fails OPEN** and the additive residue is kept, because answering `''` there would report `noFormula` and silently disable an authored check.
Step 2's answer is engine-independent, so it is identical with or without a dice engine.
- **`resolveActiveCraftingCheckFormula` and `resolveSalvageCheck` apply the shim BEFORE their emptiness test**, so readiness and the roll path can never disagree: a stored `'@craftingmod'` alone reports `noFormula` rather than reporting usable, reaching `evaluateCheckRoll`, stripping to `''` and throwing inside `new Roll('')` as a rolled — and therefore consuming — failure.
- **`playerPicks` is the one rule deferred to ROLL time** (crafting only; salvage and gathering pass no modifier context and never build a modifier choice).
...
`byRecipe` also defers the selection, but to the recipe author at recipe-edit time, so prompting for it would re-ask a question the recipe already answered; the engine's gate therefore tests for `playerPicks` specifically rather than for "some rule defers selection".
...
When any one of the four fails, no choice descriptor is built, the roll prompt renders no modifier fieldset, and the scalar resolves through the deterministic path above.
- When the control IS offered, it renders the eligible modifiers as options (icon + label + resolved numeric value) with a `maxPicks` bound (the resolved cap, clamped to the number of options), and the appended term carries the **SUM of the modifiers the player selects**.
The **best legal selection** is pre-selected — the highest-valued `maxPicks` modifiers, tie-broken by eligible-set order (first-listed among equal values wins) — so a player who simply confirms the roll gets exactly the deterministic scalar a non-interactive craft would have rolled.
At `maxPicks === 1` that pre-selection is the single highest-valued modifier, which is the historical behaviour unchanged.
...
- On any craft where the control is not offered — including every **non-interactive** call (API / macro / headless) — `playerPicks` resolves deterministically to the **best legal selection**: the sum of the highest `maxModifierPicks` values.
At a cap of 1 that is exactly `max(...)` (the historical behaviour), and unbounded it is the plain sum, because picking everything is then legal and optimal.
The scalar is therefore always resolved and API results stay deterministic.
...
The selection's summed value is appended to the working formula **before** the advantage transform, before the situational-bonus append, and before `Roll`, and the displayed formula is recomputed from that same appended string — so the posted roll message and the run journal show exactly what evaluated.
...
`playerPicks` remains crafting-only, so a salvage batch never carries a modifier choice.
