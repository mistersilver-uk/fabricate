...
A placement is **ADDITIVE** — and therefore liftable out of the formula and re-appended as a trailing term — IFF all three hold: it sits at **bracket depth 0**; the run of additive operators immediately before it is **at most one**; and the nearest non-whitespace character after it is **absent or `+`/`-`**.
