...
- A check roll formula MAY reference **`@craftingmod`**, a Fabricate-resolved scalar computed from the system's **`checkModifiers`** catalogue (`{id,label,icon?,expression}[]`) under a **COMBINATION RULE** — persisted as **`defaultModifierPolicy`** — over an eligible id set (`defaultModifierIds`).
...
An **authored empty** `modifierIds` (`{ modifierIds: [] }`) is a real pick under `byRecipe` — zero eligible modifiers, so `@craftingmod` resolves to 0 for that recipe — distinct from an absent/`null` value, which inherits the system default set; the two are told apart by `Array.isArray` at the point the pick is authored, not by the filtered array's length, so a malformed entry (e.g. a stray non-string id) does not silently flip an authored empty set back to inherit.
...
Resolution is **deterministic** for `addAll`, `highest` and `byRecipe` (by the time the engine rolls, a `byRecipe` selection is already made and stored, so the scalar is simply the sum of the already-narrowed, already-capped list), and for `playerPicks` whenever the deferred control below is not offered; `@craftingmod` is resolved to a scalar and substituted **before** the string reaches Foundry's `Roll` (Foundry would otherwise treat an unresolved `@craftingmod` as 0), feeding **both** evaluation (`checkRoll.js` `evaluateCheckRoll`) and display (`resolveCheckFormulaDisplay`) so the shown formula equals what evaluates.
Both paths build that context through the one shared `buildCraftingModifierContext`, so a displayed formula cannot disagree with the rolled one on any axis the context carries.
A formula with no `@craftingmod` token is unchanged (full single-formula back-compat).
- **`playerPicks` is the one rule deferred to ROLL time** (crafting only; salvage and gathering never author `@craftingmod` and never build a modifier choice).
It is the one rule where the eligible set becomes a selection control inside the interactive roll prompt, and `@craftingmod` is substituted only once the player has answered.
...
- The deferred control is offered ONLY when all four of these hold — the call is **interactive** (`interactive === true`), the check's roll formula literally contains the `@craftingmod` token, the system's combination rule is `playerPicks`, and **at least two** modifiers are eligible after catalogue validation.
`CraftingEngine._buildInteractiveModifierChoice` enforces the first three and `buildCraftingModifierChoice` the fourth.
The token condition exists because a formula that never references `@craftingmod` has nowhere to spend the pick, and the two-modifier condition because a control over zero or one option presents no choice at all.
...
- When the control IS offered, it renders the eligible modifiers as options (icon + label + resolved numeric value) with a `maxPicks` bound (the resolved cap, clamped to the number of options), and `@craftingmod` resolves to the **SUM of the modifiers the player selects**.
...
An **empty `chosenModifierIds` array is an answer** ("I picked nothing"), not an absence, so it wins over the pre-selection and reduces `@craftingmod` to 0; only a `null`/`undefined` single id falls through to the next source.
...
`@craftingmod` is therefore never left unresolved and API results stay deterministic.
...
The selection's summed value is substituted into the working formula **before** the advantage transform, before the situational-bonus append, and before `Roll`, and the displayed formula is recomputed from that same substituted string — so the posted roll message and the run journal show exactly what evaluated.
