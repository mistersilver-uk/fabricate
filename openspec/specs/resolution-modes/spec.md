...
Each assembled fragment is validated with `Roll.validate` and a refusal blocks THAT ENTRY ALONE — an authored `1d4[fire` reduces cleanly and the grammar refuses it, and appending it would throw inside `new Roll(...)` as a rolled, therefore CONSUMING, failure where before it merely contributed 0.
