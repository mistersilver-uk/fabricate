...
- A check roll formula MAY reference **`@craftingmod`**, a Fabricate-resolved scalar computed from the system's **`checkModifiers`** catalogue (`{id,label,icon?,expression}[]`) under a **COMBINATION RULE** — persisted as **`defaultModifierPolicy`**, which keeps only three values (`addAll` → sum, `highest` → deterministic `max(...)` scalar, `playerPicks` → the player selects one at roll time; see below) — over an eligible id set (`defaultModifierIds`).
The retired fourth value `byRecipe` conflated the combination rule with WHO decides it; it is now a legacy value, translated to `addAll` on read at both the system level (`normalizeModifierPolicy`'s `LEGACY_POLICY_ALIASES`) and the recipe level (`Recipe._normalizeCraftingModifier`) — both translations exist because either entry point can see a persisted `byRecipe` independently (a hand-built context bypassing `Recipe`, or a recipe built through it) and each guards its own.
A recipe MAY request an override of either axis through **`Recipe.craftingModifier`** (`{policy?, modifierIds?}`; both keys absent, or the value itself absent/`null`, means inherit both axes), but the request is honoured only as far as the system's **`recipeModifierAuthority`** delegates (see below); a stored override the system does not delegate stays on disk and is simply not consulted.
...
Resolution is **deterministic** for `addAll`/`highest` (and for a legacy `byRecipe`, which resolves identically to `addAll` once translated), and for `playerPicks` whenever the deferred pick-one control below is not offered; `@craftingmod` is resolved to a scalar and substituted **before** the string reaches Foundry's `Roll` (Foundry would otherwise treat an unresolved `@craftingmod` as 0), feeding **both** evaluation (`checkRoll.js` `evaluateCheckRoll`) and display (`resolveCheckFormulaDisplay`) so the shown formula equals what evaluates.
A formula with no `@craftingmod` token is unchanged (full single-formula back-compat).
- **`craftingCheck.recipeModifierAuthority`** — the delegation axis, orthogonal to the combination rule above — states how much of the crafting-check modifier decision a system hands to its recipes: `none` (recipes inherit both the eligible set and the combination rule; no per-recipe control is offered), `setOnly` (a recipe may choose its own eligible id subset; the combination rule stays system-wide), or `setAndRule` (a recipe may override both).
A fourth state, **`null`/ABSENT**, means "not yet stamped" and is real and load-bearing, not an oversight: it resolves as `setAndRule` — today's pre-1055 behaviour — so a client where the stamp has not yet run, an imported payload, or an internally hand-built context keeps honouring whatever overrides it honoured before this axis existed.
`resolveRecipeModifierAuthority` (`craftingModifierResolver.js`) is the single enforcement point for what a `null`/absent/unrecognized value resolves to; `resolveModifierPolicy` and `resolveEligibleModifierIds` both consult it before reading a recipe override, so the authority precondition cannot be bypassed by either axis independently.
An **authored empty** `modifierIds` (`{ modifierIds: [] }`) is a real override — zero eligible modifiers, so `@craftingmod` resolves to 0 for that recipe — distinct from an absent/`null` override, which inherits; the two are told apart by `Array.isArray` at the point the override is authored, not by the filtered array's length, so a malformed entry (e.g. a stray non-string id) does not silently flip an authored empty set back to inherit.
Under `setOnly`, a recipe's own narrowed eligible subset is what the two-eligible-modifier floor (below) is evaluated against: a `playerPicks` system whose recipe subset holds fewer than two eligible modifiers resolves deterministically for THAT recipe, exactly as the system-wide floor would with that few modifiers catalogue-wide.
- The crafting-check combination rule (`defaultModifierPolicy`, per-recipe `Recipe.craftingModifier.policy` when the system's authority is `setAndRule`) supports one **deferred** value, **`playerPicks`** (crafting only; salvage and gathering never author `@craftingmod` and never build a modifier choice).
It is the one policy where the eligible set becomes a pick-one control inside the interactive roll prompt, and `@craftingmod` is substituted only once the player has answered.
- The deferred pick-one control is offered ONLY when all four of these hold — the call is **interactive** (`interactive === true`), the check's roll formula literally contains the `@craftingmod` token, the **effective** policy is `playerPicks` (the recipe override when the system's `recipeModifierAuthority` is `setAndRule` and one is authored, else the system default), and **at least two** modifiers are eligible — after catalogue validation AND after `recipeModifierAuthority` has resolved which id set applies (the system's default set at `none`, or the recipe's own subset at `setOnly`/`setAndRule`).
