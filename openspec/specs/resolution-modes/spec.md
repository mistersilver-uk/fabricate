...
- A crafting system carries **ONE named check-modifier catalogue at the SYSTEM level**, `CraftingSystem.checkModifiers`, of `{id, label, expression, icon?, min?, max?}`.
...
A check roll formula ALWAYS carries the resulting **check-modifier scalar**; the GM authors no placeholder and cannot forget one, so a catalogue that reaches a rolled check always contributes.
The rule states BOTH how the eligible values reduce to one number AND **who selects them**, and it has four values: `addAll` (sum the activity's own default set; nobody selects), `highest` (deterministic `max(...)` scalar of that same set, NOT a keep-highest dice pool; nobody selects), **`bySubject`** (the record being resolved selects, at authoring time), and `playerPicks` (the PLAYER selects, at roll time).
`bySubject` replaces the pre-1095 `byRecipe` because the rule's MEANING is activity-independent while its LABEL is not — it renders "By recipe", "By component" or "By gathering row" from the activity.
...
This rule is identical on all three subjects and is enforced by one shared attach (`src/utils/checkModifierPicks.js`), because `GatheringTask` alone is normalized by TWO mirrored whitelist rebuilds and a hand-written copy in either would drift.
- An entry's **`min`** / **`max`** clamp the RESOLVED value, **after expression evaluation and before combination** — so `highest` compares clamped values and `addAll` sums them, and a bound means the same thing under every rule.
...
Containing it to the offending entry is the point: the clamp would otherwise carry that value into the COMBINED scalar, and the appended term is refused whole for exponent notation — so one entry's bound deleted every other eligible modifier's contribution from the roll, including well-formed ones on a different activity's selection.
...
Resolution is **deterministic** for `addAll`, `highest` and `bySubject` (by the time the engine rolls, a `bySubject` selection is already made and stored, so the scalar is simply the sum of the already-narrowed, already-capped, already-clamped list), and for `playerPicks` whenever the deferred control below is not offered.
...
All three return the same shape, which is what lets the per-activity inert cause (`noCheck` / `noFormula`) be one derivation rather than three.
...
Gathering's seam is on the **FORMULA-ROLLED modes only** (`progressive`, `routed`); the `d100` mode rolls no authored formula and reports the check-modifier catalogue inert with cause `noCheck`.
Gathering's separate d100 `characterModifiers` library is a DIFFERENT concept with percentage-point / multiplicative semantics and is untouched.
**The partition is stated in both directions:** character modifiers do not participate in `progressive` or `routed`.
...
GATHERING does NOT, and the gap is deliberate rather than pending: it threads the modifier CONTEXT through both formula-rolled runners and resolves a `playerPicks` selection through the deterministic best-legal-selection, but builds no roll-time prompt, and the roll-time prompt is deferred to issue 683 with the rest of the seam.
...
The scalar is still always resolved, so a gathering `playerPicks` roll appends the best legal selection rather than nothing.
