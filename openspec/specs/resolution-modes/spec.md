...
`bySubject` replaces the pre-1095 `bySubject` because the rule's MEANING is activity-independent while its LABEL is not — it renders "By recipe", "By component" or "By gathering row" from the activity.
`bySubject` is accepted as a **legacy read alias and is never re-emitted**, exactly as `breakToolsOnFail` reads `consumeCatalystsOnFail`; the `1.22.0` migration rewrites it at the system level.
...
- **`playerPicks` is the one rule deferred to ROLL time**, and it is now available on **all three** activities: each may supply a `modifierChoice`, and their dialogs render the modifier fieldset on the same terms crafting's does.
