...
- **Known limitation — essence attribution uses a narrower tier ladder than bucket attribution (pre-`572`, deferred).** The bucket-once submission resolver (`resolveAlchemySubmissionComponent`, issue 572) composes the shared `resolveComponentForItem` (roles → legacy scalar → raw source-reference fall-through) PLUS a bare top-level `item.registeredItemUuid` supplement PLUS a name fallback, but the essence-accumulation path (`accumulateItemEssences` → `findMatchingComponent`) resolves a submission's component through `resolveComponentForItem` + the name fallback only, WITHOUT the bare-`registeredItemUuid` supplement.
So a submission resolvable SOLELY by that bare-`registeredItemUuid` tier — an id-less legacy/alchemy-formula link that carries no own `flags.fabricate.essences` but whose component DEFINES essences — is bucket-credited to its component by signature matching yet contributes NONE of that component's essences to an essence recipe's essence requirement.
This gap predates issue 572 and is out of its scope: essence accumulation legitimately keys on the item (its essence flags), not the bucket id, so unifying the two ladders is a separate change, not a bucket-once regression.
A future change MAY compose the essence path onto the same tier union.
