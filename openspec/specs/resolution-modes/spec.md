...
- **A roll-shaped expression is legal in the library and BLOCKING in a check.** `isRollExpression` is derived, a check appends one resolved scalar as a dice-grammar `Constant`, and `appendCheckModifierTerm` drops the WHOLE term for a value it cannot express — so a roll-shaped entry an activity SELECTS would delete every other modifier from that roll.
  Readiness raises `modifierRollExpression` at `critical`, naming the entries, for the selecting activities only; a roll-shaped entry referenced solely by a gathering drop row is correct.
...
Containing it to the offending entry is the point: the clamp would otherwise carry that value into the COMBINED scalar, and the appended term is refused whole for exponent notation — so one entry's bound deleted every other eligible modifier's contribution from the roll — including the contribution of well-formed entries selected by a DIFFERENT activity, since all three select over the one shared catalogue.
