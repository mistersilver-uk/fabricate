...
Gathering's seam is on the **FORMULA-ROLLED modes only** (`progressive`, `routed`); the `d100` mode rolls no authored formula and reports the check-modifier catalogue inert with cause `noModifierSupport`.
...
`migrateUnifyModifierLibraries` merges `gatheringConfig.systems[].characterModifiers` into `CraftingSystem.modifiers`, rewrites every reference on drop rows, stamina-cost modifiers and events, and deletes the legacy key, so one library now serves both applications.
