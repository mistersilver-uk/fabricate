...
Tab count badges summarize composition membership, not runtime availability. Tasks and Hazards count records whose `compositionState` is `includedByMatch`, `explicitlyIncluded`, `forceIncluded`, or `includedButUnavailable`; excluded, candidate, non-matching, and library-disabled records do not count. The Validation tab can render multiple numeric chips: critical readiness issues use danger tone, warning readiness issues use warning tone, and the chip text is only the count.

The overview right inspector's Runtime preview uses the same `manager-fact-grid` + `manager-fact-line` text treatment as the Gathering environment library inspector's Environment details summary card.
