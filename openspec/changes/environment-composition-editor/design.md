...
Behaviour rules in UI: automatic task mode and automatic hazard mode keep Included / Excluded / Non-matching sections, with non-matching and library-disabled records in the dedicated paginated Non-matching section. Manual hazard mode also keeps its Matching candidates section. Manual task mode is the exception: it renders only Included in this environment plus a single Available to add section, with no Excluded section and no separate Non-matching section. Available to add orders matching addable or restorable task rows first, then enabled non-matching rows, then library-disabled rows; matching rows use Include, enabled non-matching rows use Force add, and library-disabled rows remain non-addable until enabled in the reusable library. Manual mode can force-add enabled non-matching records; those rows render in the included section as `forceIncluded` with explicit force-included copy. Drag-reorder remains hazard-only with non-drag fallback buttons. Status pills always carry text, never colour alone.
