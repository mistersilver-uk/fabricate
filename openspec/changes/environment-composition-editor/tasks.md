...
- [x] Refine manual task and hazard composition layout so it renders Included plus Available to add only; Available to add orders matching addable records before non-matching/library-disabled records while automatic task and hazard modes keep Excluded and standalone Non-matching.
- [x] Refine editor tab badges so Tasks/Hazards count composition membership instead of runtime availability, and Validation shows separate error and warning chips.
...
- [x] Update environment editor component contract tests for manual task and hazard Available to add copy, section gating, ordering, and preserved include/force-add/library-disabled/open-source actions.
- [x] Update environment editor component tests for composition-based tab badge counts and split validation error/warning badges.
- [x] Run `npm test` (2086 pass / 0 fail).
