...
Nested submenu buttons are rendered below the parent item when expanded. They set `activeGatheringTab` to `environments`, `tasks`, `encounters`, or `settings`, route to the Gathering browser surface, and keep the submenu expanded. The expanded parent group uses a soft container background and non-layout-shifting outline treatment; the parent `Gathering` row and chevron keep the same position when collapsed or expanded. The parent row stays visually neutral, and only the selected child button uses the selected menu-item treatment.
