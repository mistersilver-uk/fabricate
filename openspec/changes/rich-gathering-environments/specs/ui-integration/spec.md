...
13. Stamina authoring MUST expose system-level stamina configuration, including max/current provider strategy, regeneration rule, manual adjustment permissions, and task stamina costs.
...
18. GM screenshots MUST prove environment without scene link, region/biome filters, condition controls, node count/restock, respawn policy, risk/encounter controls, stamina economy controls, validation, and narrow-width stacking.
...
18. Actor Gathering screenshots MUST prove environment search/filtering, region/biome/risk chips, stamina summary, environment detail conditions, task stamina/node availability, potential results, start blockers, gathering log, and narrow-width stacking.
