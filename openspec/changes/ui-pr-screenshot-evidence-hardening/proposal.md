...
- Add a `publish` command (`npm run screenshots:ui:publish`) that uploads collected screenshots to S3 (`pr-screenshots/<number>/`, headless) and idempotently embeds `![pr-<number> ...]` markdown into a managed block in the PR body via `gh pr edit --body-file`.
...
- GitHub Releases / orphan-branch hosting (pollute the repo) — S3 is the chosen headless host. Browser-mode `user-attachments` URLs (still accepted by `check`) are deferred because they need an interactive browser session.
