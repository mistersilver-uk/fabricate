...
- Add a `publish` command (`npm run screenshots:ui:publish`) that uploads collected screenshots with the `gh image` extension and idempotently embeds `![pr-<number> ...](https://github.com/user-attachments/assets/...)` markdown into a managed block in the PR body via `gh pr edit --body-file`.
...
- A non-`gh image` upload host (orphan branch / S3) — native GitHub attachments are the chosen route.
