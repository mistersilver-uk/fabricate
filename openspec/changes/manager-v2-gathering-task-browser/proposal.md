...
Replace the Manager V2 Gathering `Tasks` placeholder with a reusable task-definition browser backed by `gatheringConfig.systems[systemId].tasks`.

## Motivation

GMs can already define reusable gathering task definitions in the underlying config and attach them to environments, but Manager V2 only exposes a placeholder for the library. A browser lets GMs inspect, find, duplicate, delete, and select reusable task definitions before the full authoring form is ready.

## Proposed Change

- Render a task-definition library browser in the Gathering `Tasks` tab only.
...
- Show paginated rows with definition identity, region, biomes, drops count, availability, matching environment count, status, and edit/duplicate/delete actions.
- Track the selected task definition in the Manager V2 root so the right inspector can show reusable task details and drop summaries.
...
- Keep task deletion routed through the existing reusable task delete confirmation behavior.
...
- Adds a store action for reusable gathering task duplication.
...
- Empty reusable task-definition libraries show a create-task prompt that calls the existing task creation action.
...
- Status toggles call the existing reusable task update action.
