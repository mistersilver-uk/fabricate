...
The drop rules section reserves a larger bounded scroll region in the central editor so a GM can scan and tune several rules without losing the inspector. At normal Manager V2 widths, each drop rule stays on one horizontal grid row: the component cell shows thumbnail, name, and reference copy; the chance cell keeps the percent and slider together over a neutral Manager V2 track; quantity stays as an unlabeled positive-integer text input; modifiers render as signed condition chips; and row actions are limited to duplicate/delete. Drop rows fill the card width, suppress horizontal scrolling, and use column headers plus vertical cell separators rather than repeated labels inside row values. Drop row colours follow the component browser row model: neutral surfaces by default, subtle hover, and success/accent selection instead of warning/info special-case styling.

Rendered validation should cover both a normal desktop manager window and a narrow manager width where the editor table remains bounded without horizontal scrolling.
