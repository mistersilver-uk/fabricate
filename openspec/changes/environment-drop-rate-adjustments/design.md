...
hazardDropRateAdjustmentsEnabled: {
  [hazardId]: boolean // false disables applying stored hazard adjustments; absent/true means enabled
}
```
...
When `hazardDropRateAdjustmentsEnabled[hazardId] === false`, the stored hazard adjustment remains on the environment but composes as zero for that hazard.
