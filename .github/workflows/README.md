...
Those checks run the dispatch ref's own scripts, and the tree moves onto `release` only for the merge, because the workflow text comes from the dispatch ref and an older `release` may not carry the exports it imports (issue #1988).
...
Before that read, guard also diagnoses **tester-configuration drift** (issue #1872): it fetches `origin/release`'s `release.s3.config.json` and compares `early-access`'s set of tester group names, each paired with its secret env-var name, against the dispatch ref's own, logging a `::warning::` naming both sides when they disagree, because `early-access` is published only from `release` while this job may run from a different ref.
The comparison reads both configuration shapes (a legacy array of groups sharing one channel secret, or an object keyed by group), so the same pairs in either shape agree, and a declaration it cannot read counts as drift.
...
It builds `v$VERSION` in a worktree beside its own checkout with `.github/actions/release-source/action.yml` and runs the workflow ref's publisher over it, for the reason `release-s3.yml` does below, passing that worktree's commit as `--source-sha`.
...
The payload's `release.version` is the bare version, never `v`-prefixed, because Foundry string-compares a prefixed part against a bare one and can rank an older release above a newer one, and the payload is checked against that bare version before the un-draft.
...
Each channel's `testerGroups` is an object keyed by group, each naming its own `testerSecretEnv`: `closed-beta-2026` on beta, and `apprentice-crafter-2026` and `guild-artisan-2026` on early access (the #1763 rotation renamed the latter from `patrons-2026`); the map's declared names are what the publisher reads.
The tag supplies the built bytes, but the publisher tooling runs from the **workflow ref**: `.github/actions/release-source/action.yml` checks the tag out into its own worktree beside the workflow ref's checkout and installs that tree's dependencies, and the ref's `scripts/release-s3.js` builds it through `--source-root` and stamps its commit through `--source-sha`.
Reading the configuration from the tag made a rotation impossible to apply to an already-minted version (issue #1872), and running the tag's own publisher made a configuration shape newer than the tag unreadable (issue #1988).
The cross-version build contract is therefore the one command every tag's tree must answer: the ref's publisher runs `node scripts/release.js --dist-version <version> --no-zip` in that tree and reads the `dist/module.json` it produces.
...
The same dispatch back-fills a newly added tester group: the targets already carrying that version from the same build resume without a write, and only the added group's target is written (issue #1988).
