...
See [Changing the resolution mode]({% link crafting-systems/index.md %}#feature-toggles).
...
- [Simple Mode]({% link crafting/recipes/simple.md %}): one set of ingredients and one result, with an optional pass or fail check.
- [Progressive Mode]({% link crafting/recipes/progressive.md %}): a skill-check value buys results in difficulty order.
