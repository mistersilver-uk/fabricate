...
With a **Tiered check**, each success outcome tier is assigned its own result set, exactly the way [Routed by check]({% link crafting/recipes/routed.md %}) recipes work.
...
See [Crafting Checks]({% link checks/crafting.md %}) for how the check is authored and rolled.
...
See [Journal]({% link player-app/journal.md %}) for where a character's runs and attempts are listed.
...
- [Simple Mode]({% link crafting/recipes/simple.md %}): standard A + B = C crafting without hidden combinations
- [Visibility & Knowledge]({% link crafting/settings.md %}): the full recipe knowledge system (learning recipes by owning an item or by learning them directly)
- [Crafting Systems]({% link crafting-systems/index.md %}): all system-level settings and feature toggles
