...
- `"ok"` -- visible and craftable
- `"visibility"` -- blocked by player list
- `"knowledge"` -- blocked by knowledge requirements
- `"locked"` -- recipe is locked (non-GM)
- `"missing-system"` -- recipe's system not found
...
Guard check before starting or resuming a crafting run. Blocks the action if visibility requirements are not met.
