...
{: .note }
> **The catalogue is system-level from `1.22.0` (issue 1095).** It used to live at `craftingCheck.checkModifiers`, which made it crafting-owned by construction; salvage and gathering now select over the same entries, so it moved to `CraftingSystem.checkModifiers` and the old key is deleted rather than aliased.
...
> **Downgrading to `1.21.0` LOSES the catalogue**: that build's normaliser is an allowlist that never saw a system-level `checkModifiers`, so it is dropped on the first read and every check modifier stops contributing until a GM re-authors it.
