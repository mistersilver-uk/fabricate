...
| `progressive.awardMode` | `string` | `"equal"` | Progressive award mode: `"equal"`, `"exceed"`, or `"partial"`. |
| `outcomes` | `string[]` | `["fail","pass"]` | Named outcome labels used for routed check routing. |
| `modifiers` | `{id, label, expression, isRollExpression, icon?, min?, max?}[]` | `[]` | **The ONE system-level modifier library** (issue 1117): every activity check selects over it, and every gathering drop row, event and stamina cost references it. A check adds its resolved scalar to the roll automatically as a single flavoured `+ N[Modifiers]` term. Each `expression` is a roll-data fragment evaluated against the acting character (a missing/failed expression contributes `0`, never `NaN`). `isRollExpression` is DERIVED on every normalise and never read from input; an entry it flags is legal on a gathering drop row and BLOCKING on any check that selects it (`modifierRollExpression`), because a check appends one settled number. `icon`, `min` and `max` are stored only when authored, so absence round-trips as key-absent and means unbounded — a bound of `0` is a real bound. `min`/`max` clamp that entry's RESOLVED value, before combination. An authored `min > max` is kept verbatim, raises the blocking `modifierBoundsInverted` readiness issue, and makes the entry contribute `0` until it is fixed. The `1.22.0` migration moves the check catalogue up from `craftingCheck.checkModifiers`; `1.23.0` merges that with the gathering `characterModifiers` library into this one key and deletes both. |
