...
| `progressive.awardMode` | `string` | `"equal"` | Progressive award mode: `"equal"`, `"exceed"`, or `"partial"`. |
| `outcomes` | `string[]` | `["fail","pass"]` | Named outcome labels used for routed check routing. |
| `modifiers` | `{id, label, expression, isRollExpression, icon?, min?, max?}[]` | `[]` | **The ONE system-level modifier library** (issue 1117): every activity check selects over it, and every gathering drop row, event and stamina cost references it. A check adds its eligible entries to the roll automatically as flavoured `[Modifiers]` terms: the flat ones sum into one `+ N[Modifiers]` term and each rolling one is appended as its own `+ (…)[Modifiers]` dice term. Each `expression` is a roll-data fragment evaluated against the acting character (a missing/failed expression contributes `0`, never `NaN`). `isRollExpression` is DERIVED on every normalise and never read from input; it is a DISPLAY classification and never a gate, because a check appends a rolling entry as dice (issue 1118 retired the blocking `modifierRollExpression` readiness issue). `icon`, `min` and `max` are stored only when authored, so absence round-trips as key-absent and means unbounded — a bound of `0` is a real bound. `min`/`max` clamp that entry's contribution — the resolved value for a flat expression, and the ROLLED result for a rolling one, expressed in the formula as `min(max((1d8), -1), 6)`. An authored `min > max` is kept verbatim, raises the blocking `modifierBoundsInverted` readiness issue, and makes the entry contribute `0` until it is fixed. The `1.22.0` migration moves the check catalogue up from `craftingCheck.checkModifiers`; `1.23.0` merges that with the gathering `characterModifiers` library into this one key and deletes both. |
| `defaultModifierPolicy` | `string` | `"addAll"` | The combination rule, carried INDEPENDENTLY by `craftingCheck`, `salvageCraftingCheck` and `gatheringCraftingCheck`: how the eligible modifiers combine into what is added to the roll, **and who selects them**. `"addAll"` takes that activity's default set and `"highest"` takes its single highest-AVERAGING entry — `1d4` (2.5) beats a flat `+2`, and the winner is appended as dice; neither asks anyone to select. `"bySubject"` lets the record being resolved select at authoring time — the recipe, the component or the gathering task, labelled "By recipe" / "By component" / "By gathering task" — and `"playerPicks"` lets the player select at roll time on an interactive attempt; both SUM what was picked, and both are bounded by `maxModifierPicks`. A non-interactive `"playerPicks"` attempt resolves to the best legal selection (the highest-AVERAGING `maxModifierPicks` entries, which is `"highest"` at a cap of `1`). The system owns this field alone — a subject never overrides it. The pre-1095 `"byRecipe"` is read as `"bySubject"` and never written back. An unrecognised value falls back to `"addAll"`. |
| `defaultModifierIds` | `string[]` | `[]` | Library entry ids this activity applies by default. An id naming no entry in the SYSTEM's `modifiers` is dropped (order and de-duplication preserved). Under `"bySubject"` this is what a subject inherits until it picks its own; under `"playerPicks"` it is the menu the player chooses from. |
| `maxModifierPicks` | `number` | key absent | The cap on how many modifiers a selecting rule (`"bySubject"`, `"playerPicks"`) may pick, per activity check. Only a positive integer is stored, so `null`, `0`, `2.5`, and junk all normalise to the same shape: key absent. **Absence means unlimited**, resolved as `Infinity`, and is never defaulted — a check that was never asked must not acquire a bound that truncates subject picks already on disk. Meaningless under `"addAll"`/`"highest"`, but stored regardless of the current rule. Under `"bySubject"` it truncates a subject's resolved picks to the first N in authored order without deleting the rest. |
...
{: .note }
> **Behaviour change (issue 1055).** A recipe's stored `craftingModifier.modifierIds` is now consulted **only** under `defaultModifierPolicy === "bySubject"` (spelled `"byRecipe"` before issue 1095).
> Before this change the eligible-id resolution preferred a recipe's own set under every rule.
> Under `"addAll"`, `"highest"`, and `"playerPicks"` a system now resolves its own `defaultModifierIds` for every recipe, whatever any recipe stored; nothing is deleted, and switching the system to `"bySubject"` applies those picks again.
...
> The other three rules are left unbounded: `"addAll"` and `"highest"` select nothing, and the subject-selecting rule may already have records that picked several modifiers, which a stamp of `1` would silently truncate.
...
{: .note }
> **There is ONE modifier library from `1.23.0` (issue 1117).** It used to live at `craftingCheck.checkModifiers`, which made it crafting-owned by construction; `1.22.0` moved it to `CraftingSystem.checkModifiers` so salvage and gathering could select over the same entries, and `1.23.0` merged that with the gathering `characterModifiers` library into `CraftingSystem.modifiers`.
> Every old key is deleted rather than aliased.
> An id authored in BOTH libraries keeps the check entry's id; the gathering entry is re-keyed with a `-gathering` suffix and every gathering reference to it is rewritten.
> The same migration rewrites `craftingCheck.defaultModifierPolicy === "byRecipe"` to `"bySubject"`, and `migrateExportPayload` applies the identical per-system transform to an imported bundle.
> The move is guarded — an authored system-level catalogue is never clobbered — and it seeds nothing onto a salvage or gathering check that has no block.
> **Downgrading to `1.22.0` LOSES the merged library**: that build's normalisers are allowlists that never saw `CraftingSystem.modifiers`, so it is dropped on the first read — every check modifier stops contributing to every roll AND every gathering drop row, event and stamina cost loses the entry it references, until a GM re-authors both.
...
| Field | Type | Default | Description |
|:------|:-----|:--------|:------------|
| `checkMode` | `string` | `"none"` | How a matched brew is resolved. `"none"` runs no check and always succeeds. `"simple"` runs a pass/fail check from `craftingCheck.simple` (pass produces the success group, fail produces the reserved `role: 'failure'` group). `"tiered"` runs a mandatory routed check from `craftingCheck.routed` and routes by outcome tier, exactly like `routedByCheck`. |
| `learnOnCraft` | `boolean` | `true` | Mark a recipe as learned for the crafting character when a submission matches it. A match learns the recipe whether or not the check passed, and whether the brew resolves immediately or after a time gate. Under `global` visibility this is the only source of discovery a player can reach for themselves, so turning it off leaves every player's Known list empty of everything except a recipe a GM grants through `game.fabricate.grantRecipeKnowledge`. |
