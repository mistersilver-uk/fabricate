...
- **A named progressive check trigger fires**, when a trigger you name from the active progressive check matches the roll, whatever that trigger's own tool-breakage or outcome effects do.
  See [Tool breakage triggers]({% link crafting-checks.md %}#tool-breakage-triggers) for how a trigger's own condition is authored.
...
- [Crafting Systems]({% link crafting-systems.md %}#editing-components).
...
- [Crafting Checks]({% link crafting-checks.md %}#tool-breakage-triggers).
