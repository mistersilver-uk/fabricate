...
When an interactive check posts, one chat message carries the main roll first and the pre-rolls after it under the same visibility mode; an entitled non-secret handoff retains serialized pre-rolls beside the existing main serialized roll, and reconstruction never evaluates them again.
