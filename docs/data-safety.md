---
layout: default
title: How Fabricate Protects Your Worlds & Data
nav_order: 10
---

# How Fabricate Protects Your Worlds & Data

This page explains how Fabricate looks after the data it stores in your world, and the engineering practices that sit behind every version you can install.
It is written for GMs and players who are deciding whether to trust the module with their game.
The goal is to be honest and concrete, not to make promises Fabricate cannot keep.

Fabricate runs on top of Foundry Virtual Tabletop, so some of the protection here comes from the Foundry platform itself.
Foundry decides which versions you are offered through its package registry, stores your Fabricate configuration inside your world's settings, and shows the confirmation dialogs Fabricate asks it to display.
Everything else described below is Fabricate's own engineering, layered on top of that platform.
...
2. If those pass, a real Foundry integration test boots an actual Foundry world and exercises the module end to end.
...
Foundry's package registry also records which Foundry versions each release is compatible with.
Fabricate declares a minimum supported Foundry version, a verified version, and a maximum.
Foundry uses those declarations so you are only offered a Fabricate version that says it works with the Foundry you are running.

---

## Automated testing

Fabricate ships with a large automated test suite, currently over three thousand seven hundred tests.
...
A release candidate cannot be cut unless this check passes cleanly.
...
That gives you a chance to fix the underlying problem first.
...
I work full time in a demanding job, and I have a family and a home to look after, so I do not have eight to ten hours a day to pour into this.
...
On top of all of that, I review what ships and I am accountable for it.

AI makes the work faster.
The engineering rigour and human ownership are what protect your game.
