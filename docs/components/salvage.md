...
This is separate from the recipe crafting check, and a system can have both.
You author it on **Checks › Salvage**.
See [Salvage Checks]({% link checks/salvage.md %}).
...
Each item salvaged this way adds an entry to your character's salvage history in the [Journal]({% link player-app/journal.md %}), and history keeps only your most recent runs, so a large batch can push older salvage entries out of it.
...
A recovered material whose own amount was rolled states that roll beside it too, the same "rolled 1d6 = 4" shape the check total uses, and a roll of zero or lower still gets its own row, naming the roll and stating that nothing was recovered.
...
See [Progressive Mode]({% link crafting/recipes/progressive.md %}#player-result-re-ordering) for how the same setting behaves on recipes.
...
- [Crafting Checks]({% link checks/crafting.md %}).
...
- [Complications]({% link components/complications.md %}).
