...
For richer, fantasy-appropriate Systems browser and manager screenshots, the `full` profile also imports six real exported crafting systems from `scripts/foundry/fixtures/systems/` — Camp Cookery (`simple`), Jeweller's Bench (`progressive`), Masterwork Armory (`routedByCheck`), Runesmith's Forge (`routedByIngredients`), Philosopher's Crucible (`simple`, with essences and multi-step recipes), and Mythwright (`simple`, with real gathering realms, environments, and tasks) — alongside the synthetic Arcane Forge fixture, which remains the behavioural craft/gathering fixture.
