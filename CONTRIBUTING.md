...
As of this writing the registry holds 269 cases: 150 `exact`, 3 `window`, 116 `beyond`.

A change to the lab's own inputs is attributed rather than treated like an ordinary render-file change.
By default a PR touching the case registry, `labActors.js`, `labRunStates.js`, or any other file the lab depends on selects **surface coverage**: one frame of every route and tab the lab renders — every manager route, every player tab, plus the light-theme pair — which is 42 of the 269 publishable cases.
...
A paragraph copied verbatim from the prerelease line can therefore cite a path that arrived with later work and is absent there, and the copy is refused even though its own text was reviewed and merged.
