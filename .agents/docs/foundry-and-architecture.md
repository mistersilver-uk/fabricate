...
Consequence for the writer's own client: the whole `settingChangeBridge` leg — `manager.reload()` plus its replicated announcement — can run between one of that manager's `save()` calls attributing a change and the caller's own notifier draining it.
Because `reload()` and both notifiers are synchronous, no two reloads' deltas can interleave in a one-shot slot, and that is why `src/systems/revisionBookkeeping.js` keeps its reload-delta slot and its pending-attribution set deliberately independent rather than sharing state.
