...
- UI-changing PRs must include smoke-run screenshot evidence for the relevant changed views before opening or updating the PR. Use `npm run screenshots:ui:plan -- --base origin/main` to identify expected views, run `npm run test:foundry` to produce real Foundry screenshots under `test-results/`, then use `npm run screenshots:ui -- --base origin/main --pr <number>` to collect the relevant smoke artifacts into `tmp/pr-screenshots/<number>/`. Upload those screenshots through GitHub's native attachment flow, embed the returned `![pr-<number> ...](https://github.com/user-attachments/assets/...)` image markdown in the PR body's `Screenshots (if applicable)` section, then immediately run `npm run screenshots:ui:clean -- --pr <number>` so PR-scoped screenshots are not committed as repository assets. Artifact references are automation fallback evidence, not the normal visible screenshot handoff. Do not install or use helper upload extensions unless the user explicitly approves that fallback. If capture is blocked, put `SCREENSHOTS_NEEDED: <specific reason and visual change summary>` in the PR body.
...
- For UI-touching PRs, `Screenshots (if applicable)` must embed uploaded GitHub attachment image markdown with `pr-<number>` in the alt text, or include `SCREENSHOTS_NEEDED: <specific reason>`. Do not commit PR-scoped screenshots under docs or other asset directories.
