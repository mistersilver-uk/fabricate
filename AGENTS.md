...
- UI-changing PRs must include generated screenshot evidence for the relevant changed views before opening or updating the PR. Use `npm run screenshots:ui:plan -- --base main` to identify expected views, then use `npm run screenshots:ui -- --base main --pr <number>` to generate focused representative Playwright screenshots directly into `docs/assets/pr-screenshots/pr-<number>/`. Do not run the full Foundry smoke harness just to create PR screenshots unless the user explicitly asks for live Foundry evidence or the UI behavior cannot be represented by focused fixtures. If capture is blocked, put `SCREENSHOTS_NEEDED: <specific reason and visual change summary>` in the PR body.
