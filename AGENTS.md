...
The same holds for an ESLint suppression — `eslint.config.js` sets no `linterOptions`, so ESLint 9's default `reportUnusedDisableDirectives: 'warn'` is live and `--max-warnings=0` turns it into a failure, meaning a stale `eslint-disable` directive fails the gate exactly as a stale `svelte-ignore` does.
ESLint is the ONLY static analysis on `*.svelte` — Prettier and Stylelint still exclude it, Svelte compiler warnings still do not fail the build, and SonarCloud indexes none of it.
