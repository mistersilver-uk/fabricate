...
- UI-changing PRs must include generated screenshot evidence for the relevant changed views before opening or updating the PR. Use `npm run screenshots:ui:plan -- --base origin/main` to identify expected views, then use `npm run screenshots:ui -- --base origin/main --pr <number>` to generate focused representative Playwright screenshots into `tmp/pr-screenshots/<number>/`. Upload those screenshots through GitHub's native attachment flow, embed the returned `![pr-<number> ...](https://github.com/user-attachments/assets/...)` image markdown in the PR body's `Screenshots (if applicable)` section, then immediately run `npm run screenshots:ui:clean -- --pr <number>` so PR-scoped screenshots are not committed as repository assets. Artifact references are automation fallback evidence, not the normal visible screenshot handoff. Do not install or use helper upload extensions unless the user explicitly approves that fallback. Do not run the full Foundry smoke harness just to create PR screenshots unless the user explicitly asks for live Foundry evidence or the UI behavior cannot be represented by focused fixtures. If capture is blocked, put `SCREENSHOTS_NEEDED: <specific reason and visual change summary>` in the PR body.
