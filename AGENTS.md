...
- **`foundry.utils.setProperty` vivifies an intermediate only when it is `=== undefined`, so a `null` or primitive intermediate THROWS instead of being replaced.** `common/utils/helpers.mjs`'s traversal step tests `target[p] === undefined` before descending, so a `null` intermediate is traversed INTO and a primitive intermediate is assigned ONTO — both throw, in strict mode, because the helpers module is ESM regardless of the caller's own mode.
Verified against Foundry 14.361.
Item data built from `Item#toObject()` presents both shapes as the NORM, not the exception (a `null` container field, an integer quantity field), so any dotted write into it from GM- or macro-authored data needs its own guard — an unguarded write can abort mid-operation AFTER earlier side effects (consumption, currency, tool wear) have already committed, not merely fail cleanly at the write.
