...
- `npm run lint` + `npm run format:check` — required ESLint + Prettier gate (the `lint` CI job). Both run over a **staged path scope** (see the `lint`/`format` globs in `package.json`): currently the leaf modules `src/{models,utils,integrations,config,migration,canvas}` + `src/toolBreakageRuntime.js`. `src/systems`, `tests/`, `src/ui/**`, `*.svelte`, `src/main.js`, and `scripts/**` are NOT gated yet — widen a path in its own focused PR only once it passes BOTH ESLint and the SonarCloud quality gate (reformatting counts as new code, so it surfaces pre-existing Sonar findings). Use `npm run lint:fix` / `npm run format` to auto-fix. See the "Linting & formatting" section in `docs/contributing.md`.
