...
- `npm run lint` + `npm run lint:svelte` + `npm run lint:css` + `npm run format:check` + `npm run lint:md` — required ESLint + Svelte ESLint + Stylelint + Prettier + markdownlint gate (the `lint` CI job).
...
Prettier additionally formats every `*.svelte` file under `src/`, via `prettier-plugin-svelte` registered in `.prettierrc.json` (Prettier 3 does not auto-load plugins, so the devDependency alone is not enough).
...
Prettier now formats `*.svelte` as well, so a suppression that must sit on a particular line needs a `<!-- prettier-ignore -->` fence to stay there — see the `{' '}` separators in `ExplainerCard.svelte` and `CraftingSystemManagerRoot.svelte`, where Prettier splits a `<span>` containing an `{#if}` across several lines whatever the print width.
ESLint is still the ONLY static analysis on `*.svelte` — Stylelint still excludes it (scoped `<style>` blocks are not linted), Svelte compiler warnings still do not fail the build, and SonarCloud indexes none of it.
The Prettier half of that exclusion is gone: `format:check` covers `src/**/*.svelte` and fails CI on an unformatted component.
`tests/`, the `.js` under `src/ui/**`, `src/main.js`, and `scripts/**` are NOT gated yet — widen a path in its own focused PR only once it passes BOTH ESLint and the SonarCloud quality gate (reformatting counts as new code, so it surfaces pre-existing Sonar findings).
