...
const CUSTOM_SECONDS_PER_UNIT = (unit) =>
  ({ minutes: 60, hours: 3600, days: 30000, weeks: 150000 })[unit] ?? 3600;
  ...
  return {
    systems: {
      [SYSTEM]: {
        economy: {
          mode: 'stamina',
          stamina: { regen: { policy: 'overTime', unit: 'days', amount: 5 } },
        },
      },
    },
  };
...
function nodeService({ respawn, current = 0, max = 4, secondsPerUnit = null } = {}) {
  ...
  const settings = new Map([
    [SETTING_KEYS.GATHERING_CONFIG, { systems: { [SYSTEM]: { economy: { mode: 'nodes' } } } }],
  ]);
...
describe('gathering — calendar-aware interval lengths', () => {
  it('regenerates stamina on the calendar day length, not the Earth day', async () => {
    const { service } = makeRichState({
      config: staminaDaysConfig(),
      secondsPerUnit: CUSTOM_SECONDS_PER_UNIT,
    });
  ...
  it('default seam reproduces the Earth day and week (no calendar injected)', async () => {
    ...
    assert.equal(
      await days.service.regenerateActorStamina({ actor: a1, systemId: SYSTEM, worldTime: 86399 }),
      null
    );
    const oneDay = await days.service.regenerateActorStamina({
      actor: a1,
      systemId: SYSTEM,
      worldTime: 86400,
    });
    ...
    const weeksConfig = {
      systems: {
        [SYSTEM]: {
          economy: {
            mode: 'stamina',
            stamina: { regen: { policy: 'overTime', unit: 'weeks', amount: 3 } },
          },
        },
      },
    };
    ...
    const oneWeek = await weeks.service.regenerateActorStamina({
      actor: a2,
      systemId: SYSTEM,
      worldTime: 604800,
    });
