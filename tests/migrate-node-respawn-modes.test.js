...
function envWithTaskRespawn(respawn) {
  return {
    ...
    tasks: [{ id: 't1', nodes: { enabled: true, max: 3, current: 1, respawn } }],
  };
}
...
test('maps every legacy policy to the new schema (library + inline + runtime)', () => {
        ...
        tasks: [
          ...
          },
          {
            ...
            nodes: {
              max: 2,
              current: 0,
              respawn: { policy: 'probability', intervalSeconds: 3600, chance: 0.5 },
            },
          },
          {
            id: 'lib-hybrid',
            nodes: {
              max: 2,
              current: 0,
              respawn: { policy: 'manualAndElapsedTime', intervalSeconds: 3600, chance: 0.25 },
            },
          },
  ...
  const environments = [
    {
      ...
      tasks: [
        {
          id: 't-inline',
          nodes: { max: 4, current: 1, respawn: { policy: 'elapsedTime', intervalSeconds: 7200 } },
        },
      ],
      nodeRuntime: {
        't-rt': {
          max: 5,
          current: 2,
          respawn: { policy: 'probability', intervalSeconds: 60, chance: 0.1 },
        },
      },
    },
  ];
  ...
  const libById = Object.fromEntries(
    result.gatheringConfig.systems['sys-a'].tasks.map((t) => [t.id, t.nodes.respawn])
  );

  assert.deepEqual(libById['lib-none'], { policy: 'manual' });
  assert.deepEqual(libById['lib-elapsed'], {
    policy: 'overTime',
    gainMode: 'guaranteed',
    intervalSeconds: 3600,
  });
  assert.deepEqual(libById['lib-prob'], {
    policy: 'overTime',
    gainMode: 'chance',
    intervalSeconds: 3600,
    chance: 0.5,
  });
  assert.deepEqual(libById['lib-hybrid'], {
    policy: 'overTime',
    gainMode: 'chance',
    intervalSeconds: 3600,
    chance: 0.25,
  ...
  assert.deepEqual(result.environments[0].tasks[0].nodes.respawn, {
    policy: 'overTime',
    gainMode: 'guaranteed',
    intervalSeconds: 7200,
  });
  assert.deepEqual(result.environments[0].nodeRuntime['t-rt'].respawn, {
    policy: 'overTime',
    gainMode: 'chance',
    intervalSeconds: 60,
    chance: 0.1,
        ...
        tasks: [
          {
            ...
            nodes: {
              max: 1,
              current: 0,
              respawn: { policy: 'probability', intervalSeconds: 3600, chance: 0.3 },
            },
          },
        ],
      },
...
test('already-migrated records are returned by reference (no churn)', () => {
  ...
  assert.strictEqual(
    result.environments[0].tasks[0].nodes.respawn,
    respawn,
    'unchanged respawn kept by reference'
  );
});
...
test('preserves unrelated fields and runtime anchors when migrating', () => {
    ...
    tasks: [
      {
        ...
        nodes: {
          ...
          respawn: {
            ...
            lastRoll: { worldTime: 1000, chance: 0.5, rolls: [10] },
          },
        },
      },
    ],
  ...
  assert.deepEqual(
    node.respawn.lastRoll,
    { worldTime: 1000, chance: 0.5, rolls: [10] },
    'lastRoll preserved'
  );
});
...
test('passes through tasks with no node config or no respawn block', () => {
  const env = {
    id: 'env-1',
    tasks: [{ id: 'no-nodes' }, { id: 'nodes-no-respawn', nodes: { max: 2, current: 2 } }],
  };
  const result = migrateNodeRespawnModes({}, [env]);
  assert.strictEqual(
    result.environments[0],
    env,
    'environment with nothing to migrate kept by reference'
  );
});
