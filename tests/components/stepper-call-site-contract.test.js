...
describe('Stepper unset-value split (issue 1050, D1a)', () => {
  ...
  it('routes both character-modifier scopes through the one shared bounds row', () => {
    ...
    // 1707 wrote the modifier panel once, the bounds row has ONE call site rather than two, and
    // the per-scope wiring moved up one level to the root's two `<GatheringModifierEditor>` tags.
    ...
    const panels = [...root.matchAll(/<GatheringModifierEditor\b[\s\S]*?\/>/g)].map((tag) => tag[0]);
    assert.equal(panels.length, 2, 'one shared panel per character-modifier scope, no more');
    assert.deepEqual(
      CHARACTER_MODIFIER_BOUNDS_SCOPES.filter(
        (scope) => panels.filter((tag) => tag.includes(scope)).length !== 1
      ),
      [],
      'each scope wires the shared panel to exactly one of its own update functions'
    );
