...
test('manager-v2 gathering task browser defines bounded toolbar and compact table geometry without reorder controls', () => {
  ...
  assert.ok(dropRateWebkitTrackBlock.includes('border: 1px solid transparent;') && dropRateWebkitTrackBlock.includes('background: transparent;'), 'drop chance row WebKit range track should keep a 1 px transparent border so the thumb margin-top: -4px geometry stays correct without painting the native track outside the custom track');
  assert.ok(blockFor('.fabricate-manager-v2 .manager-v2-drop-rate-control input[type="range"]::-moz-range-track').includes('border: 1px solid transparent;'), 'drop chance row Firefox range track should keep a 1 px transparent border to preserve native-track geometry without painting nibs past the custom track');
