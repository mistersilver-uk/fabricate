...
describe('GatheringDetail (center column) mounted behavior', () => {
  ...
  // issue 301 (UI simplification): a nonRegenerating pool surfaces count-bearing
  // permanence copy BEFORE exhaustion (scarcity, current > 0) and reuses the same
  // count-bearing key once exhausted (current <= 0, "0 of M"), distinct from the
  // regenerating "replenishes over time" copy.
  it('detail shows the count-bearing scarcity line for a nonRegenerating node with current > 0', async () => {
    await renderDetail({
      task: {
    ...
    assert.ok(scarce, 'the count-bearing scarcity callout renders before exhaustion');
    assert.ok(scarce.textContent.includes('NodeScarcePermanent'), 'uses the permanence/scarcity key');
    assert.ok(scarce.textContent.includes('"current":3') && scarce.textContent.includes('"max":5'), 'interpolates current/max');
  ...
  it('detail reuses the count-bearing permanence key for an exhausted nonRegenerating node (current <= 0)', async () => {
    await renderDetail({
      task: {
    ...
    assert.ok(callout.textContent.includes('NodeScarcePermanent'), 'reuses the count-bearing permanence key when exhausted');
    assert.ok(callout.textContent.includes('"current":0') && callout.textContent.includes('"max":5'), 'count reads 0 of M');
    assert.ok(!callout.textContent.includes('NodeDepletedRespawns'), 'does NOT show the replenishes-over-time copy');
    assert.equal(target.querySelector('[data-gathering-node-respawn-eta]'), null, 'no respawn ETA for a permanently exhausted node');
