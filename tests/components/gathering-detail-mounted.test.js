...
describe('GatheringDetail (center column) mounted behavior', () => {
  ...
  it('renders the success-chance bar in-line with the right-column Attempt button', async () => {
    ...
    const action = target.querySelector(
      '[data-gathering-task-detail] .gathering-task-detail-action'
    );
    assert.ok(action, 'the inspector action row renders');
    assert.ok(
      action.querySelector('[data-gathering-success-value]'),
      'success-chance bar sits in the action row'
    );
    assert.ok(
      action.querySelector('[data-gathering-attempt]'),
      'attempt button sits in the same action row'
    );
  });
  ...
  it('renders "What you might find" with per-drop mini bars, award/event hints, and expandable modifiers', async () => {
    ...
    const successBar = target.querySelector(
      '[data-gathering-task-detail] .gathering-task-detail-action [data-gathering-success-value]'
    );
    assert.equal(
  ...
  it('shows a blocked task with a lock overlay + callout, and its conditions detail in the right inspector on select', async () => {
    ...
    const attemptWrap = target.querySelector(
      '[data-gathering-task-detail] .gathering-task-detail-attempt-wrap'
    );
    assert.ok(
      (attemptWrap.getAttribute('title') || '').includes('Conditions'),
  ...
  it('renders the blind attempt button and the Discovered Tasks section', async () => {
    ...
    assert.ok(
      blindCard.querySelector('[data-gathering-blind-attempt]'),
      'attempt button lives in the card'
    );
    assert.ok(blindCard.textContent.includes('BlindAttemptPrompt'), 'card shows the blind prompt');
    assert.ok(
      blindCard.querySelector('.gathering-detail-blind-card-divider'),
      'card has the divider'
    );
  ...
  it('detail shows the permanent-exhaustion copy and no respawn ETA for an exhausted node', async () => {
    ...
    assert.equal(
      target.querySelector('[data-gathering-node-respawn-eta]'),
      null,
