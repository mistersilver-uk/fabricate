...
describe('GatheringEventsBrowserView source contract', () => {
  it('renders an event library tabpanel with the expected toolbar filters', () => {
    assert.ok(
      browserSource.includes('class="manager-gathering-panel manager-gathering-panel-events"'),
      'browser should use the event panel class'
    );
    assert.ok(
      browserSource.includes('data-gathering-events-browser'),
      'browser should expose a data attribute hook for tests'
    );
    ...
    assert.ok(
      browserSource.includes('<ManagerSearchField'),
      'browser should render the shared search field'
    );
    ...
    assert.ok(
      browserSource.includes('value={searchTerm}'),
      'browser should render the search term'
    );
    assert.ok(
    ...
    assert.ok(
      browserSource.includes('value={statusFilter}'),
      'browser should expose a status filter'
    );
    assert.equal(
      browserSource.includes('value={regionFilter}'),
      false,
      'region filter is removed (region is geography, not composition)'
    );
    assert.ok(
      browserSource.includes('value={biomeFilter}'),
      'browser should expose a biome filter'
    );
    assert.ok(
      browserSource.includes('value={dangerFilter}'),
      'browser should expose a danger tag filter'
    );
  });

  it('exposes row identity, status toggle, and the create/edit/duplicate/delete actions', () => {
    assert.ok(
      browserSource.includes('data-gathering-event-id={event.id}'),
      'rows should expose a data attribute for assertions'
    );
    assert.ok(
      browserSource.includes('manager-gathering-event-identity'),
      'each row should use an event identity button'
    );
    ...
    assert.ok(
      browserSource.includes('onToggleEventEnabled'),
      'browser should call onToggleEventEnabled'
    );
  });
  ...
  it('renders the card-style row with four column headers (Event / Tags / Status / Actions)', () => {
    const headBlockStart = browserSource.indexOf(
      'manager-table-head manager-gathering-event-table-head'
    );
  ...
  it('renders chips for biome, time, weather, and danger via rowChips', () => {
    ...
    assert.equal(
      browserSource.includes('regionChips('),
      false,
      'region chips are removed (region is geography, not composition)'
    );
    assert.ok(
      browserSource.includes('data-gathering-event-tags'),
      'tags cell exposes a data attribute'
    );
    assert.ok(
      browserSource.includes("icon: 'fa-solid fa-triangle-exclamation'"),
      'danger chips should render a triangle warning icon'
    );
    assert.ok(
      browserSource.includes('{#if chip.icon}<i class={chip.icon} aria-hidden="true"></i>{/if}'),
      'row chip icons should be decorative'
    );
    assert.equal(
      /function\s+activeEnvironmentCount\s*\(/.test(browserSource),
      false,
      'activeEnvironmentCount should be removed'
    );
    assert.equal(
      /function\s+dropRateLabel\s*\(/.test(browserSource),
      false,
      'dropRateLabel should be removed'
    );
  });
  ...
  it('uses a four-column grid for the event table and a scrollable tags cell', () => {
    ...
    assert.ok(
      /[^-]height:\s*\d+px/.test(tagsBlock[0]),
  ...
  it('sizes the event thumbnail to 64px to match the environments browser rows', () => {
    const thumbBlock = css.match(
      /\.manager-gathering-events-table\s+\.manager-gathering-event-identity\s+\.manager-gathering-event-thumb\s*\{[^}]*\}/
    );
  ...
  it('wires event CRUD and selection state through the manager root', () => {
    assert.ok(
      rootSource.includes('selectedGatheringEventId'),
      'manager root should track the selected event id'
    );
    assert.ok(
      rootSource.includes('function selectGatheringEvent'),
      'manager root should expose selectGatheringEvent'
    );
    assert.ok(
      rootSource.includes('function createGatheringEvent'),
      'manager root should expose createGatheringEvent'
    );
    assert.ok(
      rootSource.includes('function duplicateGatheringEvent'),
      'manager root should expose duplicateGatheringEvent'
    );
    assert.ok(
      rootSource.includes('function deleteGatheringEvent'),
      'manager root should expose deleteGatheringEvent'
    );
    assert.ok(
      rootSource.includes('function toggleGatheringEventEnabled'),
      'manager root should expose toggleGatheringEventEnabled'
    );
    assert.ok(
      rootSource.includes('function updateSelectedGatheringEvent'),
      'manager root should expose updateSelectedGatheringEvent'
    );
    assert.ok(
      rootSource.includes('store.duplicateGatheringLibraryEvent'),
      'manager root should call the new store duplicate action'
    );
  });
  ...
  it('renders a "Used in environments" inspector card identical to the task one', () => {
    assert.ok(
      rootSource.includes('data-event-environment-usage'),
      'event inspector should expose the usage card data attribute'
    );
    assert.ok(
      rootSource.includes('manager-event-environment-usage-grid'),
      'event usage tiles should sit in a grid container'
    );
    assert.ok(
      rootSource.includes('manager-event-environment-usage-card'),
      'event usage should render tiled cards'
    );
    assert.ok(
      rootSource.includes('manager-event-environment-usage-thumb'),
      'event usage tile should include a thumbnail image'
    );
    assert.ok(
      rootSource.includes('gatheringEventReferencingEnvironments'),
      'inspector should filter environments referencing the event'
    );
    assert.ok(
      rootSource.includes('enabledEventIds'),
      'usage should be derived from enabledEventIds'
    );
  ...
  it('shows usage tiles as squares in a three-column grid (shared with the task card)', () => {
    ...
    assert.ok(
      gridBlock && /grid-template-columns:\s*repeat\(3,\s*minmax\(0,\s*1fr\)\)/.test(gridBlock[0]),
      'usage grid should be three equal columns'
    );
    const thumbBlock = css.match(/\.manager-task-environment-usage-thumb,[^{]*\{[^}]*\}/);
    assert.ok(
      thumbBlock && /aspect-ratio:\s*1\s*\/\s*1/.test(thumbBlock[0]),
      'usage thumbnails should be square'
    );
  });
});
