...
describe('1505 Notice — the API the library states', () => {
  ...
  it('emits neither control unless it is asked for, and never names one in English', async () => {
    ...
    assert.equal(
      ...
      null,
      'the attribute is OMITTED because an empty aria-label suppresses the name rather than ' +
        'falling back to content — a glyph-only control still owes a `dismissLabel`'
    );
    harness.remount();
  });
