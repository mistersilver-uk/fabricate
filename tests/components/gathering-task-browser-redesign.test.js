...
describe('GatheringTasksBrowserView card-style row', () => {
  it('renders four column headers (Task / Tags / Status / Actions)', () => {
    const headBlockStart = browserSource.indexOf(
      'manager-table-head manager-gathering-task-table-head'
    );
    ...
    assert.ok(
      headBlock.includes('FABRICATE.Admin.Manager.Environment.Tasks.Tags'),
      'Tags column header should be present'
    );
  ...
  it('renders the tags chip area as its own grid cell with a data attribute hook', () => {
    assert.ok(
      browserSource.includes('manager-gathering-task-tags-cell'),
      'tags live in a dedicated tags-cell container'
    );
    assert.ok(
      browserSource.includes('data-gathering-task-tags'),
      'tags cell exposes a data attribute for tests'
    );
    assert.equal(
      browserSource.includes('manager-gathering-task-info-cell'),
      false,
      'info-cell wrapper should be removed in this iteration'
    );
    assert.equal(
      browserSource.includes('manager-gathering-task-tags-row'),
      false,
      'tags-row class should be replaced by tags-cell'
    );
  });

  it('renders all tag dimensions in a single chip row via rowChips', () => {
    assert.equal(
      browserSource.includes('regionChips(task)'),
      false,
      'region chip helper removed (region is geography, not composition)'
    );
    ...
    assert.ok(
      browserSource.includes('data-gathering-task-tags'),
      'tags chip row exposes a data attribute'
    );
    assert.ok(
      /manager-availability-pill is-\$\{chip\.kind\}/.test(browserSource),
      'chips render with per-kind variant class'
    );
    assert.equal(
      browserSource.includes("' is-any'"),
      false,
      'unrestricted dimensions render no chip (no is-any placeholder)'
    );
    assert.equal(
      /function\s+anyChip\s*\(/.test(browserSource),
      false,
      'anyChip helper should be removed'
    );
  });

  it('removes the obsolete drops count, env count, and availability helpers', () => {
    assert.equal(
      /function\s+activeEnvironmentCount\s*\(/.test(browserSource),
      false,
      'activeEnvironmentCount helper should be removed'
    );
    assert.equal(
      /function\s+dropSummary\s*\(/.test(browserSource),
      false,
      'dropSummary helper should be removed'
    );
    assert.equal(
      /function\s+availabilityLabels\s*\(/.test(browserSource),
      false,
      'availabilityLabels helper should be removed'
    );
    assert.equal(
      /function\s+availabilityChips\s*\(/.test(browserSource),
      false,
      'availabilityChips helper should be replaced by timeChips/weatherChips'
    );
  });
  ...
  it('keeps drop component name search by retaining dropReferenceText in the haystack', () => {
    ...
    assert.ok(
      haystackMatch[1].includes('${dropReferenceText(task)}'),
      'haystack should still include dropReferenceText so drop names are searchable'
    );
    assert.equal(
      haystackMatch[1].includes('${dropSummary(task)}'),
      false,
      'haystack should not include dropSummary'
    );
  });
  ...
  it('makes the tags cell a scrollable wrapping flex container', () => {
    ...
    assert.ok(
      /display:\s*flex/.test(block[0]),
      'tags cell must be a flex container so chips do not stack vertically'
    );
    assert.ok(
      /flex-wrap:\s*wrap/.test(block[0]),
      'tags cell must wrap so chips spill onto additional rows'
    );
    assert.ok(/align-content:\s*flex-start/.test(block[0]), 'wrapped rows pin to the top');
    assert.ok(
      /overflow-y:\s*auto/.test(block[0]),
      'tags cell must be scrollable so absurd tag counts do not expand the row'
    );
    assert.ok(
      /[^-]height:\s*\d+px/.test(block[0]),
      'tags cell must use a fixed height so all rows match'
    );
  });
  ...
  it('sizes the gathering task thumbnail to 64px to match the environments browser rows', () => {
    const sizeBlock = css.match(
      /\.manager-gathering-tasks-table\s+\.manager-gathering-task-identity\s+\.manager-gathering-task-thumb\s*\{[^}]*\}/
    );
