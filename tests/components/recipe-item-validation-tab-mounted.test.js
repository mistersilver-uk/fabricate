...
describe('RecipeItemValidationTab (mounted)', () => {
  it('flags a missing linked item and missing recipes as blocking, with the summary card + count tiles', async () => {
    ...
    assert.ok(
      root.querySelector(
        '[data-recipe-item-validation-group="requirements"] .manager-recipe-val-group-label'
      )
    );
    ...
    assert.equal(
      root.querySelectorAll('[data-recipe-item-validation-counts] .manager-recipe-rail-count')
        .length,
      2
    );
  ...
  it('passes all checks when linked, with recipes and valid uses', async () => {
    const root = await harness.mount({
      recipeItem: draft({
        linkedRecipeIds: ['r1'],
        caps: { item: { limitUses: true, maxUses: 2 }, learn: {} },
      }),
  ...
  it('shows the uses check only in item mode', async () => {
    const itemRoot = await harness.mount({
    ...
    harness.remount();
    const knowRoot = await harness.mount({
      recipeItem: draft({ linkedRecipeIds: ['r1'] }),
      linkedItem: { uuid: 'Item.a' },
  ...
  it('flags an invalid ntimes learns count in knowledge mode', async () => {
    const root = await harness.mount({
      recipeItem: draft({
        ...
        caps: {
          item: {},
          learn: { limitLearning: true, learningMode: 'ntimes', learnsAllowed: 0 },
        },
      }),
      linkedItem: { uuid: 'Item.a' },
      visibilityMode: 'knowledge',
