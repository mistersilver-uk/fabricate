    ...
    dropRows: [
      {
        ...
        conditionModifiers: {
          weather: [],
          timeOfDay: [],
          biome: [{ conditionId: 'forest', value: 10 }],
        },
      },
      {
        id: 'd2',
        name: 'Iron Ingot',
        dropRate: 45,
        conditionModifiers: { weather: [], timeOfDay: [], biome: [] },
      },
    ],
  };
  const result = await service.previewDropBreakdown({
...
test('taskSuccessChance applies condition modifiers to the eager (no-actor) success chance', async () => {
  ...
  assert.equal(
    service.taskSuccessChance(
      { resolutionMode: 'routed', dropRows: [{ dropRate: 50 }] },
      environment
    ),
    null
  );
    ...
    dropRows: [
      {
        ...
        dropRate: 95,
        conditionModifiers: {
          weather: [{ conditionId: 'rain', value: 40 }],
          timeOfDay: [],
          biome: [],
        },
...
test('issue 299: previewDropBreakdown final chance matches resolveD100Attempt for a multiplicative row', async () => {
  ...
  const resolved = await service.resolveD100Attempt({
    task,
    environment,
    actor: { uuid: 'Actor.x' },
  });
