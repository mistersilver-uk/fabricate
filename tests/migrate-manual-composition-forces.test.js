...
test('the runner runs 1.29.0 from 1.28.0 and bumps the version', async () => {
  ...
  assert.equal(
    ...
    '1.29.0, the 1.30.0 world-scope lift, the 1.31.0 tool-requirement backfill, the 1.32.0 ' +
      'essence election and the 1.33.0 subject-mark seed are all pending'
  );
  assert.equal(ladder.store.get('migrationVersion'), '1.33.0');
...
test('1.29.0 composes with the neighbouring 1.28.0 migration in one pass', async () => {
  ...
  assert.equal(summary.ran, 6, '1.28.0, 1.29.0, 1.30.0, 1.31.0, 1.32.0 and 1.33.0 all run');
  assert.equal(ladder.store.get('migrationVersion'), '1.33.0');
