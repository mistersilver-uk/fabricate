...
test('moves gatheringConfig-only tools onto the matching system and clears the config copy', () => {
  ...
  const gatheringConfig = {
    systems: {
      'sys-1': {
        tasks: [],
        tools: [
          { id: 't1', componentId: 'axe' },
          { id: 't2', componentId: 'saw' },
        ],
      },
    },
  };
  ...
  assert.deepEqual(
    result.systems[0].tools.map((t) => t.id),
    ['t1', 't2']
  );
  assert.equal(
    'tools' in result.gatheringConfig.systems['sys-1'],
    false,
    'config tools copy cleared'
  );
  assert.deepEqual(
    result.gatheringConfig.systems['sys-1'].tasks,
    [],
...
test('skips config tools without an id and tools for an unknown system (orphan-preserve)', () => {
  ...
  assert.deepEqual(
    result.gatheringConfig.systems['sys-missing'].tools.map((t) => t.id),
    ['orphan']
  );
});
...
test('is idempotent: a second run finds nothing to move', () => {
  ...
  assert.deepEqual(
    second.systems[0].tools.map((t) => t.id),
    ['t1']
  );
});
...
test('version gate: 0.7.0 is NOT re-applied when migrationVersion is already 0.7.0', async () => {
  ...
  const config = settings.store.get('gatheringConfig');
  assert.ok(
    'tools' in config.systems['sys-1'],
    'config tools untouched when the 0.7.0 gate blocks the run'
  );
  ...
  assert.ok(
    ...
    'gatheringConfig not persisted (0.8.0/0.9.0 are data no-ops here)'
  );
