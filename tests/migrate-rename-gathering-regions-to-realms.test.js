        ...
        gatheringRegions: [
          {
            ...
            modifiers: [
              { id: 'm1', kind: 'eventChance', operation: 'add', visibility: 'gmOnly', value: 10 },
            ],
          },
        ],
...
test('preserves the Foundry scene-bridge fields and modifier vocab', () => {
  ...
  assert.deepEqual(realm.sceneMappings, [
    { id: 'sm1', sceneUuid: 'Scene.s1', sceneRegionUuid: 'Region.r1' },
  ]);
...
test('does not clobber an already-migrated key; leaves a stale legacy key inert', () => {
  ...
  assert.deepEqual(
    systems[0].gatheringRealms.map((r) => r.id),
    ['already'],
    'existing realms not clobbered'
  );
  assert.ok(
    Array.isArray(systems[0].gatheringRegions),
    'stale legacy key left inert (no data loss)'
  );
});
...
test('migrates each key independently when old and new keys are mixed', () => {
  ...
  assert.deepEqual(
    environments[0].excludedRealmIds,
    ['south'],
    'legacy excludedRegionIds migrated independently'
  );
});
