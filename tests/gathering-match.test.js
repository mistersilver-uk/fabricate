...
test('empty record constraints match any environment and report "any" evidence', () => {
  const { matches, evidence } = evaluateEnvironmentMatch({}, environment, conditions, {
    includeDanger: true,
  });
...
test('overlapping biome tags report a match (region is ignored for composition)', () => {
  ...
  const { matches, evidence } = evaluateEnvironmentMatch(record, environment, conditions, {
    includeDanger: true,
  });
  assert.equal(matches, true);
  assert.equal('region' in evidence, false);
...
test('danger is only applied when includeDanger is set (events)', () => {
  ...
  const taskView = evaluateEnvironmentMatch(record, environment, conditions, {
    includeDanger: false,
  });
  assert.equal(taskView.matches, true);
  assert.equal(taskView.evidence.danger.applicable, false);

  const eventView = evaluateEnvironmentMatch(record, environment, conditions, {
    includeDanger: true,
  });
...
test('danger is a severity ceiling: events at or below the environment level match', () => {
  ...
  const below = evaluateEnvironmentMatch({ dangerTags: ['hazardous'] }, env, conditions, {
    includeDanger: true,
  });
  assert.equal(below.evidence.danger.state, 'match');
  const equal = evaluateEnvironmentMatch({ dangerTags: ['dangerous'] }, env, conditions, {
    includeDanger: true,
  });
  assert.equal(equal.evidence.danger.state, 'match');
  const above = evaluateEnvironmentMatch({ dangerTags: ['deadly'] }, env, conditions, {
    includeDanger: true,
  });
  assert.equal(above.evidence.danger.state, 'mismatch');
  assert.equal(above.matches, false);
  const none = evaluateEnvironmentMatch({ dangerTags: [] }, env, conditions, {
    includeDanger: true,
  });
...
test('an event is ranked by its highest danger tag against the ceiling', () => {
  const env = { dangerLevel: 'dangerous' };
  const mixed = evaluateEnvironmentMatch({ dangerTags: ['safe', 'deadly'] }, env, conditions, {
    includeDanger: true,
  });
...
test('a record whose ONLY constraint was a (now-stripped) region matches any environment', () => {
  ...
  const elsewhere = evaluateEnvironmentMatch(record, otherEnv, conditions, {
    includeDanger: false,
  });
