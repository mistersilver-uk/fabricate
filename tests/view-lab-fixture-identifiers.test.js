...
/**
 * One fixture and one projection per journal case state, built once. Both identity sources below
 * read it, so the run ids and the yield-entry ids they guard come from a single journal build.
 */
const LAB_JOURNAL_STATES = [null, ...Object.keys(LAB_JOURNAL_CASE_STATE_RUN_IDS)].map((state) => {
  const fixture = journalFixture(state);
  return { fixture, listing: journalProjection(state, undefined, fixture) };
});

/** @returns {string[]} Run identities actually generated by the default and focused fixtures. */
function labRunIds() {
  return LAB_JOURNAL_STATES.flatMap(({ fixture }) =>
    Object.values(fixture.containers).flatMap((container) =>
...
/**
 * @returns {string[]} Every `data-yield-entry` value a journal projection really emits.
 *
 * READ FROM THE PROJECTION, for the reason `data-essence-carrier` below is: `YieldScale` stamps
 * `entry.id`, which is `resultRowId || id || drop-N` for a recorded row and the authored row id
 * for an active run's authored yield. Rebuilding those two rules here would admit values the DOM
 * never carries and reject the ones it does.
 */
function labYieldEntryIds() {
  return LAB_JOURNAL_STATES.flatMap(({ listing }) =>
    [...listing.activeRuns, ...listing.history].flatMap((run) =>
      (run.gatheringYield?.entries ?? []).map((entry) => entry.id)
    )
  );
}
...
const IDENTITY_SOURCES = {
  ...
  // The DEFINING evidence of the sixteen `history-data` witnesses (issue 1648, Q-M3): their
  // record-selection step was guarded while the attribute their `expectSelector` reads was not,
  // so a renamed drop row would have failed the capture closed rather than here.
  'data-yield-entry': new Set(labYieldEntryIds()),
