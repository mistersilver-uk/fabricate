...
const PARSE_CASES = [
  [
    'v1.4.0-beta.3',
    { tag: 'v1.4.0-beta.3', version: '1.4.0-beta.3', kind: 'beta', prerelease: 'beta' },
    'the post-cutover prerelease tag the retired RC regex rejected'
  ],
  [
    'v1.0.0-rc.85',
    { tag: 'v1.0.0-rc.85', version: '1.0.0-rc.85', kind: 'beta', prerelease: 'rc' },
    'transitional: rc tags stay valid until the Phase 2 cutover'
  ],
  [
    'v1.4.0',
    { tag: 'v1.4.0', version: '1.4.0', kind: 'stable', prerelease: null },
...
test('the CLI prints the bare version so a workflow can capture it', () => {
  const result = runCli(['v1.4.0-beta.3', '--kind', 'beta']);
  assert.equal(result.stdout.trim(), '1.4.0-beta.3');
});
