...
import {
  normalizeRespawn,
  normalizeDepletedBehavior,
  normalizeNodeConfig,
  VALID_RESPAWN_POLICIES,
  VALID_RESPAWN_GAIN_MODES,
  VALID_RESPAWN_UNITS,
} from '../src/systems/gatheringNodeConfig.js';
...
test('normalizeRespawn maps legacy respawn policies to the manual|overTime schema at read time', () => {
  ...
  assert.deepEqual(
    {
      policy: normalizeRespawn({ policy: 'elapsedTime' }).policy,
      gainMode: normalizeRespawn({ policy: 'elapsedTime' }).gainMode,
    },
...
test('normalizeRespawn trims the amount expression and coerces numeric fields', () => {
  ...
  assert.equal(
    normalizeRespawn({ policy: 'overTime', gainMode: 'expression', amountExpression: 5 })
      .amountExpression,
    ''
  );
});
...
test('normalizeRespawn preserves world-time anchors and lastRoll', () => {
  const out = normalizeRespawn({
    ...
    chance: 0.3,
    lastEvaluatedWorldTime: 1000,
    nextEvaluationWorldTime: 1060,
    lastRoll: { worldTime: 1000, chance: 0.3, rolls: [10] },
  });
...
test('normalizeDepletedBehavior defaults missing/empty to null (no visual change)', () => {
  ...
  assert.equal(
    normalizeDepletedBehavior({ swapImage: '   ' }),
    null,
    'a blank swap image is dropped'
  );
  assert.equal(normalizeDepletedBehavior({ postfixName: false }), null);
});

test('normalizeDepletedBehavior — swap-image only mode', () => {
  assert.deepEqual(normalizeDepletedBehavior({ swapImage: '  icons/x.webp ' }), {
    swapImage: 'icons/x.webp',
  });
});
...
test('normalizeDepletedBehavior — both swap + postfix compose', () => {
  assert.deepEqual(normalizeDepletedBehavior({ swapImage: 'icons/x.webp', postfixName: true }), {
    swapImage: 'icons/x.webp',
    postfixName: true,
  });
});
...
test('normalizeDepletedBehavior — the removed deleteToken field is ignored (swap-image only)', () => {
  ...
  assert.equal(
    normalizeDepletedBehavior({ deleteToken: true }),
    null,
    'a bare deleteToken normalizes to no behavior'
  );
});

test('normalizeNodeConfig carries depletedBehavior through (and omits it when none)', () => {
  const withBehavior = normalizeNodeConfig({
    enabled: true,
    max: 3,
    depletedBehavior: { swapImage: 'icons/x.webp' },
  });
  ...
  assert.ok(
    !('depletedBehavior' in withoutBehavior),
    'no depletedBehavior key when none configured'
  );

  const legacyDeleteNode = normalizeNodeConfig({
    enabled: true,
    max: 3,
    depletedBehavior: { deleteToken: true, swapImage: 'icons/x.webp' },
  });
  assert.deepEqual(
    legacyDeleteNode.depletedBehavior,
    { swapImage: 'icons/x.webp' },
    'legacy deleteToken dropped through the node config'
  );
});
