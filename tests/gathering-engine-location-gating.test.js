...
function task(overrides = {}) {
  return {
    ...
    toolIds: [],
    resultGroups: [
      { id: 'group-a', name: 'Herbs', results: [{ id: 'res', componentId: 'herb', quantity: 1 }] },
    ],
    ...overrides,
  };
}
    ...
    parties: [
      {
        id: 'p1',
        enabled: true,
        memberActorUuids: ['Actor.actor-1'],
        travelActorUuid: 'Actor.t1',
        currentRealmOverride: { mode: 'manual', realmIds: ['r-secret'] },
      },
    ],
  });
  const listing = await engine.listForActor({ viewer, actor });
    ...
    parties: [
      {
        ...
        memberActorUuids: ['Actor.actor-1'],
        travelActorUuid: 'Actor.t1',
        currentRealmOverride: { mode: 'manual', realmIds: ['r-here'] },
      },
    ],
  });
  const listing = await engine.listForActor({ viewer, actor });
...
test('ungated legacy environment is unaffected by location gating', async () => {
  ...
  assert.equal(env.attemptable, true);
  assert.equal(
    env.blockedReasons.some((r) => r.code === 'NO_CURRENT_REALM'),
    false
  );
});
...
test('disabled subsystem: a location-gated environment is NOT blocked and reports gated=false', async () => {
  ...
  assert.equal(
    env.blockedReasons.some((r) => r.code === 'LOCATION_BLOCKED'),
    false
  );
  assert.equal(env.attemptable, true);
});
...
test('disabled subsystem: the start-attempt location guard is skipped', async () => {
  ...
  assert.equal(
    reasons.some((r) => r.code === 'NO_CURRENT_REALM'),
    false,
    'no NO_CURRENT_REALM when disabled'
  );
  assert.equal(
    reasons.some((r) => r.code === 'LOCATION_BLOCKED'),
    false,
    'no LOCATION_BLOCKED when disabled'
  );
});
...
test('realms enabled: the model surfaces realmsEnabled + the party current realm (disclosed)', async () => {
  ...
  assert.equal(
    env.currentRealms[0].label,
    'Verdant Expanse',
    'non-secret current realm name disclosed'
  );
...
test('realms enabled, UNGATED environment still surfaces the party current realm', async () => {
  ...
  assert.equal(
    env.currentRealms[0]?.label,
    'Verdant Expanse',
    'current realm surfaced despite no gating'
  );
});
...
test('listing.realmContext: a secret undiscovered current realm is redacted (no leak in the serialized listing)', async () => {
  ...
  assert.equal(listing.realmContext.enabled, true);
  assert.equal(
    listing.realmContext.realms[0].placeholder,
    true,
    'secret undiscovered realm redacted'
  );
