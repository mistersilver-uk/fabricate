/**
 * Regression gate: no module under `src/**` may re-hardcode the dnd5e/pf2e player-character actor
 * type (issue 1024).
 */
...
// All three quote characters are accepted everywhere a quoted `character` or `type` can appear,
// matching the `quoted path` shape in `quantity-literal-gate.test.js`.
const SHAPES = [
...
describe('the actor-type literal gate can actually fail', () => {
  ...
  it('does NOT flag the `entry.kind === \'character\'` near-miss', () => {
    const findings = findActorTypeLiterals({
      ...
      // The literal in its ONE legitimate home, which is a declaration and not a comparison.
  ...
  it('does NOT flag prose, which is how this design is DOCUMENTED', () => {
    // The earlier revision of this gate split raw text, so a comment naming the retired predicate
    // VERBATIM — the honest way to record what a routed site used to do — was a finding.
    const findings = findActorTypeLiterals({
      'src/fake/prose.js': [
        '/**',
