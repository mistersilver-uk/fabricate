...
test('1.19.0 is version-gated — a later deliberate opt-out is NOT flipped back on', async () => {
  ...
  // reloads. The seed tracks the CURRENT highest version — seeding the older 1.19.0 would
  // leave 1.20.0 unapplied, so `result.ran` would be 1 and the assertion below would be
  // measuring "the later migration ran", not "the 1.19.0 gate held".
  const store = new Map([
