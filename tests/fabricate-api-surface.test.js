...
test('Fabricate exposes the canonical gathering location getters and mutators', () => {
  ...
  assert.ok(
    mainSource.includes('getGatheringLocationService()'),
    'getGatheringLocationService method'
  );
  assert.ok(
    mainSource.includes('getGatheringLocationForActor('),
    'getGatheringLocationForActor method'
  );
  assert.ok(
    mainSource.includes('setGatheringPartyRealmOverride('),
    'setGatheringPartyRealmOverride method'
  );
  assert.ok(
    mainSource.includes('clearGatheringPartyRealmOverride('),
    'clearGatheringPartyRealmOverride method'
  );
  assert.ok(
    mainSource.includes('revealGatheringRealmForActor('),
    'revealGatheringRealmForActor method'
  );
  assert.ok(
    mainSource.includes('hideGatheringRealmForActor('),
    'hideGatheringRealmForActor method'
  );
});
...
test('Fabricate retains the deprecated *Region* delegates that forward to the realm methods', () => {
  ...
  assert.ok(
    mainSource.includes('setGatheringPartyRegionOverride('),
    'setGatheringPartyRegionOverride delegate'
  );
  assert.ok(
    mainSource.includes('clearGatheringPartyRegionOverride('),
    'clearGatheringPartyRegionOverride delegate'
  );
  assert.ok(
    mainSource.includes('revealGatheringRegionForActor('),
    'revealGatheringRegionForActor delegate'
  );
  assert.ok(
    mainSource.includes('hideGatheringRegionForActor('),
    'hideGatheringRegionForActor delegate'
  );
  // The delegates forward to the canonical realm method via the shared deprecate() helper.
  assert.ok(
    mainSource.includes("deprecate('getGatheringRegionStore', 'getGatheringRealmStore')"),
    'getGatheringRegionStore warns + forwards'
  );
  assert.ok(
    mainSource.includes('return this.getGatheringRealmStore();'),
    'delegate forwards to canonical realm method'
  );
});
...
test('game.fabricate.api exposes the canonical realm class + deprecated alias', () => {
  ...
  assert.ok(
    mainSource.includes('GatheringRegionStore: GatheringRealmStore,'),
    'GatheringRegionStore alias in api'
  );
...
test('game.fabricate.gathering exposes the canonical realm helpers', () => {
  assert.ok(
    mainSource.includes('getPartyStore: () => fabricate.getGatheringPartyStore()'),
    'getPartyStore helper'
  );
  assert.ok(
    mainSource.includes('getRealmStore: () => fabricate.getGatheringRealmStore()'),
    'getRealmStore helper'
  );
  assert.ok(
    mainSource.includes(
      'getLocationForActor: (options) => fabricate.getGatheringLocationForActor(options)'
    ),
    'getLocationForActor helper'
  );
  assert.ok(
    mainSource.includes(
      'setPartyRealmOverride: (options) => fabricate.setGatheringPartyRealmOverride(options)'
    ),
    'setPartyRealmOverride helper'
  );
  assert.ok(
    mainSource.includes(
      'revealRealmForActor: (options) => fabricate.revealGatheringRealmForActor(options)'
    ),
    'revealRealmForActor helper'
  );
});
