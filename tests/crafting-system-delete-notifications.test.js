...
test('CraftingSystemManager.deleteSystem emits one summary and suppresses per-recipe delete notices', async () => {
  ...
  manager.systems.set(
    ...
    manager._normalizeSystem({
      ...
      recipeItemDefinitions: [{ id: 'recipe-item-1', name: 'Recipe Scroll' }],
    })
  );
  ...
  assert.deepEqual(
    recipeManager.remainingRecipes().map((recipe) => recipe.id),
    ['recipe-other']
  );
  ...
  assert.deepEqual(notifications, ['Deleted crafting system "Alchemy" and 6 related entities.']);
});
...
test('CraftingSystemManager.deleteSystem stays resilient when one recipe deletion fails', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      recipeManager.deleteCalls.map((call) => call.recipeId),
      ['recipe-1', 'recipe-2']
    );
    // (c) save still ran
