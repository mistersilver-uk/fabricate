/**
 * The smoke and the View Lab must name the same Foundry build (issue #957). These assertions run
...
test('the smoke image is pinned to an exact Foundry build, not a floating major', () => {
  ...
  // gate on whichever unrelated pull request happens to run next.
...
test('the smoke world fixture targets the pinned Foundry build', () => {
  ...
  // file is the only place the world's core version can be fixed.
