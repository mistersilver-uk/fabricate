/**
 * THE STARTUP PRUNE ORDERING (issue 1363). AND IT REACHES PLAYERS.** `cleanupInvalidRuns` is scoped
 * to `selectWritableActors`, so a player client booting after the GM's writeback prunes its OWN
 * actors' runs — and players never run the remap at all.
 */
...
/**
 * An actor whose `getFlag` walks a DOTTED key exactly as Foundry's does and whose `update` applies
 * flattened paths, including Foundry's `-=` deletion syntax.
 */
function makeActor(id, flags = {}) {
  const actor = {
    ...
    getFlag(scope, key) {
      ...
      // Foundry returns a DEEP CLONE, so a caller that mutates what it read cannot reach the stored
      // document.
...
test('a PLAYER client withholds too, because it never runs the remap at all', () => {
  // The remap is active-GM gated, so a player booting after the GM's writeback would prune its OWN
  // actors' runs and nothing would ever repair them.
...
test('the setting-key mirror is PINNED against SETTING_KEYS', () => {
  // `worldScopeRekeyPending.js` spells the key itself rather than importing `SETTING_KEYS`, because
  // `startupPassComposition.js` is documented as reading no globals and importing
  // `src/config/settings.js` there would drag `src/ui/theme.js` into that seam's closure.
...
test('_cleanupCraftingPreferences WITHHOLDS its sweep while a re-key map is pending', async () => {
  // THE OTHER DOOR, and it needs its own end-to-end arm.
...
// `componentIdentityRemap` reads an essence id, and gating there would suppress housekeeping while
// protecting nothing.
