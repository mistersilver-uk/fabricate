 ...
 * - every selection helper returning a NEW `Set`, which is what makes the lifted
 *   browser state propagate.
 *
...
describe('component bulk edit model (issue 772) — describing the selection', () => {
  const describe_ = (pageIds, filteredIds, selectedIds) =>
    describeComponentSelection({ pageIds, filteredIds, selectedIds: new Set(selectedIds) });
...
describe('component bulk edit model (issue 772) — the selection set', () => {
  it('toggleComponentSelection adds, removes, and always returns a NEW Set', () => {
    const empty = new Set();
    const withA = toggleComponentSelection(empty, 'a');
    ...
    const withoutA = toggleComponentSelection(withA, 'a');
    assert.notEqual(withoutA, withA);
    assert.deepEqual([...withoutA], []);
  });

  it('setComponentSelection selects or clears a whole run of ids', () => {
    const selected = setComponentSelection(new Set(['a']), ['b', 'c'], true);
    assert.deepEqual([...selected].sort(), ['a', 'b', 'c']);

    const cleared = setComponentSelection(selected, ['a', 'b'], false);
    assert.notEqual(cleared, selected);
    assert.deepEqual([...cleared], ['c']);
  });

  it('pruneComponentSelection drops ids that no longer resolve to a component', () => {
    // A delete, an unlink or a search change must never leave a phantom id in the
    // count or in an Apply.
    const selected = new Set(['a', 'b', 'c']);
    const pruned = pruneComponentSelection(selected, ['a', 'c', 'd']);
    assert.notEqual(pruned, selected);
    assert.deepEqual([...pruned], ['a', 'c']);
    assert.deepEqual([...selected], ['a', 'b', 'c'], 'the input Set is not mutated');
    assert.deepEqual([...pruneComponentSelection(selected, [])], []);
  });
});
