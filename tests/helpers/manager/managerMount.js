/**
 * Mount the manager root against the store double and land on one route (issue 1669, extracted from
...
export function createManagerMounts({ component, queries, adopt, adoptStore }) {
  ...
  // Mount the manager against a fresh store on a fresh host element.
  ...
  // Mount the manager against a store double, click through to one page, and settle the DOM.
  ...
  // Open WORLD > RULES & RESOURCES > CURRENCY, which is where the currency ladder is authored since
  // issue 1278 and where it was grouped with the two character libraries in issue 1311. It used to
  // be a card on System Settings; the route needs no selected crafting system, because the config
  // it edits is world scope.
  ...
  // Mount the manager and open the tabbed System Overview page for Alchemy with an injected
  // validation report.
