...
export const CHECKS_TREE_RAW_MODULES = Object.freeze([
  ...
  // Issue 1098: `routedOutcomeKeywords.js` now reads the failure-result policy to decide which
  // outcome tiers a result-authoring control may offer, so this leaf joins the closure with it.
  ...
  // `checkRoll.js` evaluates a trigger's condition through the shared breakage evaluator, and that
  // module's own closure is the rest of this block.
  'src/toolBreakageRuntime.js',
  // Issue 1363 (epic 1357, PR 3): `toolBreakageRuntime.js` now resolves the EFFECTIVE tool-breakage
  // authority through the world scope rather than re-defaulting locally, so its closure gains the
  // resolver and the two pure scope modules underneath it.
  ...
  // through the shared read seam as well, so the closure gains the seam and the two scope modules
  // its sibling exports need.
  ...
  // The shared, GM-configurable player-character predicate (issue 1024).
  'src/config/playerCharacterTypes.js',
]);

/**
 * Every `.svelte` module in the checks editor tree, plus the shared primitives those components
 * render.
 */
