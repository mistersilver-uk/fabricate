...
test('the override is no longer keyed by crafting system — a party is in ONE place', () => {
  // This test used to assert the opposite: that an override set for system B did not resolve for
  // system A.
  const service = makeService({
    realms,
    parties: [{ id: 'p1', enabled: true, currentRealmOverride: { mode: 'manual', realmIds: ['r1'] } }]
...
// The three tests that stood here asserted the resolver fast-exiting when a system's travel toggle
// was off (issue 1282).
...
// Auto (travel-actor) sensing: current realm derived live from the marker's token position via
// realm sceneMappings, when there is no manual override.
