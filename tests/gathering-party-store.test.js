...
} from '../src/systems/GatheringPartyStore.js';
...
test('composite invariant: travel actor may also be a member of its own party', async () => {
  const { store } = makeStore();
  const party = await store.create({
    name: 'Heroes',
    memberActorUuids: ['Actor.alice'],
    travelActorUuid: 'Actor.alice',
  });
  const enabled = await store.setEnabled(party.id, true);
  assert.equal(enabled.enabled, true);
