...
test('degradesOnUse:true + destroyWhenExhausted:true maps to limitedUses + destroy', () => {
  ...
  const recipes = [
    recipe('r1', 'sys-1', {
      catalysts: [
        { componentId: 'whetstone', degradesOnUse: true, maxUses: 3, destroyWhenExhausted: true },
      ],
    }),
  ];
...
test('degradesOnUse:true + destroyWhenExhausted:false maps to limitedUses + flagBroken', () => {
  ...
  const recipes = [
    recipe('r1', 'sys-1', {
      catalysts: [
        { componentId: 'whetstone', degradesOnUse: true, maxUses: 5, destroyWhenExhausted: false },
      ],
    }),
  ];
...
test('degradesOnUse:true with null maxUses maps to limitedUses maxUses:null', () => {
  ...
  const recipes = [
    recipe('r1', 'sys-1', {
      catalysts: [
        { componentId: 'anvil', degradesOnUse: true, maxUses: null, destroyWhenExhausted: false },
      ],
    }),
  ];
...
test('semantically different catalysts on the same componentId are NOT merged', () => {
  ...
  assert.deepEqual(modes, [
    'breakageChance:flagBroken',
    'limitedUses:destroy',
    'limitedUses:flagBroken',
  ]);
});
      ...
      steps: [
        {
          ...
          ingredientSets: [
            { id: 'set-a', catalysts: [{ componentId: 'step-set-cat', degradesOnUse: false }] },
          ],
        },
      ],
      ingredientSets: [
        { id: 'set-b', catalysts: [{ componentId: 'recipe-set-cat', degradesOnUse: false }] },
      ],
    }),
  ];
...
test('a salvage catalyst and an identical recipe catalyst in the same system collapse to one tool', () => {
  ...
  const recipes = [
    recipe('r1', 'sys-1', { catalysts: [{ componentId: 'shared', degradesOnUse: false }] }),
  ];
...
test('catalyst systemItemId alias is honored as componentId', () => {
  const systems = [system('sys-1')];
  const recipes = [
    recipe('r1', 'sys-1', { catalysts: [{ systemItemId: 'legacy', degradesOnUse: false }] }),
  ];
  const out = migrateCatalystsToTools(recipes, systems);
...
test('version gate: 0.6.0 conversion is NOT re-applied when migrationVersion is already 0.6.0', async () => {
  ...
  const systems = settings.store.get('craftingSystems');
  assert.equal(
    systems[0].tools?.length ?? 0,
    0,
    'no Tool created when the 0.6.0 conversion is gated'
  );
  ...
  assert.ok(
    ...
    'craftingSystems not persisted (no conversion, no rename)'
  );
  assert.ok(
    !setKeys.includes('gatheringConfig'),
    'gatheringConfig not persisted (no task catalysts here)'
  );
  assert.equal(settings.store.get('migrationVersion'), '1.31.0');
...
test('the dead gathering task.catalysts survives 0.6.0 but is stripped by 1.7.0', async () => {
  const settings = makeSettings({
    ...
    craftingSystems: [{ id: 'sys-1' }],
    gatheringConfig: {
      systems: { 'sys-1': { tasks: [{ id: 't', catalysts: [{ componentId: 'x' }] }] } },
    },
  });
  const runner = new MigrationRunner({
    getSetting: settings.getSetting,
