...
import {
  ...
  SettingDouble,
  SettingHost,
} from './helpers/settingDocumentHost.js';
...
describe('the two O(1) storage keys', () => {
  ...
  it('registers exactly two new keys, and nothing per record', () => {
    ...
    // Issue 1232 landed the conversion that honours the target, so the deferral this loop
    // used to assert for BOTH keys now holds for the layout ALONE. The asymmetry is the
    // concept and is asserted as such in `tests/recipe-storage-target-control.test.js`:
    // the layout records observed state and must never be assertable by hand, while the
    // target is the GM's pre-downgrade reverse-conversion control.
    assert.equal(
      storageKeys.find((entry) => entry.key === 'recipeStorageLayout').definition.config,
      false,
      'the layout is observed state, never a GM-flippable row'
    );
    assert.deepEqual(
