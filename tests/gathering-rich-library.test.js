...
test('task and event libraries match environments by tags and global conditions', async () => {
  ...
  assert.deepEqual(
    composed.tasks.map((task) => task.id),
    ['task-forest-rain']
  );
  assert.equal(composed.tasks[0].resolutionMode, 'd100');
  assert.deepEqual(
    composed.events.map((event) => event.id),
    ['event-thorns']
  );
});
...
test('composeEnvironment carries the system eventVisibility rule onto composed.rules', () => {
          ...
          tasks: [
            {
              id: 'task-a',
              name: 'Forage',
              dropRows: [{ id: 'drop-a', componentId: 'herb', quantity: 1, dropRate: 50 }],
            },
          ],
        },
      },
  ...
  const composed = service.composeEnvironment(environment(), system);
  assert.equal(
    composed.rules.eventVisibility,
    'dangerLevelOnly',
          ...
          tasks: [
            {
              id: 'Task-Mixed',
              name: 'Enabled Mixed',
              dropRows: [{ id: 'drop-a', componentId: 'herb', quantity: 1, dropRate: 100 }],
            },
            {
              id: 'task-other',
              name: 'Other',
              dropRows: [{ id: 'drop-b', componentId: 'herb', quantity: 1, dropRate: 100 }],
            },
          ],
          events: [
...
test('environment blindSelection keeps weights and drops legacy strategy/uuid fields', async () => {
  ...
  const weighted = await store.create(
    environment({
...
test('environment reveal override is discarded — system-level reveal governs', async () => {
  ...
  const withOverride = await store.create(
    environment({
      reveal: { policy: 'onAttempt', scope: 'party' },
      compositionMode: 'manual',
      enabledTaskIds: ['lib-task'],
    })
  );
  ...
  const garbage = await store.create(
    environment({
      id: 'env-reveal-bad',
      reveal: { policy: 'bogus', scope: 'nope' },
      compositionMode: 'manual',
      enabledTaskIds: ['lib-task'],
    })
  );
    ...
    config: {
      systems: {
        'system-a': {
          ...
          events: [
            {
              id: 'event-secret-snake',
              name: 'Secret Snakebite',
              dangerTags: ['hazardous'],
              dropRate: 75,
            },
          ],
          ...
          tasks: [
            {
              ...
              dropRows: [
                { id: 'drop-secret-herb', componentId: 'herb', quantity: 1, dropRate: 100 },
              ],
            },
          ],
          events: [
            {
              id: 'event-secret-thorns',
              name: 'Secret Thorns',
              dangerTags: ['hazardous'],
              dropRate: 100,
            },
...
test('paused gathering rejects before d100 rolls, history, item awards, or rich commits', async () => {
  ...
  assert.deepEqual(
    result.blockedReasons.map((reason) => reason.code),
    ['GAME_PAUSED']
  );
...
test('paused gathering stays listable but marks tasks non-attemptable', async () => {
  ...
  assert.deepEqual(
    listing.blockedReasons.map((reason) => reason.code),
    ['GAME_PAUSED']
  );
  assert.deepEqual(
    listing.environments[0].tasks[0].blockedReasons.map((reason) => reason.code),
    ['GAME_PAUSED']
  );
});
    ...
    config: {
      systems: {
        'system-a': {
          ...
          events: [
            { id: 'event-snake', name: 'Snakebite', dangerTags: ['hazardous'], dropRate: 10 },
          ],
        },
      },
    },
...
test('d100 resolution applies numeric task and event modifier providers', async () => {
      ...
      events: [
        {
          ...
          name: 'Thorns',
          dropRate: 10,
          eventModifier: { provider: 'static', value: 1 },
        },
      ],
    },
  });
  ...
  assert.deepEqual(
    result.items.map((row) => [row.id, row.effectiveRoll, row.modifier]),
    [['drop-modified', 91, 1]]
  );
  assert.deepEqual(
    result.events.map((row) => [row.id, row.effectiveRoll, row.modifier]),
    [['event-modified', 91, 1]]
  );
});
...
test('drop resolution applies matching weather and time modifiers and ignores non-matches', async () => {
  ...
  assert.deepEqual(
    result.items.map((row) => [row.id, row.conditionModifier, row.finalDropRate]),
    [
      ['drop-bonus', 15, 95],
      ['drop-clamped', 25, 100],
    ]
...
test('timed d100 runs complete from their start-time library snapshot after conditions and rules change', async () => {
  ...
  const start = await engine.startAttempt({
    viewer,
    actor,
    environmentId: 'env-a',
    taskId: 'task-timed-rain',
  });
  ...
  assert.equal(
    JSON.stringify(activeListing.activeRuns[0].economyEvidence).includes('runtimeSnapshot'),
    false
  );
  assert.equal(processed.completed.length, 1);
  assert.equal(calls.completed.status, 'succeeded');
  assert.deepEqual(
    calls.created[0].resultGroups[0].results.map((row) => row.id),
    ['drop-rain-herb']
  );
  assert.deepEqual(
    calls.completed.checkResult.events.map((row) => row.id),
    ['event-rain-thorns']
  );
...
test('timed d100 legacy runs snapshot legacy event behavior before rules are authored', async () => {
          ...
          tasks: [
            {
              ...
              timeRequirement: { minutes: 1 },
              dropRows: [
                { id: 'drop-legacy-herb', componentId: 'herb', quantity: 1, dropRate: 100 },
              ],
            },
          ],
          events: [
  ...
  assert.deepEqual(
    calls.completed.checkResult.events.map((row) => row.id),
    ['event-legacy-thorns', 'event-legacy-mud']
  );
});
...
test('interactive d100 confirm: pre-rolls an Nd100 pool, uses its faces, and posts to chat (DSN)', async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      1,
      'the always-dropping row awarded its item from the pooled face'
    );
...
test('interactive d100 situational bonus shifts a borderline drop outcome', async () => {
  ...
  try {
    ...
    assert.equal(
      calls.created.length,
      1,
      '+10 bonus clears the 56 threshold with a 50 face → item awarded'
    );
  } finally {
    restoreRollBonus();
    restoreDialogBonus();
