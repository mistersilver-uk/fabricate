...
import {
  ...
  blocksEnable,
} from '../src/ui/svelte/apps/manager/recipe/recipeReadiness.js';
...
describe('evaluateRecipeReadiness', () => {
  it('flags an empty single-step recipe with critical, enable-blocking issues', () => {
    ...
    assert.ok(
      issues.some(
        (i) => i.id === 'noIngredientSet' && i.severity === 'critical' && i.target === 'ingredients'
      )
    );
    assert.ok(
      issues.some(
        (i) => i.id === 'noResultGroup' && i.severity === 'critical' && i.target === 'results'
      )
    );
  ...
  it('does not warn when a complete recipe is merely disabled', () => {
    ...
    assert.equal(
      issues.some((i) => i.id === 'disabledIncomplete'),
      false,
      'a complete disabled recipe is not flagged incomplete'
    );
  ...
  it('adds the stepsNamed check and per-step gaps for a multi-step recipe', () => {
    ...
    assert.equal(
      issues.some((i) => i.id === 'noResultGroup'),
      false
    );
    assert.equal(blocksEnable(issues), true);
  });
  ...
  it('flags an identical tag match within an OR group regardless of tag order', () => {
    ...
    assert.equal(
      issues.filter((i) => i.id === 'duplicateAlternative').length,
      1,
      'sorted-tag signature collapses tag order'
    );
  });
  ...
  it('does not flag tag matches that differ only by tagMatch', () => {
    ...
    assert.equal(
      issues.some((i) => i.id === 'duplicateAlternative'),
      false,
      'any vs all are distinct matches'
    );
  });
  ...
  it('flags two identical currency alternatives within an OR group as duplicateAlternative', () => {
    ...
    assert.equal(
      issues.filter((i) => i.id === 'duplicateAlternative').length,
      1,
      'identical currency matches collide'
    );
  });
  ...
  it('does not flag currency alternatives with distinct units or amounts', () => {
    ...
    assert.equal(
      issues.some((i) => i.id === 'duplicateAlternative'),
      false,
      'distinct unit/amount are distinct matches'
    );
  });
  ...
  it('flags two identical currency requirements in a set as duplicateRequirement', () => {
      ...
      ingredientSets: [
        {
          ...
          ingredientGroups: [
            {
              id: 'g1',
              options: [{ quantity: 1, match: { type: 'currency', unit: 'gp', amount: 100 } }],
            },
            {
              id: 'g2',
              options: [{ quantity: 1, match: { type: 'currency', unit: 'gp', amount: 100 } }],
    ...
    assert.equal(
      issues.filter((i) => i.id === 'duplicateRequirement').length,
      1,
      'identical currency requirements collide'
    );
  });
      ...
      ingredientSets: [
        {
          ...
          ingredientGroups: [
            ...
            {
              id: 'g2',
              options: [{ quantity: 3, match: { type: 'component', componentId: 'cmp-herb' } }],
            },
          ],
        },
      ],
  ...
  it('reports noDuplicateMatches satisfied and no duplicate issues for all-distinct matches', () => {
    ...
    assert.equal(
      issues.some((i) => i.id === 'duplicateRequirement'),
      false
    );
  ...
  it('does not flag duplicates among options that have no usable match yet', () => {
    ...
    assert.equal(
      issues.some((i) => i.id === 'duplicateAlternative'),
      false,
      'empty matches are not signatures'
    );
    assert.equal(
      issues.some((i) => i.id === 'duplicateRequirement'),
      false
...
describe('evaluateRecipeReadiness alchemy enable blockers', () => {
  ...
  it('does not surface the result-selection blocker for a TIERED alchemy recipe with more than one result set', () => {
    ...
    assert.equal(
      issues.some((i) => i.id === 'alchemyResultSelection'),
      false,
      'tiered skips the cardinality rule'
    );
    assert.equal(
      check(checks, 'alchemyResultSelection'),
      undefined,
      'no cardinality check row under tiered'
    );
  });
  ...
  it('fires the result-selection blocker (without double-firing noResultGroup) when every alchemy result set is a failure set', () => {
    ...
    assert.equal(
      issues.some((i) => i.id === 'noResultGroup'),
      false,
      'noResultGroup does not double-fire'
    );
    assert.equal(
      check(checks, 'hasResultGroup').satisfied,
      true,
      'a present result set satisfies hasResultGroup'
    );
  });

  it('leaves the result-selection check silent for an alchemy recipe with no result sets at all', () => {
    const { checks, issues } = evaluateRecipeReadiness(alchemyRecipe([]), {
      alchemy: { checkMode: 'simple' },
    });
    ...
    assert.equal(
      check(checks, 'alchemyResultSelection'),
      undefined,
      'no vacuously-satisfied cardinality row'
    );
    assert.equal(
      issues.some((i) => i.id === 'alchemyResultSelection'),
      false
    ...
    assert.ok(
      issues.some((i) => i.id === 'noResultGroup'),
      'the empty case stays a noResultGroup gap'
    );
  });
  ...
  it('ignores the reserved failure result set when counting alchemy success result sets', () => {
    ...
    assert.equal(
      issues.some((i) => i.id === 'alchemyResultSelection'),
      false,
      'one success + one failure group is valid'
    );
  ...
  it('surfaces a signature-collision blocker from precomputed conflicts, carrying its coded params', () => {
    const conflict = {
      ...
      params: {
        recipeA: 'Mana Potion',
        recipeB: 'Healing Potion',
        setA: '1',
        setB: '1',
        components: 'Water',
      },
      message:
        'Overlapping signatures between "Mana Potion" and "Healing Potion" (shared components: Water)',
    };
    const { checks, issues } = evaluateRecipeReadiness(alchemyRecipe([{ id: 'r1' }]), {
      alchemy: { checkMode: 'simple' },
      signatureConflicts: [conflict],
    });
    ...
    assert.deepEqual(
      blocker.params,
      conflict.params,
      'carries the coded params so the tab can localize without leaking ids'
    );
  ...
  it('reports an alchemy recipe as ready once the result-selection and signature blockers are resolved', () => {
    ...
    assert.equal(
      issues.some((i) => i.id === 'signatureCollision'),
      false
    );
    assert.equal(blocksEnable(issues), false);
  });
  ...
  it('adds no alchemy checks or blockers for a non-alchemy system', () => {
    ...
    assert.equal(
      issues.some((i) => i.id === 'signatureCollision'),
      false,
      'conflicts are ignored without an alchemy context'
    );
  });
});
  ...
  it('flags a tag requirement overlapping a component it tags as one warning', () => {
    const { checks, issues } = evaluateRecipeReadiness(
      overlapRecipe([
        {
          id: 'g1',
          options: [{ quantity: 1, match: { type: 'component', componentId: 'cmp-iron-ore' } }],
        },
        {
          id: 'g2',
  ...
  it('flags two tag requirements whose tagged components intersect', () => {
    const { issues } = evaluateRecipeReadiness(
      overlapRecipe([
        ...
        {
          id: 'g2',
          options: [{ quantity: 1, match: { type: 'tags', tags: ['ore'], tagMatch: 'any' } }],
        },
      ]),
      { systemComponents }
    );
  ...
  it('does not double-flag exact-duplicate requirements as an overlap', () => {
    const { issues } = evaluateRecipeReadiness(
      overlapRecipe([
        ...
        {
          id: 'g2',
          options: [{ quantity: 3, match: { type: 'tags', tags: ['metal'], tagMatch: 'any' } }],
        },
      ]),
      { systemComponents }
    );
    assert.equal(
      issues.filter((i) => i.id === 'duplicateRequirement').length,
      1,
      'exact dup flagged once'
    );
    assert.equal(
      issues.some((i) => i.id === 'requirementOverlap'),
      false,
      'no overlap double-flag'
    );
  });
                ...
                ingredientGroups: [
                  {
                    id: 'g1',
                    options: [
                      { quantity: 1, match: { type: 'component', componentId: 'cmp-iron-ore' } },
                    ],
                  },
                  {
                    id: 'g2',
  ...
  it('emits exactly one overlap issue for three mutually-overlapping requirements in a set', () => {
    const { issues } = evaluateRecipeReadiness(
      overlapRecipe([
        ...
        {
          id: 'g3',
          options: [{ quantity: 1, match: { type: 'component', componentId: 'cmp-copper-ore' } }],
        },
      ]),
      { systemComponents }
    );
    assert.equal(
      issues.filter((i) => i.id === 'requirementOverlap').length,
      1,
      'one issue per set'
    );
  });
...
describe('evaluateRecipeReadiness routed check-mode warnings', () => {
  ...
  it('treats a group referencing only a deleted tier id as unrouted', () => {
    ...
    assert.equal(
      issues.some((i) => i.id === 'unproducedOutcomeTier'),
      false
    );
  });
  ...
  it('warns when an authored success tier produces no result group, deep-linking to results', () => {
    ...
    assert.equal(
      issues.some((i) => i.id === 'unroutedResultGroup'),
      false,
      'both groups are routed'
    );
  ...
  it('does not warn for a single-result-group check-routed recipe (no mapping required)', () => {
    ...
    assert.equal(
      issues.some((i) => i.id === 'unroutedResultGroup'),
      false,
      'single group is never unrouted'
    );
    assert.equal(
      issues.some((i) => i.id === 'unproducedOutcomeTier'),
      false,
      'single group needs no tiers produced'
    );
  ...
  it('does not fire routed warnings when the recipe is not check-routed', () => {
    ...
    assert.equal(
      check(checks, 'routedResultGroupsRouted'),
      undefined,
      'no routed checklist entry off check-mode'
    );
