...
9. For UI changes, verify generated screenshots are present for the changed views as visible GitHub attachment image embeds in the PR `Screenshots (if applicable)` section, and evaluated against acceptance criteria rather than merely attached.
...
- Normal UI PR screenshot evidence comes from visible GitHub attachment image embeds in the PR description with `pr-<number>` in alt text, or a specific `SCREENSHOTS_NEEDED:` reason. Uploaded artifacts and `test-results/` paths are automation fallback evidence, not a substitute for rendered screenshots in manual/agent-updated PR descriptions. PR-scoped screenshots should not be committed as repository assets.
