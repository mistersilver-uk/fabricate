...
6. Keep the change docs concrete:
   ...
   - for UI work: screenshot acceptance criteria, representative fixtures, pointer hit-test needs, a UX review gate, expected focused generated screenshot evidence from `npm run screenshots:ui:plan -- --base origin/main` and `npm run screenshots:ui -- --base origin/main --pr <number>`, expected GitHub attachment image embeds in the PR description, and whether fixture images must use copied non-SVG Foundry/dnd5e raster assets from `tests/fixtures/ui-assets/manifest.js`
...
- For UI-changing PRs, plan focused generated screenshot evidence before PR creation or update. Screenshots must be generated under `tmp/pr-screenshots/<number>/`, uploaded through GitHub's native attachment flow, embedded in the PR body as `![pr-<number> ...](https://github.com/user-attachments/assets/...)`, and then cleaned with `npm run screenshots:ui:clean -- --pr <number>`. The PR body must show uploaded GitHub attachment images or include `SCREENSHOTS_NEEDED: <specific reason and visual change summary>` when capture is blocked. Treat artifact references as automation fallback evidence, not the normal visible screenshot handoff. Do not plan a full smoke-harness run merely to create PR screenshots.
