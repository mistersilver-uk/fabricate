...
6. Keep the change docs concrete:
   ...
   - for UI work: screenshot acceptance criteria, representative smoke coverage, pointer hit-test needs, a UX review gate, expected smoke screenshot evidence from `npm run screenshots:ui:plan -- --base origin/main`, `npm run test:foundry`, and `npm run screenshots:ui -- --base origin/main --pr <number>`, expected GitHub attachment image embeds in the PR description, and whether smoke data needs Foundry/dnd5e non-SVG raster imagery
