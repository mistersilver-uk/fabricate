...
- For UI-changing PRs, plan focused generated screenshot evidence before PR creation or update. Screenshots must be generated under `tmp/pr-screenshots/<number>/`, attached or uploaded to the PR, referenced from the PR body, and then cleaned with `npm run screenshots:ui:clean -- --pr <number>`. The PR body must link uploaded/attached screenshot evidence or include `SCREENSHOTS_NEEDED: <specific reason and visual change summary>` when capture is blocked. Do not plan a full smoke-harness run merely to create PR screenshots.
