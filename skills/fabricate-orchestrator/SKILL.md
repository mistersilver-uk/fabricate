...
- For UI-changing PRs, plan focused generated screenshot evidence before PR creation or update. The PR body must embed committed `docs/assets/pr-screenshots/pr-<number>/` images, link uploaded screenshot artifacts, or include `SCREENSHOTS_NEEDED: <specific reason and visual change summary>` when capture is blocked. Do not plan a full smoke-harness run merely to create PR screenshots.
