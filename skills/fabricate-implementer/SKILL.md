...
9. For UI changes, run `npm run screenshots:ui:plan -- --base origin/main` and generate focused representative evidence with `npm run screenshots:ui -- --base origin/main --pr <number>` once a PR number exists. Attach or upload the generated `tmp/pr-screenshots/<number>/` files to the PR, update the PR body with the uploaded evidence links/artifact references, then run `npm run screenshots:ui:clean -- --pr <number>`. Do not run the full Foundry smoke harness just to create PR screenshots. If capture is blocked, add `SCREENSHOTS_NEEDED: <specific reason and visual change summary>` to the PR body.
