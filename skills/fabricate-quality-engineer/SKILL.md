...
- For UI-changing PRs, treat unrelated image markdown as missing evidence. Accepted evidence is uploaded/attached screenshot evidence, uploaded screenshot artifacts, `test-results/` artifact paths, or a specific `SCREENSHOTS_NEEDED:` handoff. PR-scoped screenshots should be generated under `tmp/pr-screenshots/<number>/` and cleaned after upload, not committed as assets.
