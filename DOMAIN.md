...
| **Canvas Interactable**         | A Fabricate Tool station or Gathering-Task resource node placed on the Foundry canvas, **region-first**: a **Scene Region** (the *where*) carrying a custom **`fabricate.interactable` Region Behaviour** (the *what happens* — a `RegionBehaviorType` that OWNS the authoritative state) plus an optional **linked visual** (the *what it looks like*). A GM drags an entry from the GM-only scene-control browser (or a tool-linked Item) to spawn a Region + behaviour + linked Tile, or a **region-only** interactable with no visible marker. **Activation is token presence**: a controlled token entering the region offers the controlling player a non-blocking interact prompt. **No synthetic actor or proxy token is ever created.** A Tool interactable injects a session-scoped `activeCanvasTool` (virtual-present); a Gathering-Task interactable carries **per-behaviour node state**, a resolved `environmentId`, and an optional **depleted behavior**.                                                                                                                                                                                                            | `src/canvas/*` (`InteractableManager`, `interactableResolution`, `environmentResolution`, `interactableSocket`); `src/canvas/regions/*` (`FabricateInteractableRegionBehavior`, `interactableRegionFlags`, `interactableRegionActivation`, `interactableRegionNodeAdapter`, `interactableRegionWorldTime`, `interactableConfigSheet`); `src/canvas/linkedVisuals/linkedInteractableVisual.js`; `behavior.system`; module manifest `documentTypes.RegionBehavior.interactable` + `CONFIG.RegionBehavior.dataModels` | openspec/specs/data-models/spec.md, openspec/specs/gathering-and-harvesting/spec.md |
| **Linked Visual**               | The presentation-only marker a Canvas Interactable points at: a **Tile** (default), a **Drawing** (labelled zone), or an existing **GM-placed Token**. It carries only reverse flags (`isInteractableVisual`, `linkedRegionUuid`, `linkedBehaviorId`) — no interactable state. **Deleting the visual does NOT destroy the interactable** (recovery governed by `linkedVisual.missingPolicy`: `ignore`/`warn`/`recreate`); a missing visual is a clean no-op. A linked **Token** is the GM's own document and is never mutated/deleted by depletion by default.                                                                                                                                                            | `behavior.system.linkedVisual`, `src/canvas/linkedVisuals/linkedInteractableVisual.js`, `buildLinkedVisualFlags`/`readLinkedVisualRef` | openspec/specs/data-models/spec.md, openspec/specs/gathering-and-harvesting/spec.md |
| **Per-Behaviour Node State**    | A gathering-task interactable's own depletion/respawn state at `behavior.system.node`, **independent of** `environment.nodeRuntime[taskId]`. A drop-time snapshot of the task's node CONFIG (config + runtime; `null` means an UNLIMITED node), depleted when `node.current <= 0`, with calendar-aware world-time respawn. Threaded into start-attempt and listing as a `nodeStateOverride` (the behaviour-backed adapter) that takes precedence over the environment node for that task. All writes route through the active GM over the `module.fabricate` socket; a world-time respawn pass iterates placed region behaviours, active-GM only; depletion reflects onto the linked visual.                                                                                                                                                            | `behavior.system.node`, `src/canvas/regions/interactableRegionNodeAdapter.js`, `src/canvas/regions/interactableRegionWorldTime.js`, `src/canvas/interactableSocket.js` | openspec/specs/data-models/spec.md, openspec/specs/gathering-and-harvesting/spec.md |
| **Depleted Behavior**           | The optional linked-visual transition a gathering-task node applies when depleted (`node.current <= 0`), keyed off the linked visual's `documentName`: **Tile** `swapImage` (texture swap, original stashed in `flags.fabricate.nodeOriginal`) or terminal `deleteToken` (delete the tile, no revert); **Drawing** reversible hide + optional `(depleted)` label; **Token** SAFE no-op by default (never mutated/deleted — `deleteToken` ignored for a Token) with a single `tokenHide` opt-in for a reversible hide. Orthogonal to `depletionTiming` (`onStart`/`onSuccess`). `deleteToken` is terminal and mutually exclusive with `swapImage`/`postfixName`.                                                                                                                                                                  | `depletedBehavior`, `src/canvas/depletedBehavior.js`, `src/canvas/linkedVisuals/linkedInteractableVisual.js`, `flags.fabricate.nodeOriginal` | openspec/specs/data-models/spec.md, openspec/specs/gathering-and-harvesting/spec.md |
...
| **Interactable Activation Pipeline** | The shared token-presence path for a Canvas Interactable. A region `tokenEnter` (behaviour event handlers run on EVERY client) leads to an eligibility check on the controlling player's client (precedence DISABLED → LOCKED → CONSUMED → USES_EXHAUSTED → COOLDOWN → NODE_DEPLETED), then a non-blocking prompt; on Interact an activation request routes to the **active GM**, which re-validates (behaviour/region exist; user controls the actor; token inside; source + environment valid; eligibility) and emits a grant back to the requesting user, who opens the Fabricate UI locally. A `controlToken` hook + keybinding re-raise the prompt for a token already inside on scene load. No active GM ⇒ fails cleanly.                                                                                                                              | `behavior.system.state`/`activation`, `src/canvas/regions/interactableRegionActivation.js` (`evaluateActivationEligibility`/`buildActivationRequest`/`validateActivationRequest`/`describeGrant`), `src/canvas/interactableSocket.js`, `InteractableManager` | openspec/specs/data-models/spec.md, openspec/specs/gathering-and-harvesting/spec.md |
...
`- tasks
   ...
   |- node config + depletedBehavior (canvas interactable depletion/respawn visual)
   |- visibility gate
   |- timeRequirement
   |- check
   |- resultGroups
...
|  |- node (gatheringTask; per-behaviour depletion/respawn snapshot)
|  |- state { enabled, consumed, locked, uses, cooldown }
|  `- activation { trigger: regionEnter, audience }
`- Linked Visual (presentation-only; visual.flags.fabricate)
   |- isInteractableVisual / linkedRegionUuid / linkedBehaviorId
   `- nodeOriginal (depleted-behavior revert stash; image/text/hidden)
...
graph TD
    ...
    subgraph "Canvas (Scene Regions)"
        ...
        REG --> BEH
        BEH -.->|presentation-only| LV
        BEH -->|tool -> activeCanvasTool| TOOL
        BEH -->|gatheringTask -> per-behaviour node| GT
    end
