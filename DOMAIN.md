...
- Issue `#286` (unify gathering regions) **SUPERSEDES the prior `#257` region model** in these ways (shipped): (1) `GatheringRegion`
  ...
  (`evaluateEnvironmentMatch` dropped the region axis and `evidence.region`). Note this is an intentional behavior change: a task/hazard
  ...
  is removed** (the `regions` vocabulary dimension, the task/hazard `region` match tag, and the inert `environment.region` single string),
  and all legacy region UI (vocabulary panel, task/hazard region pickers, task/hazard/environments-browser region filters + chips, player
  ...
  strips task/hazard region tags, leaves `enabled` unset (→ false), and fires a one-time GM notice; pre-unification exports upgrade on the
