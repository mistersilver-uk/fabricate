...
| **Routed**                     | Outcome-based single-selection resolution. Exactly one result group is selected per attempt. **Routed is the only non-`simple` runtime/editor result-routing model**: the GM authors `simple \| routed \| progressive \| alchemy` and nothing else. Routing behaviour is fully provider-driven (`ingredientSet \| macroOutcome \| rollTableOutcome`); the former `mapped` algorithm survives as `routed`+`ingredientSet` and the former `tiered` algorithm is reproduced by name-matching `routed`+`macroOutcome` data, so neither legacy mode has any live resolution branch. A reserved failure keyword as a `ResultGroup.name` is rejected under **every** routed provider, not only `rollTableOutcome`, and group names must be unique under trim/case normalization.                                                                                                                                                                                                                                                                                                                                                                                                                                     | `resolutionMode: "routed"`                                                           | openspec/specs/resolution-modes/spec.md                     |
...
| **Per-die Crit**               | A configured forced-outcome row (`diceCrits[]`, resolved by `resolveCheckCrit`) that overrides the normal threshold when a die's POST-MODIFIER total (`group.sum`, so keep/drop/explode modifiers are out of scope) equals its `raw`. A matched forced **failure** takes precedence over a forced success. The forced disposition differs by runner: **simple** and **routed** force success/failure (and may set `breakTools`); **progressive** forces award-all (`MAX_SAFE_INTEGER`) or award-none (`0`); **routed** additionally REROUTES — a forced success routes to the best succeeding tier and a forced failure to the worst failing tier, and when no tier of that disposition exists the tier-derived match is **dropped entirely** (`outcome` null → no outcome). | `resolveCheckCrit`/`routeCritOutcome` in `src/systems/checkRoll.js`; per-die rows normalized by `_normalizeSimpleDiceCrit` | openspec/specs/resolution-modes/spec.md, openspec/specs/gathering-and-harvesting/spec.md |
