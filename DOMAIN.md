...
| **Linked Visual**               | The presentation-only marker a Canvas Interactable points at: a **Tile** (default), a **Drawing** (labelled zone), or an existing **GM-placed Token**. It carries only reverse flags (`isInteractableVisual`, `linkedRegionUuid`, `linkedBehaviorId`) — no interactable state. **Deleting the visual does NOT destroy the interactable** (recovery governed by `linkedVisual.missingPolicy`: `ignore`/`warn`/`recreate`); a missing visual is a clean no-op. In the current model the marker is purely presentation: it is **not** auto-depleted/swapped/hidden per-interactable (env-driven marker depletion is a possible FUTURE option, not current behaviour).                                                                                                                                                            | `behavior.system.linkedVisual`, `src/canvas/linkedVisuals/linkedInteractableVisual.js`, `buildLinkedVisualFlags`/`readLinkedVisualRef` | openspec/specs/data-models/spec.md, openspec/specs/gathering-and-harvesting/spec.md |
| **Gathering-Task Interactable Shortcut** | A gathering-task interactable is a **pure `(environment, task)` shortcut**. Activating it opens the gathering UI scoped to that environment + task (auto-selecting both); node counts, depletion, and respawn are owned entirely by the **environment's `nodeRuntime[taskId]`** — the single source of truth read/decremented by a normal gathering attempt. The behaviour carries **no `node` field** and no per-interactable/per-token node pool. **REMOVED from the model (do not reintroduce):** the per-interactable/per-token node snapshot+adapter, the `nodeStateOverride`/`tileRef`/`resolveTileNodeState` engine seam, the per-behaviour world-time respawn pass, and the per-marker depleted-behaviour auto-swap. (Env-driven marker depletion is a possible FUTURE option, not current behaviour.)                                                                                                                                                            | `behavior.system.{environmentId,taskId}`, `environment.nodeRuntime[taskId]`, `InteractableManager.openGrant`, `describeGrant` in `src/canvas/regions/interactableRegionActivation.js` | openspec/specs/data-models/spec.md, openspec/specs/gathering-and-harvesting/spec.md |
| **Depleted Behavior (task config; not interactable-driven)** | A task-level node-config option authored on the gathering task. It is still normalized and editable as config (`gatheringNodeConfig.js`, `adminStore`, `GatheringTaskEditView`), but in the current model it does **NOT** drive any per-interactable/per-marker visual transition — there is no per-interactable depletion reflection onto the linked visual. Treat env-driven marker depletion as a possible FUTURE feature.                                                                                                                                                                  | `depletedBehavior`, `src/systems/gatheringNodeConfig.js`, `src/ui/svelte/stores/adminStore.js`, `src/ui/svelte/apps/manager/GatheringTaskEditView.svelte` | openspec/specs/data-models/spec.md, openspec/specs/gathering-and-harvesting/spec.md |
...
| **Interactable Activation Pipeline** | The shared token-presence path for a Canvas Interactable. A region `tokenEnter` (behaviour event handlers run on EVERY client) leads to an eligibility check (precedence DISABLED → LOCKED → CONSUMED → USES_EXHAUSTED → COOLDOWN — there is **no NODE_DEPLETED gate**; node depletion is owned by the environment's `nodeRuntime[taskId]` and checked by the gathering engine when the session opens), then a non-blocking prompt shown on the **mover's** client AND on the **owning non-GM player's** client (a GM moving a player's token prompts both; the GM is not spammed for players' autonomous moves). On Interact an activation request routes to the **active GM**, which re-validates (behaviour/region exist; user controls the actor; token inside; source + environment valid; eligibility) and emits a grant to the requesting user, who opens the Fabricate UI locally. A **denied** activation sends a localized reason (`FABRICATE.Canvas.Interactable.Denied.*`) back to the requesting player. A `controlToken` hook + keybinding re-raise the prompt for a token already inside on scene load. No active GM ⇒ fails cleanly.                                                                                                                              | `behavior.system.state`/`activation`, `src/canvas/regions/interactableRegionActivation.js` (`evaluateActivationEligibility`/`buildActivationRequest`/`validateActivationRequest`/`describeGrant`/`activationDenialMessageKey`), `src/canvas/interactableSocket.js`, `InteractableManager` | openspec/specs/data-models/spec.md, openspec/specs/gathering-and-harvesting/spec.md |
...
`- tasks
   ...
   |- node config + depletedBehavior (task-level config; environment nodeRuntime owns depletion/respawn; depletedBehavior does not drive a per-interactable marker)
   |- visibility gate
   |- timeRequirement
   |- check
   |- resultGroups
