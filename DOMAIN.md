...
| **Tool-Breakage Authority** | The per-system switch (`CraftingSystem.toolBreakage.authority`, default `toolSpecific`) that decides **whether** a required tool breaks, normalized on read (unknown/missing → `toolSpecific`; no versioned migration, mirroring the inline `resolutionMode`/`salvageResolutionMode` defaulters). `toolSpecific` (a strict superset of pre-419 behaviour): each Tool's own `breakage.mode` decides, plus the legacy per-die `DiceCrit.breakTools` / routed per-tier `outcome.breakTools` force-break on top. `checkDriven`: the active check's **Check Breakage** triggers decide whether **all required tools** break for the attempt, and each Tool's own mode is **not** evaluated except `immune` (always honoured, filtered out of the force-break set). The governing rule: **authority decides WHETHER; Check Breakage triggers decide WHEN under `checkDriven`; the Tool's `onBreak` decides WHAT happens; an `immune` Tool never breaks under either authority.** The decision is reached through one shared evaluator (`evaluateCheckBreakage`) that crafting, salvage, and gathering all route through, so it cannot drift between surfaces. | `CraftingSystem.toolBreakage.authority`, `evaluateCheckBreakage`/`createToolBreakageRuntime` in `src/toolBreakageRuntime.js`, `CraftingEngine._applyToolBreakage`, `CraftingSystemManager._normalizeSystem` | openspec/specs/data-models/spec.md, openspec/specs/gathering-and-harvesting/spec.md, openspec/specs/ui-integration/spec.md |
| **Check Breakage (`checkBreakage`)** | The per-check, GM-authored **trigger model** that decides **WHEN** required tools break under `checkDriven` authority — `{ enabled, triggers[] }`, where triggers are ORed (any match breaks **all** required tools; v1 never targets a single named tool). A `CheckBreakageTrigger` is `{ id, label, condition }` with `condition` one of: `rollTotal` (raw roll total `data.total`, with `==/<=/>=/</>` operator + `value`); `progressiveValue` (the awarding `value`, meaningful only on progressive checks — absent → never matches; **distinct** from `rollTotal` because a progressive crit can overwrite `value` while `data.total` keeps the raw roll); `outcomeTier` (resolved tier/outcome in `tierIds[]`/`outcomeKeys[]`); and `diceGroup` (`groupId` = index into the evaluated `roll.dice` term order, an aggregate `total`/`anyDie`/`allDice`/`lowestDie`/`highestDie` over active-only raw faces, plus operator + value). It is the **single generalization** of the legacy `breakTools` flags, not a parallel path: a per-die `DiceCrit.breakTools` is equivalent to a `diceGroup`+`total`+`==`+`crit.raw` trigger and a routed `outcome.breakTools` to an `outcomeTier` trigger; the shared evaluator reads `data.breakTools` as an implicit always-on trigger so the two seed presets need no separate persistence. Authored **only under `checkDriven`** (the per-die/per-tier `breakTools` toggles remain the surface under `toolSpecific`). **Distinct from the per-realm gathering `toolBreakagePolicy`** (`failureOnBreak \| successDespiteBreak`), which governs what a *broken* tool does to the gather outcome, not when tools break. | `check.checkBreakage`, `evaluateCheckBreakage`/`createToolBreakageRuntime` in `src/toolBreakageRuntime.js`, `rolledDiceGroups` in `src/systems/checkRoll.js`, `CraftingSystemManager._normalizeCheckBreakage`, `CheckBreakage.svelte` | openspec/specs/data-models/spec.md, openspec/specs/gathering-and-harvesting/spec.md, openspec/specs/ui-integration/spec.md |
...
| **Per-die Crit**               | A configured forced-outcome row (`diceCrits[]`, resolved by `resolveCheckCrit`) that overrides the normal threshold when a die's **group total** (`group.sum`) equals its `raw`. `die` is a canonical plain (unmodified) `NdS` term (bare `dN` ≡ `1dN`) and `raw` is matched against the group total over the plain producible range `[N, N*S]` (for `N>1` a group sum, not a single die face). Only plain die terms are crit-eligible: **modified pools (keep/drop/explode/reroll)** are NOT crit-eligible — the editor offers no crit row, and when a die group leaves the plain-eligible set (a modifier is added, or the die is removed from the formula) its crits are **eagerly purged from the editor draft and staged data** (and dropped by normalization on save as a backstop; `_normalizeDiceCrits` shares the `parsePlainDiceGroups` classifier with the editor so UI eligibility and persistence cannot drift). A matched forced **failure** takes precedence over a forced success. The forced disposition differs by runner: **simple** and **routed** force success/failure (and may set `breakTools`); **progressive** forces award-all (`MAX_SAFE_INTEGER`) or award-none (`0`); **routed** additionally REROUTES — a forced success routes to the best succeeding tier and a forced failure to the worst failing tier, and when no tier of that disposition exists the tier-derived match is **dropped entirely** (`outcome` null → no outcome). **`breakTools` is engine-only (#431):** a crit/tier `breakTools` flag force-breaks tools (`CraftingEngine._applyToolBreakage({forceBreak})`) **only for engine-evaluated checks** (simple/routed/progressive — tagged by `_markEngineEvaluated`). A macro/builtIn check result passes `data` through verbatim, so a macro returning `data:{ breakTools:true }` must NOT force breakage — `breakTools` is an authored-crit/tier concept absent from the macro contract. **As of issue 419** the force-break decision flows through the single shared `evaluateCheckBreakage` seam (which subsumes the old `_checkForcesToolBreak` and preserves the macro guard via `engineEvaluated !== true`): the legacy `breakTools` flag is honoured as an **implicit always-on Check Breakage trigger** under both authorities — a per-die `breakTools` is the `diceGroup`+`total`+`==`+`crit.raw` trigger and a routed `outcome.breakTools` the `outcomeTier` trigger — so it is now the seed-preset face of the **Check Breakage** model rather than a separate mechanism. The crit's force-success/force-fail disposition is unchanged. | `resolveCheckCrit`/`routeCritOutcome` in `src/systems/checkRoll.js`; per-die rows normalized by `_normalizeDiceCrits`/`_normalizeSimpleDiceCrit`; plain-die classifier `parsePlainDiceGroups` in `src/utils/craftingCheckExpression.js`; `evaluateCheckBreakage` in `src/toolBreakageRuntime.js` | openspec/specs/resolution-modes/spec.md, openspec/specs/gathering-and-harvesting/spec.md, openspec/specs/data-models/spec.md |
...
- Result creation, ingredient consumption, and Tool usage/breakage (`limitedUses` / `breakageChance` / `diceExpression` / `immune`; `destroy` / `flagBroken` / `replaceWith`), with breakage governed by **Tool-Breakage Authority** (`toolSpecific` per-tool mode + legacy force-break, or `checkDriven` **Check Breakage** triggers over all required non-`immune` tools) through the shared `evaluateCheckBreakage` seam — crafting, salvage (now at parity), and gathering route the same decision so it cannot drift
- Shared **Check** roll engine (`src/systems/checkRoll.js`: `runFormulaPassFail`/`runFormulaProgressive`/`runFormulaRouted` + per-die crits) driving crafting, salvage, and gathering checks alike; salvage routes a routed tier name through `component.salvage.outcomeRouting`; a per-component `salvage.dcOverride` shifts the simple/routed DC (no DC for progressive)
