...
| **Gathering Region**      | Named geography (e.g. `The Verdant Expanse`) scoped to one crafting system. A Region is geography, NOT an environment container: it owns no tasks/hazards/drops. It carries `enabled`, `secret`, a `biomes` list (terrain/ecology traits drawn from the system biome vocabulary), and `sort`. `secret` regions affect runtime availability but appear to undiscovered players as placeholders. **Phase 1 (SHIPPED):** normalization, validation, CRUD, stale-reference handling, system import/export ride-along, and a minimal quick list in the Travel route (edits name and enabled state only; supports create/delete). `sceneMappings` (Phase 3 Scene-Region automation) and `modifiers` (Phase 4 region modifiers) normalize/validate and round-trip but are **not yet applied at runtime** — they are reserved, not-yet-shipped. The full Region editor (description/img/secret/biomes/ordering authoring) is a planned Phase 2 follow-up. | `GatheringRegion`; `normalizeGatheringRegion`/`validateGatheringRegionList` in `src/systems/gatheringRegions.js`; persisted on `CraftingSystem.gatheringRegions[]`; CRUD via `GatheringRegionStore`; quick list in `GatheringRegionQuickList.svelte` | openspec/specs/data-models/spec.md, openspec/specs/gathering-and-harvesting/spec.md, openspec/specs/ui-integration/spec.md |
...
| **Gathering Region Settings** | Per-crafting-system region behavior settings: `revealMode` (`manual` \| `onPartyTokenEntry` \| `alwaysVisible`, default `manual`) controls player discovery of secret region identities, and `modifierVisibility` (`visible` \| `gmOnly`, default `visible`). Unknown values coerce to defaults on read and are rejected at save/import boundaries. **SHIPPED:** the record, defaults, and read/write normalization; `revealMode` is consumed by disclosure logic. `onPartyTokenEntry` automation (Phase 3) and modifier-visibility application (Phase 4) are not yet shipped. | `GatheringRegionSettings`; `normalizeGatheringRegionSettings`/`validateGatheringRegionSettings` in `src/systems/gatheringRegions.js`; persisted on `CraftingSystem.gatheringRegionSettings` | openspec/specs/data-models/spec.md, openspec/specs/gathering-and-harvesting/spec.md |
...
|- gatheringRegionSettings (revealMode default manual; modifierVisibility default visible)
...
|- location availability (opt-in: includedRegionIds | includedBiomeIds | excludedRegionIds | excludedBiomeIds; exclusions win; absent/empty = ungated legacy)
|- region / biomes (legacy compatibility/display metadata; NOT GatheringRegion ids)
`- tasks
...
- Per-system Gathering Regions (geography) and Gathering Region Settings (`revealMode`, `modifierVisibility`); regions ride along with crafting-system import/export
- Optional, opt-in environment location availability rules (`includedRegionIds`/`includedBiomeIds`/`excludedRegionIds`/`excludedBiomeIds`); legacy `region`/`biomes` stay as compatibility/display metadata
...
- Issue `#257` (location-aware gathering regions): **Phase 1 shipped** — `GatheringRegion`/`GatheringParty`/`GatheringRegionSettings`
  ...
  and the GM `Travel` route (party management, travel-actor assignment, per-system override, stale repair, and a minimal region quick
  list editing name/enabled only). **Not yet shipped (later phases):** Scene Region automation and `travelActor` token sensing (Phase 3,
  `GatheringRegionSceneMapping`/`onPartyTokenEntry`), region modifier *application* (Phase 4, `GatheringRegionModifier` records
  normalize/validate but do not yet adjust runtime), the full Region editor route, environment availability editor controls,
  player travel view, and player-facing/full discovery controls (Phase 2 follow-ups).
