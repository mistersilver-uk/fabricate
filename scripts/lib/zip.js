...
/** Zip the contents of `srcDir` into `outZipPath`. */
export function zipDirectory(srcDir, outZipPath) {
  if (process.platform === 'win32') {
    ...
    // `zip` can exclude *.zip/LOCK by glob, but to also drop sourcemaps we stage a copy without
    // *.map first (mirroring the win32 staging above) and zip that, then clean the staging up.
