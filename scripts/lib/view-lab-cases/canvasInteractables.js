...
export const CASES = Object.freeze([
  ...
  browserCase({
    ...
    // `beyond`: the smoke never opens the Interactable browser, so there is no counterpart to fall short of.
    reaches: 'beyond',
    smokeLabels: [],
    steps: [],
    // The populated list, not merely the window: an empty `fab-ib-list` renders a different screen in the same chrome.
  ...
  browserCase({
    ...
    // Scoped inside the panel: `.fab-ib-list .fab-ib-row` alone is satisfied by the tab this case navigated away from.
  ...
  browserCase({
    ...
    steps: [
      ...
      // `Forge` matches exactly one smithing Tool, so the filtered list is one row rather than a shorter one.
  ...
  configCase({
    ...
    // What Foundry's own Region > Behaviors > Add Behavior path produces: an empty system, born valid and inert.
    query: { interactable: 'unconfigured' },
    steps: [],
    // The banner and the identity body it force-opens: the banner alone passes on a panel whose picker never rendered.
    expectSelector:
  ...
  configCase({
    ...
    // The one frame that proves the portal resolves (issue 1520), and the `>` is the whole assertion.
    expectSelector:
      '.fabricate-interactable-config-app > .fabricate-select-popover [data-popover-option]',
    kinds: ['canvas', 'interactables'],
    // The positioning seam belongs here; its absence was a routing gap rather than a judgement (issue 1520).
    sourceMatches: [
      /^src\/ui\/svelte\/apps\/InteractableConfigRoot\.svelte$/,
      /^src\/ui\/InteractableConfigApp\.svelte\.js$/,
  ...
  interactablesManagerCase({
    ...
    // `window`: a third interactable resolves and locks and a fourth does not, so every badge the row draws is photographed.
    reaches: 'window',
    smokeLabels: ['interactables-manager-list'],
    steps: [],
    // A row with its actions, so a list that loses its per-row controls fails here rather than publishing as healthy.
    expectSelector:
  ...
  interactablesManagerCase({
    ...
    steps: [
      ...
      // The one selection the panel cannot make for itself: system and source auto-pick, and the region does not.
      ...chooseSelectOption(
    ...
    // `:not([disabled])` asserts `canPromote` — region and system and source — so a broken auto-pick fails here.
    expectSelector:
  ...
  interactablesManagerCase({
    ...
    // `beyond`: the smoke opens the promote card but never rests on a panel, so there is no counterpart.
    reaches: 'beyond',
    smokeLabels: [],
    steps: [
      { selector: '[data-interactable-manager-promote-toggle]' },
