 ...
 * in the main suite.
 *
 * A CHANGE TO THIS FILE SELECTS SURFACE COVERAGE — one frame of every screen the lab renders, not
 * one region of it — and that is a decision rather than an oversight (issue 1049). Two of the lab's
 * other inputs are narrowed from their own diff — the case registry per case literal,
 * `labActors.js` per fixture table — and the obvious next move is to narrow this one too, on the
