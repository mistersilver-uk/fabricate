...
export async function describeBlockingOverlay(page) {
  try {
    return await page.evaluate(() => {
      ...
      if (activeTour)
        return `a Foundry NUE tour ("${activeTour.title ?? activeTour.id ?? 'unknown'}")`;
      const dialog = document.querySelector('dialog[open], .application.dialog, #client-settings');
      if (dialog) {
        const label =
          dialog.getAttribute('aria-label') || dialog.querySelector('.window-title')?.textContent;
  ...
  await context.addInitScript(
    ({ key, tourIds, stepIndex }) => {
      try {
        ...
        globalThis.localStorage.setItem(key, JSON.stringify({ ...base, core }));
      } catch {
...
export function withDeadline(promise, ms, label) {
  ...
  const deadline = new Promise((_, reject) => {
    timer = setTimeout(
      () => reject(new Error(`Operation '${label}' exceeded ${ms}ms deadline`)),
      ms
    );
  });
 ...
 */
export async function waitForManagerGeometrySettled(
  page,
  { timeout = 1500, fallbackMs = 500 } = {}
) {
  await page
    .evaluate(() => {
      delete globalThis.__fabGeomSettle;
    })
    .catch(() => {});
  await waitForSettled(
    ...
    () => {
      ...
      const app =
        manager?.closest('.application, .app') ||
        document.querySelector('#fabricate-crafting-system-manager');
      if (!app) return false;
      const outer = app.getBoundingClientRect();
      const product = manager.getBoundingClientRect();
      const state = globalThis.__fabGeomSettle || { sig: null, stable: 0 };
      const sig = [
        window.innerWidth,
        window.innerHeight,
      ...
      if (state.sig === sig) state.stable += 1;
      else {
        state.sig = sig;
        state.stable = 0;
      }
      ...
      globalThis.__fabGeomSettle = state;
...
export async function settleManagerNav(page, { timeout = 2000, fallbackMs = 750 } = {}) {
  await page
    .evaluate(() => {
      delete globalThis.__fabNavSettle;
    })
    .catch(() => {});
  await waitForSettled(
    page,
    () => {
      ...
      const state = globalThis.__fabNavSettle || { sig: null, stable: 0 };
      if (state.sig === sig) state.stable += 1;
      else {
        state.sig = sig;
        state.stable = 0;
      ...
      // eslint-disable-next-line unicorn/no-global-object-property-assignment -- a page handle the walk re-assigns and deletes; defineProperty would freeze it.
      globalThis.__fabNavSettle = state;
...
export async function waitForManagerApplicationRendered(page) {
  ...
  const resolveRegisteredManager = (selector) => {
    const renderedOuters = [...document.querySelectorAll(selector)].filter(
    ...
    const registeredApps = instances?.values
      ...
      : instances
        ? [...instances]
        : [];
    const uniqueApps = [...new Set([explicitApp, ...registeredApps].filter(Boolean))];
    const applicationCandidates = uniqueApps.map((app, index) => {
      ...
      const ownsManager = Boolean(
        manager?.isConnected &&
        (element === manager || element?.contains?.(manager) || manager?.contains?.(element))
      );
...
export async function setManagerWindowSize(page, { width, height, sourceViewport = null }) {
  ...
  await withDeadline(page.setViewportSize(viewport), 15_000, `setViewportSize ${width}x${height}`);
  await waitForManagerApplicationRendered(page);
  await withDeadline(
    page.evaluate(
      async ({ width, height, viewport }) => {
        const app = globalThis.__fabricateSmokeManagerApp;
  ...
  if (geometry.browser.width !== viewport.width || geometry.browser.height !== viewport.height) {
    throw new Error(
      `Manager source viewport drifted: ${JSON.stringify({ expected: viewport, actual: geometry.browser })}`
    );
  }
  if (
    ...
    throw new Error(
      `ApplicationV2 outer rectangle escapes the browser viewport: ${JSON.stringify(geometry)}`
    );
  }
  if (
    ...
    throw new Error(
      `Tool Studio product rectangle escapes the ApplicationV2 frame: ${JSON.stringify(geometry)}`
    );
  }
  return geometry;
}
...
export async function exerciseManagerPointerTargets(page, systemId) {
  ...
  await softClick(
    page.locator('.fabricate-manager .manager-breadcrumbs button:has-text("The Herbalist")'),
    { trial: true }
  );
  await softClick(
    page.locator('.fabricate-manager .manager-breadcrumbs button:has-text("Crafting Systems")'),
    { trial: true }
  );
...
export async function exerciseManagerSystemEditPointerTargets(page, systemId) {
  if ((await page.locator('.fabricate-manager #manager-system-name').count()) === 0) {
    ...
    if ((await editButton.count()) === 0) {
      ...
      await page
        .locator(managerSystemRowSelector(systemId))
        .first()
  ...
  await softClick(page.locator('.fabricate-manager [data-feature-key="gathering"] input'), {
    trial: true,
  });
  await softClick(
    page.locator(
...
export async function exerciseManagerEnvironmentPointerTargets(page) {
  ...
  await page
    .locator('.fabricate-manager .manager-environment-row')
    .first()
    .waitFor({ state: 'visible', timeout: 5000 });
 ...
 */
export async function waitForFabricateWindowsClosed(
  page,
  { timeout = 800, fallbackMs = 500 } = {}
) {
  await waitForSettled(
    page,
    () => {
      ...
      const hasDiscard = [...document.querySelectorAll('button')].some((button) =>
        /Discard Changes/.test(button.textContent || '')
      );
...
export async function closeOpenApplications(page) {
  ...
  async function discardDirtyDraft() {
    ...
    if ((await discardButton.count()) > 0) {
      try {
        await discardButton.click({ timeout: 2000, force: true });
        await page.waitForTimeout(500);
      } catch {
        /* ignore */
      }
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    await page.evaluate(async (selector) => {
      ...
      if (ui.windows) {
        for (const app of Object.values(ui.windows)) {
          try {
            closePromises.push(Promise.resolve(app.close()));
          } catch {
            /* ignore */
          }
        }
      ...
      for (const app of applicationV2s) {
        ...
        if (
          element.querySelector?.('.fabricate-manager, .fabricate-app') ||
          element.id?.startsWith?.('fabricate-')
        ) {
          try {
    ...
    for (let i = 0; i < (await closeButtons.count()); i++) {
      try {
        await closeButtons.nth(i).click({ timeout: 1000, force: true });
      } catch {
        /* ignore */
      }
    }
  ...
  if ((await remaining.count()) > 0) {
    const ids = await remaining.evaluateAll((elements) =>
      elements.map((element) => element.id || element.closest?.('[id]')?.id || element.tagName)
    );
...
export async function activateSceneAndAwaitCanvasReady(page, sceneId, { timeout = 90_000 } = {}) {
  ...
  await page
    ...
    .catch(() => {});
  await page.waitForFunction(isCanvasReadyForScene, sceneId, { timeout }).catch((error) => {
    ...
    process.stderr.write(
      `Canvas never reported ready for scene ${sceneId} (waited up to ${timeout}ms): ` +
        `${error?.message?.split('\n', 1)[0] ?? error}. Continuing against a possibly undrawn canvas.\n`
    );
  });
}
...
export async function assertNoScreenshotOverlays(page, options = {}) {
    ...
    const diag = await page
      .evaluate((selector) => {
        return [...document.querySelectorAll(selector)]
          .map(
            (el) =>
              `${el.tagName}#${el.id}.${el.className} :: ${(el.textContent || '').trim().slice(0, 120)}`
          )
          .join(' || ');
      }, OVERLAY_SELECTOR)
      .catch(() => '');
    throw new Error(
      `Screenshot target still has ${visibleCount} visible modal or notification overlay(s). [${diag}]`
    );
  }
  ...
  if (count > 0) {
    const diag = await page
      ...
      .catch(() => '');
    throw new Error(
      `Screenshot target is covered by ${count} modal or notification overlay(s). [${diag}]`
    );
  }
  ...
  if (Array.isArray(allowedIds)) {
    ...
    const visibleFabricateWindows = await page.evaluate(() => {
      return [...document.querySelectorAll('[id^="fabricate-"]')]
        .filter((el) => {
          if (!el.classList.contains('application') && !el.classList.contains('window-app'))
            return false;
          const style = globalThis.getComputedStyle(el);
          if (
            style.display === 'none' ||
    ...
    if (stray.length > 0) {
      throw new Error(
        `Screenshot target has stray Fabricate window(s) bleeding through: ${stray.join(', ')}.`
      );
    }
  }
}
...
export async function deleteSmokeWorldDocuments(page, cleanupData) {
  await page.evaluate(async (cleanupData) => {
    ...
    if (cleanupData.systemId) {
      ...
      try {
        await environmentStore?.cleanupByCraftingSystem?.(cleanupData.systemId);
      } catch {
        /* ok */
      }
      ...
      if (csm) {
        try {
          await csm.deleteSystem(cleanupData.systemId);
        } catch {
          /* already deleted */
        }
      }
    ...
    // they own).
    if (
      Array.isArray(cleanupData.executionSystemIds) &&
      cleanupData.executionSystemIds.length > 0
    ) {
      const environmentStore = game.fabricate?.getGatheringEnvironmentStore?.();
      const csm = game.fabricate?.getCraftingSystemManager?.();
      for (const executionSystemId of cleanupData.executionSystemIds) {
        try {
          await environmentStore?.cleanupByCraftingSystem?.(executionSystemId);
        } catch {
          /* ok */
        }
        try {
          await csm?.deleteSystem(executionSystemId);
        } catch {
          /* already deleted */
        }
      }
    ...
    if (Array.isArray(cleanupData.executionItemIds) && cleanupData.executionItemIds.length > 0) {
      try {
        await Item.deleteDocuments(cleanupData.executionItemIds);
      } catch {
        /* ok */
      }
    }
    ...
    if (singlePurposeSystemIds.length > 0) {
      ...
      for (const singlePurposeSystemId of singlePurposeSystemIds) {
        try {
          await environmentStore?.cleanupByCraftingSystem?.(singlePurposeSystemId);
        } catch {
          /* ok */
        }
        try {
          await csm?.deleteSystem(singlePurposeSystemId);
        } catch {
          /* already deleted */
        }
      }
    ...
    if (cleanupData.actorIds.length > 0) {
      try {
        await Actor.deleteDocuments(cleanupData.actorIds);
      } catch {
        /* ok */
      }
    }
    ...
    if (cleanupData.userIds.length > 0) {
      try {
        await User.deleteDocuments(cleanupData.userIds);
      } catch {
        /* ok */
      }
    }
    ...
    if (cleanupData.itemIds.length > 0) {
      try {
        await Item.deleteDocuments(cleanupData.itemIds);
      } catch {
        /* ok */
      }
    }
    ...
    if (cleanupData.sceneIds.length > 0) {
      try {
        await Scene.deleteDocuments(cleanupData.sceneIds);
      } catch {
        /* ok */
      }
    }
