...
export default {
  ...
  async run(ctx) {
    ...
    if (recipeApiCount < 2) {
      throw new Error(
        `Expected the smoke system to expose at least 2 recipes via the API; saw ${recipeApiCount}.`
      );
    }
    await openManagerCraftingSection(page, 'recipes', 'recipes');
    await page
      .locator('.fabricate-manager .manager-recipe-row:has-text("Brew Healing Potion")')
      .first()
    ...
    await captureBulkEditFrame(ctx, {
      ...
      stage: async (bulkPanel) => {
        ...
        await chooseSelectOption(page, bulkPanel.locator('[data-recipe-bulk-category]').first(), {
          index: 1,
        });
        ...
        if (await bulkPanel.locator('[data-recipe-bulk-apply]').first().isDisabled()) {
          throw new Error(
            'Recipe bulk edit Apply stayed inert after a category and Lock were staged.'
          );
        }
      },
    });
    await captureBulkEditFrame(ctx, {
      ...
      stage: async (bulkPanel) => {
        ...
        if (!(await bulkPanel.locator('[data-recipe-bulk-apply]').first().isDisabled())) {
          throw new Error(
            'Recipe bulk edit Apply was live on a pristine draft with nothing staged.'
          );
        }
        ...
        for (const axis of ['status', 'lock']) {
          await bulkPanel
            .locator(`[data-recipe-bulk-${axis}-option="unchanged"] input:checked`)
            .first()
    ...
    await captureBulkEditFrame(ctx, {
      ...
      stage: async (bulkPanel) => {
        ...
        await bulkPanel
          .locator('[data-recipe-bulk-blocked-warning]')
          .first()
          .waitFor({ state: 'visible', timeout: 5000 });
        await page
          .locator(
            '.fabricate-manager .manager-recipe-row:has-text("Temper a Blade") .manager-chip.is-danger'
          )
          .first()
          .waitFor({ state: 'visible', timeout: 5000 });
      },
    ...
    try {
      ...
      results.steps.push({ step: 'crafting-group-surfaces', passed: false, error: error.message });
      process.stderr.write(`Crafting group surface capture failed: ${error.message}\n`);
    }
    ...
    try {
      const openRecipeItemValidation = async (recipeItemId) => {
        await openManagerCraftingSection(page, 'books-scrolls', 'books-scrolls');
        await page
          .locator(`.fabricate-manager [data-books-scrolls-edit="${recipeItemId}"]`)
          .first()
          .click();
        await page
          .locator('.fabricate-manager[data-manager-view="recipe-item-edit"]')
          .first()
          .waitFor({ state: 'visible', timeout: 5000 });
        await page
          .locator('.fabricate-manager [data-recipe-item-tab-button="validation"]')
          .first()
          .click();
        ...
        await page
          .locator(
            '.fabricate-manager [data-recipe-item-tab="validation"] [data-recipe-item-validation-summary]'
          )
          .first()
          .waitFor({ state: 'visible', timeout: 5000 });
        await assertNoScreenshotOverlays(page);
      ...
      results.steps.push({ step: 'recipe-item-validation', passed: false, error: error.message });
      process.stderr.write(`Recipe-item validation capture failed: ${error.message}\n`);
    }
    ...
    await page
      .locator('.fabricate-manager [data-recipe-section="identity"]')
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });
    ...
    try {
      await page
        .locator('.fabricate-manager [data-recipe-tab-button="books-scrolls"]')
        .first()
        .click();
      await page
        .locator(
          '.fabricate-manager [data-recipe-tab="books-scrolls"] [data-recipe-section="recipe-item"]'
        )
        .first()
        .waitFor({ state: 'visible', timeout: 5000 });
      ...
      results.steps.push({
        step: 'recipe-edit-books-scrolls',
        passed: false,
        error: error.message,
      });
      process.stderr.write(`Recipe books & scrolls capture failed: ${error.message}\n`);
    }
    ...
    try {
      ...
      await page
        .locator('.fabricate-manager [data-recipe-tab="tools"]')
        .first()
        .waitFor({ state: 'visible', timeout: 5000 });
      await page
        .locator('.fabricate-manager [data-recipe-tab="tools"] [data-recipe-tool-id]')
        .first()
        .waitFor({ state: 'visible', timeout: 5000 });
      await assertNoScreenshotOverlays(page);
      ...
      results.steps.push({ step: 'recipe-edit-tools', passed: false, error: error.message });
      process.stderr.write(`Recipe tools capture failed: ${error.message}\n`);
    }
    ...
    await page
      .locator('.fabricate-manager [data-recipe-tab="ingredients"]')
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });
    await page
      .locator('.fabricate-manager [data-recipe-tab="ingredients"] [data-recipe-group]')
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });
    ...
    await ingredientsTab
      .locator('[data-recipe-option-essence]')
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });
    await ingredientsTab
      .locator('[data-recipe-currency-amount]')
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });
    await assertNoScreenshotOverlays(page);
    ...
    await page
      .locator('.fabricate-manager [data-recipe-tab="validation"]')
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });
    // Wait on both warning chips so the capture proves the new readiness signals.
    await page
      .locator('.fabricate-manager [data-issue="unroutedResultGroup"]')
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });
    await page
      .locator('.fabricate-manager [data-issue="unproducedOutcomeTier"]')
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });
    ...
    await page
      .locator('.fabricate-manager [data-recipe-tab="overview"]')
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });
    await page
      .locator('.fabricate-manager [data-recipe-section="steps"]')
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });
    ...
    await page
      .locator('.fabricate-manager [data-recipe-section="steps"] [data-recipe-duration-trigger]')
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });
    ...
    try {
      ...
      // onto the yes action).
      await disableDialog
        .getByText('Their steps are kept and restored')
        .first()
        .waitFor({ state: 'visible', timeout: 5000 });
      await disableDialog
        .locator('button[data-action="yes"]:has-text("Disable")')
        .first()
        .waitFor({ state: 'visible', timeout: 5000 });
      ...
      results.steps.push({
        step: 'multistep-disable-confirm',
        passed: false,
        error: error.message,
      });
      process.stderr.write(`Multi-step disable-confirm capture failed: ${error.message}\n`);
      // Dismiss any lingering confirm dialog so the next capture starts clean.
      await page
        .locator('.application.dialog button[data-action="no"]')
        .first()
        .click()
        .catch(() => {});
    }
    ...
    try {
      ...
      // accordion.
      await page
        .locator('.fabricate-manager [data-recipe-section="collapsed-steps"]')
        .first()
        .waitFor({ state: 'visible', timeout: 5000 });
      await page
        .locator('.fabricate-manager [data-recipe-collapsed-note]')
        .first()
        .waitFor({ state: 'visible', timeout: 5000 });
      ...
      results.steps.push({
        step: 'multistep-recipe-edit-collapsed',
        passed: false,
        error: error.message,
      });
      process.stderr.write(`Multi-step collapsed-editor capture failed: ${error.message}\n`);
    } finally {
      ...
      try {
        ...
        process.stderr.write(
          `Multi-step re-enable via UI failed, forcing via API: ${error.message}\n`
        );
        await page
          .evaluate(async (sysId) => {
    ...
    if (executionFixtures?.progressive?.systemId) {
      try {
        ...
        results.steps.push({
          step: 'recipe-edit-results-progressive',
          passed: false,
          error: error.message,
        });
        process.stderr.write(`Progressive results capture failed: ${error.message}\n`);
      } finally {
    ...
    if (alchemyFixtures?.cauldronSystemId) {
      try {
        ...
        results.steps.push({
          step: 'recipe-edit-results-alchemy',
          passed: false,
          error: error.message,
        });
        process.stderr.write(`Alchemy results capture failed: ${error.message}\n`);
      } finally {
        await softClick(page.locator('.fabricate-manager .manager-scope-return'));
        await selectSmokeSystemInManager(page, craftingSetup.systemId).catch(() => {});
      }
    ...
    try {
      ...
      await page
        .locator('.fabricate-manager [data-recipe-tab-button="access"]')
        .first()
        .click({ timeout: 5000 });
      await page
        .locator('.fabricate-manager [data-recipe-tab="access"] [data-recipe-section="access"]')
        .first()
        .waitFor({ state: 'visible', timeout: 5000 });
      await page
        .locator('.fabricate-manager [data-recipe-access-characters]')
        .first()
        .waitFor({ state: 'visible', timeout: 5000 });
      await captureStableManagerView(ctx, {
      ...
      results.steps.push({ step: 'recipe-edit-access-rail', passed: false, error: error.message });
      process.stderr.write(`Restricted recipe rail capture failed: ${error.message}\n`);
    } finally {
