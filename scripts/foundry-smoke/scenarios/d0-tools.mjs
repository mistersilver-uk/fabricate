  ...
  return page.evaluate(
    async ({ systemId, recipeId }) => {
      ...
      if (!system || !recipe || !sourceTemplate) {
        throw new Error(
          'Tool Studio fixture requires Arcane Forge, Brew Healing Potion, and Herbalist Sickle'
        );
      }
      const pack = game.packs.get('dnd5e.items');
      const index = await pack?.getIndex?.();
      const uncached = [...(index || [])].find((entry) => entry?._id && !pack.get(entry._id));
      ...
      const parityDescription =
        '<p>A well-balanced forge hammer. Durable, but the haft splinters when hard used.</p>';
      ...
      if (components.length < 2)
        throw new Error('Tool Studio repair fixture needs two managed Components');
      ...
      let { item: upsertedTool } = await csm.upsertTool(
        systemId,
        {
          id: requestedToolId,
          enabled: true,
      ...
      ({ item: upsertedTool } = await csm.upsertTool(systemId, {
        ...upsertedTool,
        description:
          'A well-balanced forge hammer. Durable, but the haft splinters when hard used.',
      }));
  ...
  await page.evaluate(
    async ({ systemId, recipeId, fixture }) => {
      ...
      if (source) {
        ...
        if (fixture.sourceRoles)
          await source.setFlag('fabricate', 'fabricate.roles', fixture.sourceRoles);
...
async function verifyToolStudioLiveReplacement(page, { systemId, recipeId, actorId, fixture }) {
  return page.evaluate(
    async ({ systemId, recipeId, actorId, fixture }) => {
      const csm = game.fabricate.getCraftingSystemManager();
      const rm = game.fabricate.getRecipeManager();
      ...
      if (!actor || !source || !tool)
        throw new Error('Tool Studio replacement fixture did not resolve');
      if (game.packs.get('dnd5e.items')?.get(fixture.uncachedReplacementUuid.split('.').at(-1))) {
        throw new Error(
          'Tool Studio direct replacement source was cached before awaited resolution'
        );
      }
      await csm.upsertTool(systemId, {
        ...tool,
        ...
        onBreak: {
          mode: 'replaceWith',
          replacementTarget: { type: 'item', itemUuid: fixture.uncachedReplacementUuid },
        },
      });
      ...
      try {
        evidence = await game.fabricate.getCraftingEngine()._applyToolBreakage(
          ...
          [
            {
              tool: csm.getSystem(systemId).tools.find((entry) => entry.id === fixture.toolId),
              item: owned,
              breakable: true,
            },
          ],
      ...
      const componentIdentity = foundry.utils.getProperty(
        createdData,
      ...
      if (order.join(',') !== 'create,delete')
        throw new Error(`Tool replacement order was ${order.join(',')}`);
      if (createdData?.system?.quantity !== 1)
        throw new Error('Tool replacement did not create quantity one');
      if (sourceId !== fixture.uncachedReplacementUuid)
        throw new Error('Tool replacement did not preserve direct Item source identity');
      if (componentIdentity)
        throw new Error('Direct Item replacement fabricated Component identity');
      if (actor.items.get(owned.id))
        throw new Error('Original Tool remained after successful replacement');
