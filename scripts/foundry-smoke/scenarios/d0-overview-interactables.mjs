...
export default {
  ...
  async run(ctx) {
    ...
    try {
      ...
      await page
        .locator(
          `${managerSystemRowSelector(craftingSetup.blockedSystemId)} .manager-system-identity`
        )
        .first()
      ...
      await page
        .locator('.fabricate-manager .manager-system-tab-panel [data-system-overview]')
        .first()
        .waitFor({ state: 'visible', timeout: 5000 });
      await page
        .locator('.fabricate-manager [data-system-overview] [data-overview-issue]')
        .first()
        .waitFor({ state: 'visible', timeout: 5000 });
      if ((await page.locator('.fabricate-manager [data-system-overview-blocker]').count()) === 0) {
        throw new Error(
          'System overview validation tab did not render the system-blocker callout for the broken system.'
        );
      }
      ...
      const taskRow = page
        .locator('.fabricate-manager [data-system-overview] [data-overview-kind="task"]')
        .first();
      ...
      await page
        ...
        .waitFor({ state: 'visible', timeout: 5000 });
      await page
        .locator('.fabricate-manager [data-system-edit-blocker]')
        .first()
        .waitFor({ state: 'visible', timeout: 5000 });
      ...
      try {
        ...
        await page
          ...
          .waitFor({ state: 'visible', timeout: 5000 });
        await page
          .locator('.fabricate-manager [data-component-difficulty]')
          .first()
          .waitFor({ state: 'visible', timeout: 5000 });
        await assertNoScreenshotOverlays(page);
        ...
        await captureBulkEditFrame(ctx, {
          ...
          stage: async (bulkPanel) => {
            await bulkPanel
              .locator('[data-component-bulk-tags-empty]')
              .first()
              .waitFor({ state: 'visible', timeout: 5000 });
            ...
            await bulkPanel
              .locator('[data-component-bulk-difficulty-staged="true"]')
              .first()
              .waitFor({ state: 'visible', timeout: 5000 });
        ...
        if (blockedNames[1]) {
          ...
          await page
            ...
            .first()
            .waitFor({ state: 'visible', timeout: 5000 });
          const difficultyInput = page
            .locator('.fabricate-manager [data-component-edit-section="difficulty"] input')
            .first();
          await difficultyInput.waitFor({ state: 'visible', timeout: 5000 });
          await difficultyInput.fill('7');
          await page
            .locator('.fabricate-manager .manager-header-actions .manager-chip:has-text("Unsaved")')
            .first()
            .waitFor({ state: 'visible', timeout: 5000 });
          await assertNoScreenshotOverlays(page);
          await screenshot(page, 'manager-component-edit-difficulty');
          await page
            .locator('.fabricate-manager button[form="manager-component-edit-form"]')
            .first()
            .click();
          await page
            .locator('.fabricate-manager[data-manager-view="components"]')
        ...
        results.steps.push({
          step: 'progressive-difficulty-captures',
          passed: false,
          error: error.message,
        });
        process.stderr.write(`Progressive difficulty capture failed: ${error.message}\n`);
      }

      results.steps.push({ step: 'system-overview-and-banner', passed: true });
    } catch (error) {
      results.steps.push({
        step: 'system-overview-and-banner',
        passed: false,
        error: error.message,
      });
      process.stderr.write(`System overview capture failed: ${error.message}\n`);
    } finally {
    ...
    try {
      ...
      if (!interactableRef?.sceneId || !interactableRef?.regionId || !interactableRef?.behaviorId) {
        throw new Error(
          `Interactable behaviour ref is incomplete: ${JSON.stringify(interactableRef)}`
        );
      }
      await page.evaluate(
        ({ s, r, b }) => {
          return game.fabricate.api
            .getInteractableConfigAppClass()
            .show({ sceneId: s, regionId: r, behaviorId: b });
        },
      ...
      if ((await linkToggle.getAttribute('aria-pressed')) !== 'true') {
        throw new Error(
          'Interactable config opened unlinked; expected the linked default (aria-pressed="true").'
        );
      }
      await assertNoScreenshotOverlays(page);
      ...
      await page
        .locator('[data-interactable-node-count]')
        .first()
        .waitFor({ state: 'visible', timeout: 10_000 });
      await page
        .locator('[data-interactable-node-respawn]')
        .first()
        .waitFor({ state: 'visible', timeout: 10_000 });
      if ((await linkToggle.getAttribute('aria-pressed')) !== 'false') {
        throw new Error(
          'Interactable config did not unlink after toggle (expected aria-pressed="false").'
        );
      }
      await assertNoScreenshotOverlays(page);
      ...
      await page
        .evaluate(async () => {
          const app = Object.values(ui.windows).find(
            (w) => w?.options?.id === 'fabricate-interactable-config'
          );
          if (app?.close) await app.close();
        })
        .catch(() => {
          /* best-effort; closeOpenApplications also sweeps it */
        });

      results.steps.push({ step: 'interactable-config', passed: true });
    } catch (error) {
      results.steps.push({ step: 'interactable-config', passed: false, error: error.message });
    ...
    const closeAllConfigWindows = async () => {
      ...
      await page
        .locator('.fabricate-interactable-config')
        .first()
    ...
    const openConfig = async (ref) => {
      ...
      await page
        ...
        .waitFor({ state: 'visible', timeout: 10_000 });
      await page
        .locator('[data-interactable-identity-section]')
        .first()
        .waitFor({ state: 'visible', timeout: 10_000 });
    };
    ...
    try {
      ...
      await page
        .locator('[data-interactable-identity-body]')
        .first()
        .waitFor({ state: 'visible', timeout: 10_000 });
      await assertNoScreenshotOverlays(page);
      ...
      if (!unconfiguredRef?.sceneId || !unconfiguredRef?.regionId || !unconfiguredRef?.behaviorId) {
        throw new Error(
          `Unconfigured interactable ref is incomplete: ${JSON.stringify(unconfiguredRef)}`
        );
      }
      await openConfig(unconfiguredRef);
      await page
        .locator('[data-interactable-needs-config]')
        .first()
        .waitFor({ state: 'visible', timeout: 10_000 });
      await page
        .locator('[data-interactable-identity-type]')
        .first()
        .waitFor({ state: 'visible', timeout: 10_000 });
      await assertNoScreenshotOverlays(page);
      ...
      results.steps.push({
        step: 'interactable-config-source',
        passed: false,
        error: error.message,
      });
    ...
    try {
      ...
      await page
        .locator('#fabricate-crafting-system-manager')
        .waitFor({ state: 'detached', timeout: 10_000 });
      ...
      await page
        .evaluate(
          async ({ sceneId, systemId, toolId, taskId }) => {
                ...
                behaviors: [
                  {
                    ...
                    system: {
                      ...
                      linkedVisual: {
                        mode: 'marker',
                        uuid: `Scene.${sceneId}.Tile.fabricateMissingTile`,
                        documentName: 'Tile',
                        missingPolicy: 'warn',
                      },
                    },
      ...
      await page
        .locator('.fabricate-interactables-manager .fab-im-row')
        .first()
        .waitFor({ state: 'visible', timeout: 10_000 });
      const rowCount = await page.locator('.fabricate-interactables-manager .fab-im-row').count();
      if (rowCount < 3) {
        throw new Error(
          `Manage list shows only ${rowCount} row(s); expected at least 3 marker-status variants.`
        );
      }
      ...
      if (
        (await page
          .locator(
            '.fabricate-interactables-manager [data-interactable-manager-chip-marker="missing"]'
          )
          .count()) === 0
      ) {
      ...
      await assertNoScreenshotOverlays(page, {
        allowFabricateWindowIds: ['fabricate-interactables-manager'],
      });
      ...
      const promotePanel = page
        .locator('.fabricate-interactables-manager [data-interactable-manager-promote]')
        .first();
      ...
      await page
        .locator('.fabricate-interactables-manager [data-interactable-manager-system]')
        .first()
        .click();
      await page
        .locator(`.fabricate-select-popover [data-popover-option="${craftingSetup.systemId}"]`)
        .first()
      ...
      await page
        .locator(
          '.fabricate-interactables-manager [data-interactable-manager-source-type-option="tool"]'
        )
        .first()
        .click();
      ...
      await page
        .locator('.fabricate-interactables-manager [data-interactable-manager-source]')
        .first()
        .click();
      ...
      const seededToolRow = page
        .locator('.fabricate-select-popover [data-popover-option]')
        .filter({ hasText: /Herbalist Sickle/i });
      if (sourceOptionLabels.length === 0 || (await seededToolRow.count()) === 0) {
        const diag = await page.evaluate(() => {
          const out = { panelRows: [], triggers: [], liveSystems: [] };
          try {
            out.liveSystems = game.fabricate
              .getCraftingSystemManager()
              .getSystems()
              .map((s) => ({ id: s.id, name: s.name, toolCount: (s.tools || []).length }));
          } catch (error) {
            out.liveSystemsErr = error.message;
          }
          const optionRows = document.querySelectorAll(
            '.fabricate-select-popover [data-popover-option]'
          );
          for (const [i, row] of optionRows.entries()) {
            out.panelRows.push({
              i,
              value: row.dataset.popoverOption,
              text: row.textContent.replaceAll(/\s+/g, ' ').trim(),
            });
          }
          const triggers = document.querySelectorAll(
            '.fabricate-interactables-manager .fabricate-select-trigger'
          );
          for (const [i, btn] of triggers.entries()) {
            out.triggers.push({ i, text: btn.textContent.replaceAll(/\s+/g, ' ').trim() });
          }
          return out;
        });
        process.stderr.write('PROMOTE DIAG: ' + JSON.stringify(diag) + '\n');
        throw new Error(
          'Promote Source picker does not list the seeded Tool — the No-sources regression is not fixed.'
        );
      }
      ...
      await page
        .locator('.fabricate-interactables-manager [data-interactable-manager-promote-confirm]')
        .first()
        .scrollIntoViewIfNeeded();
      await page
        .locator('.fabricate-interactables-manager .fab-im-promote-actions')
        .first()
        .waitFor({ state: 'visible', timeout: 5000 });
      await assertNoScreenshotOverlays(page, {
        allowFabricateWindowIds: ['fabricate-interactables-manager'],
      ...
      await page.evaluate(() => game.fabricate.api.getInteractablesManagerAppClass().show());
      await page
        .locator('.fabricate-interactables-manager')
        .first()
        .waitFor({ state: 'visible', timeout: 10_000 });
      await page
      ...
      results.steps.push({ step: 'interactables-manager', passed: false, error: error.message });
      process.stderr.write(`Manage Interactables capture failed: ${error.message}\n`);
    }
  },
};
