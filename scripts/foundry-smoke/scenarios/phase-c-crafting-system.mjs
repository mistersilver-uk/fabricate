...
export default {
  ...
  async run(ctx, { runChildren }) {
    ...
    if (!phaseBPassed) {
      ...
      results.steps.push({
        step: 'create-crafting-system',
        passed: false,
        error: 'Skipped: Phase B failed',
      });
    }
    ...
    if (phaseBPassed) {
      ...
      try {
        ...
        process.stdout.write(
          `  Created crafting system and ${craftingSetup.recipeIds.length} recipes.\n`
        );

        results.steps.push({ step: 'create-crafting-system', passed: true });
        process.stdout.write(
          `Phase C complete: System "${craftingSetup.systemId}" with ${craftingSetup.recipeIds.length} recipes.\n`
        );
        ...
        try {
          ...
          executionFixtures = await seedSmokeCraftExecutionFixtures(
            page,
            craftingSetup,
            cleanup.crafterId
          );
          ...
          results.steps.push({
            step: 'seed-craft-execution-fixtures',
            passed: false,
            error: error.message,
          });
        ...
        if (RUN_SCREENSHOT_PHASES) {
          try {
            ...
            alchemyFixtures = await seedSmokeAlchemyFixtures(
              page,
              craftingSetup,
              cleanup.crafterId
            );
            ...
            results.steps.push({
              step: 'seed-alchemy-fixtures',
              passed: false,
              error: error.message,
            });
            process.stderr.write(`Seeding alchemy fixtures failed: ${error.message}\n`);
          }
        }
        ...
        if (RUN_FULL_ONLY_BEHAVIORS) {
          try {
            ...
            await page
              .locator(
                '#sidebar [data-tab="items"][aria-selected="true"], #sidebar [data-application-part="items"]'
              )
              .first()
              .waitFor({ state: 'visible', timeout: 5000 })
              .catch(() => {
                /* selectors vary across V13 sheets — best-effort */
              });
            if (
              !otherGatheringSystemsEnabled &&
              (await page.locator('button[data-fabricate-action="gathering"]').count()) > 0
            ) {
              throw new Error('Gathering action is visible when no system enables gathering.');
            }
            if (otherGatheringSystemsEnabled) {
              results.steps.push({
                step: 'gathering-feature-gate-negative',
                passed: true,
                skipped: true,
              });
            } else {
            ...
            await page
              .locator('button[data-fabricate-action="gathering"]')
              .first()
              .waitFor({ state: 'visible', timeout: 5000 })
              .catch(() => {
                /* tolerate if sidebar hasn't re-rendered in time */
              });
          }
        } else {
          results.steps.push({
        ...
        results.steps.push({ step: 'create-crafting-system', passed: false, error: error.message });
        process.stderr.write(`Phase C failed: ${error.message}\n`);
      }
    } // end if (phaseBPassed)
  },
};
