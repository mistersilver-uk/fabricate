...
export async function runCraftExecutionAsserts(page, fixtures, crafterId) {
  return await page.evaluate(
    async ({ fixtures, crafterId }) => {
      const steps = [];
      const record = (step, passed, error = null) => {
        steps.push({ step, passed, ...(error && { error }) });
      };
      ...
      try {
        ...
        if (after !== before + 1)
          throw new Error(`Smoke Crate inventory ${before} -> ${after}, expected +1`);
        record('exec-craft-simple', true);
      } catch (error) {
        record('exec-craft-simple', false, error.message);
      }
      ...
      try {
        ...
        if (masterAfter !== masterBefore + 1) {
          throw new Error(
            `Masterwork Blade ${masterBefore} -> ${masterAfter}, expected +1 (selected tier group)`
          );
        }
        if (standardAfter !== standardBefore) {
          throw new Error(
            `Standard Blade ${standardBefore} -> ${standardAfter}, expected unchanged (unselected tier group)`
          );
        }
        record('exec-craft-routed-by-check', true);
      } catch (error) {
        record('exec-craft-routed-by-check', false, error.message);
      }
      ...
      try {
        ...
        if (amuletAfter !== amuletBefore + 1) {
          throw new Error(
            `Smoke Amulet ${amuletBefore} -> ${amuletAfter}, expected +1 (chosen set's non-first group)`
          );
        }
        if (ringAfter !== ringBefore) {
          throw new Error(
            `Smoke Ring ${ringBefore} -> ${ringAfter}, expected unchanged (other set's group / resultGroups[0])`
          );
        }
        record('exec-craft-routed-by-ingredients', true);
      } catch (error) {
        record('exec-craft-routed-by-ingredients', false, error.message);
      }
      ...
      try {
        ...
        record('exec-craft-progressive', false, error.message);
      }
      ...
      try {
        ...
        record('exec-tool-breakage-chance', false, error.message);
      }
      ...
      try {
        ...
        const first = await game.fabricate.craft(crafter, recipe, {
          componentSourceActors: [crafter],
        });
        ...
        if (!chiselAfterFirst)
          throw new Error('Smoke Chisel tool item not found after first craft');
        if (isBroken(chiselAfterFirst)) {
          throw new Error('Smoke Chisel broke before reaching maxUses (sub-threshold craft)');
        }
        const second = await game.fabricate.craft(crafter, recipe, {
          componentSourceActors: [crafter],
        });
        ...
        record('exec-tool-breakage-limited-uses', false, error.message);
      }
      ...
      try {
        ...
        if (result.success !== false)
          throw new Error('craft succeeded but the required tool is absent');
        ...
        if (bracketAfter !== bracketBefore) {
          throw new Error(
            `Smoke Bracket ${bracketBefore} -> ${bracketAfter}, expected no product on gated craft`
          );
        }
        record('exec-negative-tool-gating', true);
      } catch (error) {
        record('exec-negative-tool-gating', false, error.message);
      }
      ...
      try {
        ...
        if (shardAfter <= shardBefore) {
          throw new Error(
            `Smoke Shard ${shardBefore} -> ${shardAfter}, expected increase from salvage`
          );
        }
        record('exec-salvage-run', true);
      } catch (error) {
        record('exec-salvage-run', false, error.message);
      }
      ...
      try {
        ...
        if (result?.accepted !== true) {
          const reason =
            result?.blockedReasons?.[0]?.code || result?.blockedReasons?.[0] || 'unknown';
          throw new Error(
            `gather not accepted (state=${result?.state}, blocked=${JSON.stringify(reason)})`
          );
        }
        if (after <= before) {
          throw new Error(
            `Mystic Herb ${before} -> ${after}, expected increase from guaranteed-success gather`
          );
        }
        record('exec-gather-inventory-delta', true);
      } catch (error) {
        record('exec-gather-inventory-delta', false, error.message);
      }
...
export async function runFullProfileGatherAsserts(page, craftingSetup, gatherFixture, crafterId) {
  return await page.evaluate(
    async ({ arcaneSystemId, hazardEnvironmentId, hazardTaskId, crafterId }) => {
      const steps = [];
      const record = (step, passed, error = null) => {
        steps.push({ step, passed, ...(error && { error }) });
      };
      ...
      try {
        ...
        if (created.length > 0 || after !== before) {
          throw new Error(
            `0%-drop gather awarded items (createdResults=${created.length}, inv ${before}->${after})`
          );
        }
        record('exec-gather-empty', true);
      } catch (error) {
        record('exec-gather-empty', false, error.message);
      }
      ...
      try {
        ...
        record('exec-gather-scene-blocked', false, error.message);
      }
      ...
      try {
        ...
        if (brambleIndex === -1) throw new Error('smoke-bramble-event not seeded (full profile)');
        ...
        record('exec-gather-hazard-event', false, error.message);
      }
