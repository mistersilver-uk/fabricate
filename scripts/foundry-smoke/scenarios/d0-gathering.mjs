...
export default {
  ...
  async run(ctx) {
    ...
    if (
      (await page
        .locator('.fabricate-manager .manager-environment-row.is-selected:has-text("Azure Grove")')
        .count()) === 0
    ) {
      throw new Error('Manager environments browser did not show selected environment row state.');
    }
    const sceneEvidenceCount = await page
      .locator('.fabricate-manager .manager-inspector [data-environment-fact="scene"]')
      .count();
    const sceneStatusCount = await page
      .locator(
        '.fabricate-manager .manager-inspector:has-text("Linked scene"), .fabricate-manager .manager-inspector:has-text("Scene unresolved")'
      )
      .count();
    ...
    if (
      preTaskLibraryCounts.viewTasks < 1 ||
      preTaskLibraryCounts.viewEvents < 1 ||
      preTaskLibraryCounts.viewTools < 1
    ) {
      throw new Error(
        `Manager smoke gathering library disappeared before task screenshot: ${JSON.stringify(preTaskLibraryCounts)}`
      );
    }
    await page.locator('.fabricate-manager #manager-gathering-nav-tasks').first().click();
    await page
      .locator('.fabricate-manager .manager-gathering-task-row:has-text("Forage Wild Herbs")')
      .first()
      .waitFor({ state: 'visible', timeout: 10_000 });
    await page
      .locator(
        '.fabricate-manager .manager-gathering-task-row:has-text("Forage Wild Herbs") [aria-label^="Edit"]'
      )
      .first()
      .click();
    await page
    ...
    await page
      ...
      .click();
    await page
      .locator('.fabricate-manager[data-manager-view="environment-edit"]')
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });

    // The environment editor mounts the composition editor (tabs + inspector rail).
    await page
      .locator('.fabricate-manager .manager-environment-edit-view[data-environment-editor]')
      .first()
      .waitFor({ state: 'visible', timeout: 10_000 });
    ...
    await page
      .locator('.fabricate-manager .manager-title')
      .filter({ hasText: 'Edit environment' })
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });
    const editedEnvNameField = page
      .locator('.fabricate-manager [data-environment-field="name"]')
      .first();
    await editedEnvNameField.waitFor({ state: 'visible', timeout: 5000 });
    const editedEnvName = await editedEnvNameField.inputValue();
    if (editedEnvName !== 'Azure Grove') {
      throw new Error(
        `Environment editor loaded the wrong environment: expected "Azure Grove", got "${editedEnvName}".`
      );
    }
    if (
      (await page
        .locator(
          '.fabricate-manager .environment-draft-editor, .fabricate-manager .environment-foundation'
        )
        .count()) > 0
    ) {
      throw new Error(
        'Manager environments edit route still rendered the legacy environment editor.'
      );
    }
    await assertNoScreenshotOverlays(page);
    ...
    for (const [tabId, label] of [
      ...
      await page
        .locator(`.fabricate-manager [data-environment-tab-button="${tabId}"]`)
        .first()
        .click();
      await page
        .locator(`.fabricate-manager [data-environment-tab="${tabId}"]`)
        .first()
        .waitFor({ state: 'visible', timeout: 5000 });
      await page.waitForTimeout(250);
      await assertNoScreenshotOverlays(page);
    ...
    await page
      .locator('.fabricate-manager [data-environment-tab-button="overview"]')
      .first()
      .click();
    await page
      .locator('.fabricate-manager [data-environment-tab="overview"]')
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });
    ...
    await page.locator('.fabricate-manager #manager-gathering-nav-environments').first().click();
    await page
      .locator('.fabricate-manager[data-manager-view="environments"]')
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });
    ...
    await page
      .locator('.fabricate-manager .manager-gathering-event-row:has-text("Bramble Snare")')
      .first()
      .waitFor({ state: 'visible', timeout: 10_000 });
    ...
    await page
      .locator(
        '.fabricate-manager .manager-gathering-event-row:has-text("Bramble Snare") [aria-label^="Edit"]'
      )
      .first()
      .click();
    await page
      .locator('.fabricate-manager[data-manager-view="gathering-event-edit"]')
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });
    ...
    await page
      .locator('.fabricate-manager #manager-world-nav-travel[aria-expanded="true"]')
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });
    await captureStableManagerView(ctx, {
    ...
    const gatheringToggle = page
      .locator('.fabricate-manager .manager-nav-toggle[aria-controls="manager-gathering-submenu"]')
      .first();
    ...
    if (!gatheringWasExpanded) {
      ...
      await gatheringSubmenu.waitFor({ state: 'visible', timeout: 5000 });
    }
    await captureStableManagerView(ctx, {
    ...
    if (!gatheringWasExpanded) {
      await gatheringToggle.click();
      await gatheringSubmenu.waitFor({ state: 'hidden', timeout: 5000 });
    }

    await page.locator('.fabricate-manager #manager-world-nav-parties').first().click();
    await page
      .locator('.fabricate-manager .manager-travel-parties-row')
      .first()
      .waitFor({ state: 'visible', timeout: 10_000 });
    await captureStableManagerView(ctx, {
    ...
    await page
      .locator('.fabricate-manager [data-travel-panel="realms"]')
      .first()
      .waitFor({ state: 'visible', timeout: 10_000 });
    await captureStableManagerView(ctx, {
    ...
    await page
      .locator('.fabricate-manager [data-travel-panel="map"]')
      .first()
      .waitFor({ state: 'visible', timeout: 10_000 });
    await page
      .locator(
    ...
    await page
      .locator('.fabricate-manager .manager-body.is-rail-collapsed')
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });
    await captureStableManagerView(ctx, {
    ...
    await page.locator('.fabricate-manager [data-manager-rail-toggle]').first().click();
    await page
      .locator('.fabricate-manager .manager-body:not(.is-rail-collapsed)')
      .first()
      .waitFor({ state: 'visible', timeout: 5000 });
    ...
    try {
      ...
      await page
        .locator('.fabricate-manager #manager-world-nav-travel')
        .first()
        .waitFor({ state: 'visible', timeout: 5000 });
      await captureStableManagerView(ctx, {
