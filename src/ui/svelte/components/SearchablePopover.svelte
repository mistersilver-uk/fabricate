...
<script>
  ...
  // The rendered LIST node (issue 1503). `measureListMetrics` reads it, and the action's
  // `targets.list` is the element `anchoredPopover` writes a height onto.
  let optionsList = $state(null);
  // The trigger's rendered DOM node — RENAMED from `triggerButton`, which is now the name of
  // the prop that chooses the trigger's FORM. All FOUR forms write it: the bare `<button>` by
  // `bind:this`, `Chip` and `ManagerButton` by the `element` binding each of those primitives
  // publishes for exactly this caller, and the `trigger` SNIPPET (issue 1503) by the attachment
  // in `triggerSnippetAttributes` below, because `bind:this` cannot cross a snippet boundary.
  //
