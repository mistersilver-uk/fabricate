...
<script>
  ...
  // Per-task gathering DC override (progressive has no DC, so the field shows only for routed;
  // d100 has none either). null = use the system gathering check default DC. `Stepper` renders an
  // unset value blank on its own, so this only normalizes `undefined` to `null`.
  ...
  function onQuantityKeydown(row, event) {
    ...
    event.preventDefault();
    const currentValue =
      event.currentTarget.value === '' ? quantityValue(row) : Number(event.currentTarget.value);
    const quantity = normalizeQuantity(
      (Number.isFinite(currentValue) ? currentValue : quantityValue(row)) +
    ...
    <section class="manager-task-availability-card">
      <div class="manager-task-card-heading">
        <div>
          <h3>
            {text(
              'FABRICATE.Admin.Manager.Environment.Tasks.TaskAvailability',
              'Task Availability'
            )}
          </h3>
          <p class="manager-muted">
    ...
    <section class="manager-task-drops-card">
      <div class="manager-task-card-header">
        ...
        <div class="manager-task-drop-controls">
          ...
          />
          <!-- Primary (issue 1118): the section's CREATE action in toolbar chrome is the loud one,
               and the identical verb in this screen's own empty state was already `is-primary`. -->
...
<style>
  ...
  /* The DC override is a single field on an otherwise empty card, so nothing in the chain above it
     ever gave it a width and the filled stepper resolved `width: 100%` against the whole card,
     putting the − and + most of a foot apart.
