...
<script>
  ...
  function inRealm(environment) {
    return (
      Array.isArray(environment?.includedRealmIds) && environment.includedRealmIds.includes(realmId)
    );
  }

  const includedEnvironments = $derived(environments.filter(inRealm));
  const availableEnvironments = $derived(
    environments.filter((environment) => !inRealm(environment))
  );

  function filterByName(list, term) {
    const normalized = term.trim().toLowerCase();
    return normalized
      ? list.filter((environment) =>
  ...
  const pagedAvailable = $derived(
    filteredAvailable.slice(availablePage * PAGE_SIZE, (availablePage + 1) * PAGE_SIZE)
  );
  const pagedIncluded = $derived(
    filteredIncluded.slice(includedPage * PAGE_SIZE, (includedPage + 1) * PAGE_SIZE)
  );
</script>
  ...
  <div class="manager-realm-env-editor" data-manager-realm-env-editor={realmId}>
    <section class="manager-realm-env-column" data-realm-env-column="available">
      <h4 class="manager-card-subtitle">
        ...
        {text(
          'FABRICATE.Admin.Manager.Travel.Realms.AvailableEnvironments',
      ...
      {#if filteredAvailable.length === 0}
        <p class="manager-muted">
          {availableEnvironments.length === 0
            ? text(
                'FABRICATE.Admin.Manager.Travel.Realms.NoAvailableEnvironments',
                'All environments are in this realm.'
              )
            : text(
                'FABRICATE.Admin.Manager.Travel.Realms.NoEnvironmentMatches',
    ...
    <section class="manager-realm-env-column" data-realm-env-column="included">
      <h4 class="manager-card-subtitle">
        ...
        {text(
          'FABRICATE.Admin.Manager.Travel.Realms.IncludedEnvironments',
      ...
      {#if filteredIncluded.length === 0}
        <p class="manager-muted">
          {includedEnvironments.length === 0
            ? text(
                'FABRICATE.Admin.Manager.Travel.Realms.NoIncludedEnvironments',
                'No environments are in this realm yet.'
              )
            : text(
                'FABRICATE.Admin.Manager.Travel.Realms.NoEnvironmentMatches',
                'No environments match your search.'
