...
<script>
  ...
  const suggestedDangerTags = $derived(
    DEFAULT_DANGER_TAGS.filter((tag) => !dangerTags.includes(tag))
  );
  ...
  const dropRateValid = $derived(
    Number.isFinite(dropRateRaw) && dropRateRaw >= 1 && dropRateRaw <= 100
  );
  const dropRateValue = $derived(
    Math.max(1, Math.min(100, Math.floor(Number.isFinite(dropRateRaw) ? dropRateRaw : 1)))
  );
  ...
  function availabilityMenuLabel(kind) {
    ...
    if (available.length === 0) {
      if (kind === 'weather')
        return text(
          'FABRICATE.Admin.Manager.Environment.Events.AllWeatherSelected',
          'All weather selected'
        );
      if (kind === 'biomes')
        return text(
          'FABRICATE.Admin.Manager.Environment.Events.AllBiomesSelected',
          'All biomes selected'
        );
      return text(
        'FABRICATE.Admin.Manager.Environment.Events.AllTimesSelected',
        'All times selected'
      );
    }
    if (kind === 'weather')
      return text('FABRICATE.Admin.Manager.Environment.Events.AddWeatherCondition', 'Add weather');
    if (kind === 'biomes')
      return text('FABRICATE.Admin.Manager.Environment.Events.AddBiomeCondition', 'Add biome');
    return text(
      'FABRICATE.Admin.Manager.Environment.Events.AddTimeOfDayCondition',
      'Add time of day'
    );
  }
  ...
  function emptyAvailabilityLabel(kind) {
    if (kind === 'weather')
      return text('FABRICATE.Admin.Manager.Environment.Events.AnyWeatherTitle', 'Any Weather');
    if (kind === 'biomes')
      return text('FABRICATE.Admin.Manager.Environment.Events.AnyBiomeTitle', 'Any Biome');
...
>
  {#if event}
    <section class="manager-task-core-card" data-gathering-event-core-editor>
      <div class="manager-task-card-heading">
        <div>
          <h3>
            {text('FABRICATE.Admin.Manager.Environment.Events.EventIdentity', 'Event Identity')}
          </h3>
          <p class="manager-muted">
            {text(
              'FABRICATE.Admin.Manager.Environment.Events.EventIdentityHint',
              'Name the event, give it a description, choose an image, and toggle whether it is enabled.'
            )}
          </p>
        </div>
      </div>
        ...
        <div class="manager-task-media-column">
          ...
          <div class="manager-task-core-status">
            ...
            <p class="manager-muted">
              {event.enabled === false
                ? text(
                    'FABRICATE.Admin.Manager.Environment.Events.DisabledHint',
                    'Disabled events never match player gathering.'
                  )
                : text(
                    'FABRICATE.Admin.Manager.Environment.Events.EnabledHint',
                    'This event is available when its tags match.'
                  )}
            </p>
          </div>
        </div>
        ...
        <div class="manager-task-identity-fields">
          <label class="manager-field">
            ...
            <input
              ...
              value={event.name || ''}
              oninput={(event) => onUpdateEvent({ name: event.currentTarget.value })}
            />
            {#if !nameValid}
              <span class="manager-field-error"
                >{text(
                  'FABRICATE.Admin.Manager.Environment.Events.NameRequired',
                  'Name is required.'
                )}</span
              >
            {/if}
          ...
          <label class="manager-field">
            ...
            >
            <textarea
              data-gathering-event-field="description"
    ...
    <section class="manager-task-availability-card">
      <div class="manager-task-card-heading">
        <div>
          <h3>
            {text('FABRICATE.Admin.Manager.Environment.Events.EventAvailability', 'Event Matching')}
          </h3>
          <p class="manager-muted">
            {text(
              'FABRICATE.Admin.Manager.Environment.Events.AvailabilityHint',
              'Events match environments by biome, weather, and time of day. Empty fields mean "matches any".'
            )}
          </p>
        </div>
      </div>
      ...
      <section class="manager-task-availability-card" data-gathering-event-danger-tags>
        <div class="manager-task-card-heading">
          <div>
            ...
            <p class="manager-muted">
              {text(
                'FABRICATE.Admin.Manager.Environment.Events.DangerTagsHint',
                'Danger tags let environments opt in to this event. Empty matches any environment danger profile.'
              )}
            </p>
          </div>
        </div>
                  ...
                  <span class={`manager-danger-tag-pill is-${tag}`} data-danger-tag={tag}>
                    ...
                    <button
                      ...
                      aria-label={text(
                        ...
                        'Remove {name}'
                      ).replace('{name}', dangerLabel(tag))}
                      onclick={() => removeDangerTag(tag)}
                    >
                      <i class="fas fa-xmark" aria-hidden="true"></i>
                    </button>
                  </span>
      ...
      <section class="manager-task-availability-card" data-gathering-event-drop-rate>
        <div class="manager-task-card-heading">
          <div>
            ...
            <p class="manager-muted">
              {text(
                'FABRICATE.Admin.Manager.Environment.Events.DropRateHint',
                'Chance from 1 to 100 that this event drops on a matching gathering attempt before modifiers.'
              )}
            </p>
          </div>
        </div>
    ...
    <section
      class="manager-task-availability-card manager-gathering-event-scene"
      data-gathering-event-scene
    >
      <div class="manager-task-card-heading">
        <div>
          ...
          <p class="manager-muted">
            {text(
              'FABRICATE.Admin.Manager.Environment.Events.SceneLinkHint',
              'Optionally link a scene to open when this event is triggered.'
            )}
          </p>
        </div>
      </div>
      {#if linkedSceneUuid}
        ...
        <div
          ...
          title={text(
            'FABRICATE.Admin.Manager.Environment.Events.SceneRemoveTooltip',
            'Right-click to remove the linked scene'
          )}
          oncontextmenu={(event) => {
            event.preventDefault();
            unlinkScene();
        ...
        >
          ...
          {:else}
            <span class="manager-gathering-event-scene-thumb is-placeholder" aria-hidden="true"
      ...
      {:else}
        ...
        >
          ...
          <span
            >{text(
              'FABRICATE.Admin.Manager.Environment.Events.SceneDropHint',
  ...
  {:else}
    <EmptyState
      ...
      hint={text(
        'FABRICATE.Admin.Manager.Environment.Events.SelectEventHint',
        'Choose an event from the browser to edit its identity, matching, and modifiers.'
      )}
    />
  {/if}
</main>
