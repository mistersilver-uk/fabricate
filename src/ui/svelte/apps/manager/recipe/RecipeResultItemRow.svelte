...
<!--
  ...
  ── THE COMPLICATION BAND LEFT THIS FILE (issue 1512) ────────────────────────
  The read-only complication strip issue 1286 put inside this row is
  `RecipeStageComplicationBand.svelte` now, and the parent renders it as the shared list's
  BODY. So did the `display: contents` wrapper and line this file carried to make it
  full-bleed, and the `leadingControls` / `reorderControls` snippets that existed so the
  grip and the ordinal could sit inside that line: `SortableList` draws the grip, the
  ordinal badge and the chevron rocker itself, around whatever this row renders.
