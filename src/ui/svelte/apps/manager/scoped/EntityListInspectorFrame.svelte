...
<!--
  ...
  the bar takes a slot instead of choosing between them. Issue 1504 converted these two controls
  to the shared `<Select>` and the class became the SELECT ROOT's rather than the element's, so
  `styles/fabricate.css`'s `r8-prim` block now carries the rung at
  `.manager-scoped-list-toolbar .is-size-38 .fabricate-select-trigger` — a call-site rule reaching
  a primitive-rendered trigger from a class the caller owns, which is the one shape the shared
  select's API leaves for a per-site box. It is (0,4,0) against the rung's own (0,2,0), so it
  still wins on specificity rather than on source order, and `is-size-38` stays the opt-in rather
  than becoming a fourth published `Select` size.
-->
{#snippet laneFilterSelects(row)}
  {#each laneFilters as filter (filter.id)}
    {#if !row || (filter.toolbarRow ?? 'lead') === row}
      ...
      {/if}
      <!--
        THE APP'S OWN LIST, AT THIS ROW'S OWN RUNG (issue 1504). Both toolbar controls were
        native `<select>`s, so their option lists were drawn by the operating system: no theme,
        no tick beside the live value, and a popup that differs per platform. They are shared
        ...
        `size="toolbar"` rather than the published `inline` rung, because this control line is a
        documented 34px / radius 9 band: the search field beside it, the direction toggle whose
        own comment records that its metrics are copied from these selects, and the compact
        segmented control are all sized to it. Dropping two of the five to 30 would break the row
        to adopt a rung; issue 1523 may collapse it properly.

        THE LEAD ROW'S 38px OPT-IN SURVIVES THE CONVERSION AS A CALL-SITE CLASS. Issue 1371's
        `is-size-38` was written for a `<select>` element and a converted control is a `<button>`
        inside a picker root, so the class rides the `Select` ROOT through `class` and the sheet
        rule that grants the rung is retargeted onto the trigger beneath it. That keeps the rung
        an opt-in stated by the caller — `leadSelectSizeClass` is unchanged — rather than a
        fourth published `Select` size.

        Each trigger keeps the `data-*` hook it had, through `triggerData`, so every capture
        step, mounted test and smoke step that drives these controls still addresses the control
