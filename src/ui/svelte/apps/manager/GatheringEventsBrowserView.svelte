...
<script>
  ...
  const filteredEvents = $derived(
    eventList.filter((event) => {
      ...
      const haystack =
        `${eventName(event)} ${event.description || ''} ${dangerTags.join(' ')}`.toLowerCase();
...
>
  ...
  <section
    ...
    aria-label={text('FABRICATE.Admin.Manager.Environment.Events.Table', 'Gathering events table')}
  >
    {#if eventList.length === 0}
      <EmptyState
        icon="fas fa-masks-theater"
        title={text(
          'FABRICATE.Admin.Manager.Environment.Events.EmptyTitle',
          'No gathering events yet'
        )}
        hint={text(
          'FABRICATE.Admin.Manager.Environment.Events.EmptyHint',
    ...
    {:else if filteredEvents.length === 0}
      <EmptyState
        icon="fas fa-search"
        title={text(
          'FABRICATE.Admin.Manager.Environment.Events.EmptySearchTitle',
          'No events match these filters'
        )}
        hint={text(
          'FABRICATE.Admin.Manager.Environment.Events.EmptySearchHint',
          ...
          >
            ...
            >
              ...
              <span class="manager-system-copy">
                ...
                {#if event.description}
                  <span class="manager-system-description" title={event.description}
            ...
            >
              <button
                ...
                title={text('FABRICATE.Admin.Manager.Environment.Events.Edit', 'Edit event')}
