...
<script>
  ...
  function anyLabel(field) {
    return text(
      `FABRICATE.Admin.Manager.EnvironmentEditor.Evidence.${field.label}Any`,
      field.anyFallback
    );
  }
  ...
  const shown = $derived(
    FIELDS.filter((field) => {
      ...
      return {
        ...
        entry,
        count: count(entry),
        value: valueText(field, entry),
        tone: tone(field, entry),
        pills: valuePills(field, entry),
      };
    })
  );
...
{#if variant === 'checks'}
  <table
    class="manager-environment-evidence is-checks manager-environment-evidence-table"
    aria-label={text(
      'FABRICATE.Admin.Manager.EnvironmentEditor.Evidence.Title',
      'Matching evidence'
    )}
  >
    <tbody>
      {#each shown as row (row.field.key)}
        <tr
          class={`manager-environment-evidence-row ${row.tone}`}
          data-evidence-field={row.field.key}
          data-evidence-state={row.entry?.state || 'any'}
        >
          ...
          <td class="manager-environment-evidence-values">
            <div class="manager-environment-evidence-value-list">
              {#each row.pills as pill (pill.id)}
                <span
                  class={`manager-environment-evidence-value-pill ${pill.tone}`}
                  data-evidence-value-state={pill.state}>{pill.value}</span
                >
              {/each}
            </div>
          </td>
...
{:else}
  <div
    class="manager-environment-evidence is-chips"
    aria-label={text(
      'FABRICATE.Admin.Manager.EnvironmentEditor.Evidence.Title',
      'Matching evidence'
    )}
