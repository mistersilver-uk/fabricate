...
<section class="manager-environment-tab" data-environment-tab="tasks" aria-label={text('FABRICATE.Admin.Manager.Environment.Tasks.Title', 'Tasks')}>
  <header class="manager-environment-comp-header">
    <div class="manager-environment-comp-header-copy">
      <h3 class="manager-card-title">{text('FABRICATE.Admin.Manager.Environment.Tasks.Composition', 'Task composition')}</h3>
      <p class="manager-muted">{text('FABRICATE.Admin.Manager.Environment.Tasks.CompositionHint', 'Determine which gathering tasks are available in this environment and how they behave at runtime.')}</p>
    </div>
  ...
  <p class="manager-environment-comp-callout" data-composition-mode={mode}>
    ...
    <span>{mode === 'manual'
      ? text('FABRICATE.Admin.Manager.Environment.Tasks.ManualIntro', 'Only tasks you explicitly include and that still match the environment are available to players.')
      : text('FABRICATE.Admin.Manager.Environment.Tasks.AutomaticIntro', 'All matching enabled library tasks are available unless you exclude them here.')}</span>
  </p>
