...
<section class="manager-environment-tab" data-environment-tab="hazards" aria-label={text('FABRICATE.Admin.Manager.EnvironmentEditor.Hazards.Title', 'Hazards')}>
  <header class="manager-environment-comp-header">
    <div class="manager-environment-comp-header-copy">
      <h3 class="manager-card-title">{text('FABRICATE.Admin.Manager.EnvironmentEditor.Hazards.Composition', 'Hazard composition')}</h3>
      <p class="manager-muted">{text('FABRICATE.Admin.Manager.EnvironmentEditor.Hazards.CompositionHint', 'Hazards trigger as part of a gathering task attempt. They match the environment at the library level; here you compose which apply.')}</p>
    </div>
