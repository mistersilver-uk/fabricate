...
{#if party}
  <div class="manager-party-body" data-manager-party-body={party.id}>
    <section
      class="manager-party-members"
      aria-label={text('FABRICATE.Admin.Manager.Travel.Members.SectionLabel', 'Party members')}
    >
      <div class="manager-party-members-head">
        ...
        <div class="manager-party-add" data-manager-party-add>
          <span class="manager-card-subtitle"
            >{text('FABRICATE.Admin.Manager.Travel.Members.AddLabel', 'Add members')}</span
          >
          <SearchablePopover
            ...
            triggerAriaLabel={text(
              'FABRICATE.Admin.Manager.Travel.Members.AddLabel',
              'Add members'
            )}
            dialogAriaLabel={text('FABRICATE.Admin.Manager.Travel.Members.AddLabel', 'Add members')}
            searchPlaceholder={text(
              'FABRICATE.Admin.Manager.Travel.Members.AddSearchPlaceholder',
              'Search characters...'
            )}
            searchAriaLabel={text(
              'FABRICATE.Admin.Manager.Travel.Members.AddSearchLabel',
      ...
      <ul class="manager-party-member-rows" data-manager-party-member-rows>
        {#each party.memberCards as member (member.uuid)}
          <li
            class={`manager-party-member-row ${member.stale ? 'is-stale' : ''}`}
            data-member-uuid={member.uuid}
          >
            <span class="manager-travel-portrait" aria-hidden="true">
              {#if member.img}<img src={member.img} alt="" />{:else}<i class={FALLBACK_PORTRAIT}
                ></i>{/if}
            </span>
            ...
            <div class="manager-party-member-actions">
              {#if !member.stale}
                <SearchablePopover
                  ...
                  showChevron={false}
                  triggerAriaLabel={text(
                    'FABRICATE.Admin.Manager.Travel.Members.MoveLabel',
                  ...
                  searchPlaceholder={text(
                    'FABRICATE.Admin.Manager.Travel.Members.MoveSearchPlaceholder',
                    'Search parties...'
                  )}
                  searchAriaLabel={text(
                    'FABRICATE.Admin.Manager.Travel.Members.MoveSearchLabel',
                    'Search parties'
                  )}
                  emptyHint={text(
                    'FABRICATE.Admin.Manager.Travel.Members.NoOtherParties',
                    'No other parties to move to.'
                  )}
              ...
              <button
                ...
                aria-label={text(
                  'FABRICATE.Admin.Manager.Travel.Members.RemoveLabel',
                  'Remove member'
                )}
        ...
        {:else}
          <li class="manager-party-member-empty">
            <p class="manager-muted">
              {text('FABRICATE.Admin.Manager.Travel.Members.Empty', 'No members yet.')}
            </p>
          </li>
        {/each}
      </ul>
    ...
    <section
      class="manager-party-travel-marker"
      aria-label={text('FABRICATE.Admin.Manager.Travel.Marker.SectionLabel', 'Travel marker')}
    >
      <span class="manager-card-subtitle"
        >{text('FABRICATE.Admin.Manager.Travel.Marker.Label', 'Travel marker')}</span
      >
      <div
        ...
        tabindex="0"
        aria-label={text(
          'FABRICATE.Admin.Manager.Travel.Marker.DropLabel',
          'Drop an actor to set the travel marker'
        )}
        title={text(
          'FABRICATE.Admin.Manager.Travel.Marker.DropHint',
          'Drag an actor here to set the travel marker. Right-click to clear.'
        )}
      ...
      >
        {#if party.travelActor}
          <span class="manager-party-marker-portrait" aria-hidden="true">
            {#if party.travelActor.img}<img src={party.travelActor.img} alt="" />{:else}<i
                class={FALLBACK_PORTRAIT}
              ></i>{/if}
          </span>
          <span class="manager-party-marker-name">{party.travelActor.name}</span>
        {:else if party.staleTravelActor}
          <span class="manager-party-marker-name is-stale"
            >{text(
        ...
        {:else}
          ...
          <span class="manager-party-marker-hint"
            >{text('FABRICATE.Admin.Manager.Travel.Marker.Empty', 'Drag an actor here')}</span
          >
        {/if}
      </div>
      <button
